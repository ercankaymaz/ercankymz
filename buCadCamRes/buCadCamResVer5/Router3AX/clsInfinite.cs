...
// Type: buCadCamResVer5.Router3AX.clsInfinite
...
{
  private string string_0 = nameof (clsInfinite);
      ...
      if (Job != null)
      {
        if (Job.CamList.Count > 0)
        {
          ...
          if (Cams.Count > 0)
          {
            ...
            if (!clsRouter3AX.varRouter3AXSettings.DualTable)
              Post1.StartLines.Insert(0, (object) $"( Sheet Size = {num1.ToString("f1")} x {num2.ToString("f1")}");
