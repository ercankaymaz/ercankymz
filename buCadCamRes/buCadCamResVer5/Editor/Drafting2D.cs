...
public class Drafting2D : Design
{
      ...
      else
      {
        ...
        if ((!(this.ActionMode == devDept.Eyeshot.actionType.None & clsInit.appEditor2.action != 0) ? 0 : (e.Button == MouseButtons.Left ? 1 : 0)) != 0 && !Drafting2D.selectionProcess)
        {
          ...
          if ((clsInit.appEditor2.action != actionTypeBU.eventOffset ? 0 : (Drafting2D.points.Count == 1 ? 1 : 0)) != 0)
            this.EventOffset();
          if (clsInit.appEditor2.action == actionTypeBU.libraryVertical && (entity == null ? 0 : (entity is Line ? 1 : 0)) != 0)
            clsInit.appEditor2.CreateConstraintVertical((Line) entity);
          if (clsInit.appEditor2.action == actionTypeBU.libraryHorizontal && (entity == null ? 0 : (entity is Line ? 1 : 0)) != 0)
            clsInit.appEditor2.CreateConstraintHorizontal((Line) entity);
          if (clsInit.appEditor2.action == actionTypeBU.libraryLength && (entity == null ? 0 : (entity is Line ? 1 : 0)) != 0)
            clsInit.appEditor2.CreateConstraintVertical((Line) entity);
        }
  ...
  public void EventScale(bool Override = false, double ScaleRatio = 1.0)
  {
    try
    {
      ...
      else
      {
        if (Drafting2D.points.Count != 3 || this.selEntities.Count <= 0)
          return;
        double factor = Drafting2D.points[0].Pnt3D.DistanceTo(Drafting2D.points[2].Pnt3D) / Drafting2D.points[0].Pnt3D.DistanceTo(Drafting2D.points[1].Pnt3D);
        if (factor > 0.0)
        {
          clsInit.appEditor2.UndoBuffer();
          foreach (Entity selEntity in this.selEntities)
  ...
  public void EventOffset()
  {
    ...
    foreach (Entity selEntity in this.selEntities)
    {
      if (clsVar.varEditorSet.OffsetByMouse)
      {
        ...
        double t;
        curve.Project(Drafting2D.points[0].Pnt3D, out t);
        double amount = curve.PointAt(t).DistanceTo(Drafting2D.points[0].Pnt3D);
        ICurve entity_0 = curve.Offset(amount, Vector3D.AxisZ, true)?[0];
        entity_0.Project(Drafting2D.points[0].Pnt3D, out t);
        if (entity_0.PointAt(t).DistanceTo(Drafting2D.points[0].Pnt3D) > 0.001)
          entity_0 = curve.Offset(-amount, Vector3D.AxisZ, true)?[0];
        this.method_18((Entity) entity_0, this.ActiveLayerName, false, false);
      }
      else
      {
        ICurve curve = selEntity as ICurve;
        double t;
        curve.Project(this.current, out t);
        curve.PointAt(t);
        double offsetValue = clsVar.varEditorRuntimeSet.OffsetValue;
        ICurve entity_0_1 = curve.Offset(offsetValue, Vector3D.AxisZ, true)?[0];
        ICurve entity_0_2 = curve.Offset(-offsetValue, Vector3D.AxisZ, true)?[0];
        entity_0_1.Project(this.current, out t);
        Point3D point3D1 = entity_0_1.PointAt(t);
        entity_0_2.Project(this.current, out t);
        Point3D point3D2 = entity_0_2.PointAt(t);
        if (point3D1.DistanceTo(this.current) < point3D2.DistanceTo(this.current))
          this.method_18((Entity) entity_0_1, this.ActiveLayerName, false, false);
        else
          this.method_18((Entity) entity_0_2, this.ActiveLayerName, false, false);
      }
    }
    this.Entities.Regen();
    this.ClearAllPreviousCommandData();
  ...
  public void EventFillet()
  {
    ...
    else
    {
      try
      {
        ...
        double num2 = Point3D.Distance(this.point3D_2, firstSelectedEntity1.EndPoint);
        if ((num2 >= num1 ? 0 : (secondSelectedEntity1 is Arc ? 1 : (secondSelectedEntity1 is EllipticalArc ? 1 : 0))) != 0)
          firstSelectedEntity1.Reverse();
        double num3 = Point3D.Distance(this.point3D_1, secondSelectedEntity1.StartPoint);
  ...
  private void method_13()
  {
    ...
    if (Drafting2D.points.Count != 2)
      return;
    double num = Drafting2D.points[0].Pnt3D.DistanceTo(this.current) / Drafting2D.points[0].Pnt3D.DistanceTo(Drafting2D.points[1].Pnt3D);
    if (num <= 0.0)
      return;
    foreach (Entity selEntity in this.selEntities)
    {
  ...
  public void EventOffsetEntity()
  {
    ...
    foreach (Entity selEntity in this.selEntities)
    {
      ...
      if (clsVar.varEditorSet.OffsetByMouse)
      {
        ...
        ICurve entity_0 = curve.Offset(amount, Vector3D.AxisZ, true)?[0];
        entity_0.Project(this.current, out t);
        if (entity_0.PointAt(t).DistanceTo(this.current) > 0.001)
          entity_0 = curve.Offset(-amount, Vector3D.AxisZ, true)?[0];
        this.method_15((Entity) entity_0, clsVar.varEditorSet.colorEvent);
      }
      else
      {
        ICurve curve = entity as ICurve;
        ...
        Point3D point3D2 = entity_0_2.PointAt(t);
        if (point3D1.DistanceTo(this.current) < point3D2.DistanceTo(this.current))
          this.method_15((Entity) entity_0_1, clsVar.varEditorSet.colorEvent);
        else
          this.method_15((Entity) entity_0_2, clsVar.varEditorSet.colorEvent);
      }
    }
  }
