...
function Copy-IfExists([string]$from,[string[]]$targets){
  ...
  foreach($target in $targets){$dst=Join-Path $root $target;New-Item -ItemType Directory -Force -Path (Split-Path $dst)|Out-Null;Copy-Item $src $dst -Force;Write-Host "Injected $from -> $target" -ForegroundColor DarkGreen}
}
function Build-Project([string]$relative){
  $project=Join-Path $root $relative;$name=[IO.Path]::GetFileNameWithoutExtension($project);$log=Join-Path $logDir "$name.log"
  ...
  if($LASTEXITCODE -ne 0){throw "Build failed: $relative"};Write-Host "OK: $relative" -ForegroundColor Green
}
...
$cadProject=Join-Path $root 'buCadCamRes/buCadCamRes.csproj';$cadProjectText=[IO.File]::ReadAllText($cadProject)
$compatibilitySources=@('Compatibility\buTrackMarker.cs','Compatibility\buDialogMessageBoxes.cs');$missing=@()
foreach($source in $compatibilitySources){if(-not $cadProjectText.Contains($source)){$missing+=('    <Compile Include="{0}" />' -f $source)}}
if($missing.Count -gt 0){$insert="  <ItemGroup>`r`n"+($missing -join "`r`n")+"`r`n  </ItemGroup>`r`n";$cadProjectText=$cadProjectText.Replace('</Project>',$insert+'</Project>');[IO.File]::WriteAllText($cadProject,$cadProjectText,(New-Object Text.UTF8Encoding($false)))}
...
$probeProject=Join-Path $root 'build/BuCoreProbe/BuCoreProbe.csproj'
& dotnet build $probeProject -c Release --nologo
if($LASTEXITCODE -ne 0){throw 'BuCoreProbe build failed'}
$probeDll=Join-Path $root 'build/BuCoreProbe/bin/Release/net8.0/BuCoreProbe.dll'
& dotnet $probeDll $coreInput (Join-Path $logDir 'buCore-saw-IL.txt')
if($LASTEXITCODE -ne 0){throw 'BuCore IL probe failed'}
...
$coreOutDir=Join-Path $root 'PATCHED_CORE';New-Item -ItemType Directory -Force -Path $coreOutDir|Out-Null
$coreOutput=Join-Path $coreOutDir 'buCore.dll';$coreReport=Join-Path $logDir 'buCore-IL-patch.txt'
