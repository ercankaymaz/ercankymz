...
// Type: buControls.Forms.WinControlForms.Grinding.F_CamGrindingContourClosed
...
public class F_CamGrindingContourClosed : Form
{
  ...
  public Button btn_holecamdata;
  public Button btn_holecancel;
  public Button btn_holeok;
  internal Panel panel_11;

  public F_CamGrindingContourClosed() => Class39.smethod_276(this);
  ...
  public void Init()
  {
    ...
    this.panel_7.Visible = this.IsInsideOperation;
    this.panel_11.Visible = false;
    this.btn_holesetshow.Enabled = this.Operation.InsideOperationHoleEnable;
    ArrayList EnumItems = new ArrayList();
    buGeneral.GetEnumTypeValues((object) this.camPars.Operations.Direction, ref EnumItems);
    buControlCommands.ComboboxAddItem(EnumItems, Convert.ToInt32((object) this.camPars.Operations.Direction), ref this.comboBox_2);
    this.numericUpDown_0.Value = (Decimal) this.camPars.Operations.TargetZ;
    this.numericUpDown_16.Value = (Decimal) this.camPars.Offsets.OverlapDistance;
    this.numericUpDown_5.Value = (Decimal) this.camPars.Offsets.Offset;
    this.numericUpDown_17.Value = (Decimal) this.camPars.Offsets.OffsetCount;
    this.checkBox_2.Checked = this.camPars.Offsets.AddToolDiameterAsOffset;
    ...
    if (this.SelectedTool >= 0 & this.SelectedTool <= this.Tools.Count - 1)
    {
      ...
      for (int index = 0; index <= this.Tools.Count - 1; ++index)
      {
        ...
        this.comboBox_3.Items.Add((object) $"{this.Tools[index].Data.Name} - No : {this.Tools[index].Data.No.ToString()}");
      }
      this.listBox_0.SelectedIndex = this.SelectedTool;
      if (this.Operation.IndieOperationHoleTool >= 0 & this.Operation.IndieOperationHoleTool <= this.Tools.Count - 1)
        this.comboBox_3.SelectedIndex = this.Operation.IndieOperationHoleTool;
    }
    this.numericUpDown_18.Value = (Decimal) this.Operation.IndiseOperationHoleDistance;
    this.checkBox_3.Checked = this.Operation.InsideOperationHoleEnable;
    this.Refresh();
    this.Properties.Result = DialogResult.None;
    this.Properties.Inited = true;
    Class39.smethod_469(this);
  }
  ...
  internal void method_1(object sender, EventArgs e)
  {
    ...
    if (control2.Name == this.btn_next.Name && this.tabControl_0.SelectedIndex < this.tabControl_0.TabPages.Count - 1)
      ++this.tabControl_0.SelectedIndex;
    if (control2.Name == this.btn_holesetshow.Name)
    {
      if (!this.Properties.Inited)
        return;
      if (!this.panel_11.Visible)
        this.panel_11.Visible = true;
      else
        this.panel_11.Visible = false;
    }
    if (control2.Name == this.btn_holecancel.Name)
      this.panel_11.Visible = false;
    if (control2.Name == this.btn_holeok.Name)
    {
      this.Operation.IndiseOperationHoleDistance = (double) this.numericUpDown_18.Value;
      this.Operation.InsideOperationHoleEnable = this.checkBox_3.Checked;
      this.Operation.IndieOperationHoleTool = this.comboBox_3.SelectedIndex;
      this.panel_11.Visible = false;
    }
    if (!(control2.Name == this.btn_holecamdata.Name))
      return;
    F_CamGrindingHole fCamGrindingHole = new F_CamGrindingHole();
    fCamGrindingHole.camPars = new camParameters(this.camParsHole);
    for (int index = 0; index <= this.Tools.Count - 1; ++index)
    {
      ToolBase toolBase = new ToolBase(this.Tools[index]);
      fCamGrindingHole.Tools.Add(toolBase);
    }
    fCamGrindingHole.SelectedTool = this.Operation.IndieOperationHoleTool;
    fCamGrindingHole.Init();
    int num = (int) fCamGrindingHole.ShowDialog();
    if (fCamGrindingHole.Properties.Result != DialogResult.OK)
      return;
    this.camParsHole = new camParameters(fCamGrindingHole.camPars);
  }
  ...
  internal void method_5(object sender, EventArgs e)
  {
    if (!this.Properties.Inited)
      return;
    this.btn_holesetshow.Enabled = this.checkBox_3.Checked;
  ...
  internal void method_7(object sender, EventArgs e) => this.panel_11.Visible = false;
