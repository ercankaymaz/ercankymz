...
// Type: PdfSharp.SharpZipLib.Zip.Compression.DeflaterHuffman
...
{
  private const int BUFSIZE = 16384 /*0x4000*/;
  ...
  private const int EOF_SYMBOL = 256 /*0x0100*/;
  ...
  private static readonly byte[] bit4Reverse = new byte[16 /*0x10*/]
  {
    (byte) 0,
    (byte) 8,
    (byte) 4,
  ...
  {
    int index1;
    for (index1 = 0; index1 < 144 /*0x90*/; DeflaterHuffman.staticLLength[index1++] = (byte) 8)
      DeflaterHuffman.staticLCodes[index1] = DeflaterHuffman.BitReverse(48 /*0x30*/ + index1 << 8);
    for (; index1 < 256 /*0x0100*/; DeflaterHuffman.staticLLength[index1++] = (byte) 9)
      DeflaterHuffman.staticLCodes[index1] = DeflaterHuffman.BitReverse(256 /*0x0100*/ + index1 << 7);
    for (; index1 < 280; DeflaterHuffman.staticLLength[index1++] = (byte) 7)
      DeflaterHuffman.staticLCodes[index1] = DeflaterHuffman.BitReverse(index1 - 256 /*0x0100*/ << 9);
  ...
  public DeflaterHuffman(DeflaterPending pending)
  {
    ...
    this.d_buf = new short[16384 /*0x4000*/];
    this.l_buf = new byte[16384 /*0x4000*/];
  }
  ...
  public void CompressBlock()
  {
    ...
    this.literalTree.WriteSymbol(256 /*0x0100*/);
  }
  ...
  {
    ++this.literalTree.freqs[256 /*0x0100*/];
    ...
    if (num >= extraBits)
      num = extraBits;
    if ((storedOffset < 0 ? 0 : (storedLength + 4 < num >> 3 ? 1 : 0)) != 0)
  ...
  public bool IsFull() => this.last_lit >= 16384 /*0x4000*/;
  ...
  public bool TallyDist(int distance, int length)
  {
    ...
    if ((index1 < 265 ? 0 : (index1 < 285 ? 1 : 0)) != 0)
  ...
  private class Tree
  {
    ...
    public void BuildCodes()
    {
      ...
      for (int index = 0; index < this.numCodes; ++index)
      {
        ...
        if (num2 > 0)
        {
          ...
          numArray[num2 - 1] += 1 << 16 /*0x10*/ - num2;
        }
      }
    }
    ...
    public void BuildTree()
    {
      ...
      for (int index1 = 0; index1 < length; ++index1)
      {
        ...
        if (freq != 0)
        {
          int index2;
          int index3;
          for (index2 = num1++; (index2 <= 0 ? 0 : ((int) this.freqs[numArray1[index3 = (index2 - 1) / 2]] > freq ? 1 : 0)) != 0; index2 = index3)
      ...
      do
      {
        ...
        {
          if ((index9 + 1 >= num1 ? 0 : (numArray2[numArray1[index9]] > numArray2[numArray1[index9 + 1]] ? 1 : 0)) != 0)
            ++index9;
        ...
        int index10;
        while (((index10 = index8) <= 0 ? 0 : (numArray2[numArray1[index8 = (index10 - 1) / 2]] > num7 ? 1 : 0)) != 0)
        ...
        {
          if ((index14 + 1 >= num1 ? 0 : (numArray2[numArray1[index14]] > numArray2[numArray1[index14 + 1]] ? 1 : 0)) != 0)
            ++index14;
          numArray1[index13] = numArray1[index14];
          index13 = index14;
        }
        int index15;
        while (((index15 = index13) <= 0 ? 0 : (numArray2[numArray1[index13 = (index15 - 1) / 2]] > num9 ? 1 : 0)) != 0)
    ...
    public void CalcBLFreq(DeflaterHuffman.Tree blTree)
    {
      ...
      while (index2 < this.numCodes)
      {
        ...
        index1 = index3;
        ++index2;
        while ((index2 >= this.numCodes ? 0 : (index1 == (int) this.length[index2] ? 1 : 0)) != 0)
        {
          ++index2;
          if (++num1 >= num2)
            break;
    ...
    public void WriteTree(DeflaterHuffman.Tree blTree)
    {
      ...
      while (index < this.numCodes)
      {
        ...
        code1 = code2;
        ++index;
        while ((index >= this.numCodes ? 0 : (code1 == (int) this.length[index] ? 1 : 0)) != 0)
        {
          ++index;
          if (++num1 >= num2)
            break;
    ...
    private void BuildLength(int[] childs)
    {
      ...
      do
      {
        ...
        while ((num2 <= 0 ? 0 : (index1 < this.maxLength - 1 ? 1 : 0)) != 0);
      }
      while (num2 > 0);
