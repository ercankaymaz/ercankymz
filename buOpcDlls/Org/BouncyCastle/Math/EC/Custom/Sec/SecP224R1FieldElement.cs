...
internal class SecP224R1FieldElement : AbstractFpFieldElement
{
  ...
  public override int FieldSize => SecP224R1FieldElement.Q.BitLength;
  ...
  {
    uint[] numArray = Nat224.Create();
    SecP224R1Field.Inv(((SecP224R1FieldElement) b).x, numArray);
  ...
  {
    uint[] numArray = Nat224.Create();
    SecP224R1Field.Inv(this.x, numArray);
    return (ECFieldElement) new SecP224R1FieldElement(numArray);
  }
  ...
  public override ECFieldElement Sqrt()
  {
    ...
    uint[] numArray2 = Mod.Random(SecureRandom.ArbitraryRandom, SecP224R1Field.P);
    uint[] numArray3 = Nat224.Create();
    if (!SecP224R1FieldElement.IsSquare(x))
      return (ECFieldElement) null;
    while (!SecP224R1FieldElement.TrySqrt(numArray1, numArray2, numArray3))
      SecP224R1Field.AddOne(numArray2, numArray2);
    SecP224R1Field.Square(numArray3, numArray2);
    return !Nat224.Eq(x, numArray2) ? (ECFieldElement) null : (ECFieldElement) new SecP224R1FieldElement(numArray3);
  }
  ...
  private static void RP(uint[] nc, uint[] d1, uint[] e1, uint[] f1, uint[] t)
  {
    ...
    for (int index = 0; index < 7; ++index)
    {
      ...
      SecP224R1FieldElement.RM(nc, numArray1, numArray2, d1, e1, f1, t);
    }
  }
  ...
  private static bool TrySqrt(uint[] nc, uint[] r, uint[] t)
  {
    ...
    numArray2[0] = 1U;
    uint[] numArray3 = Nat224.Create();
    SecP224R1FieldElement.RP(nc, numArray1, numArray2, numArray3, t);
    ...
    for (int index = 1; index < 96 /*0x60*/; ++index)
    {
      ...
      SecP224R1FieldElement.RS(numArray1, numArray2, numArray3, t);
      if (Nat224.IsZero(numArray1))
      {
        SecP224R1Field.Inv(numArray5, t);
