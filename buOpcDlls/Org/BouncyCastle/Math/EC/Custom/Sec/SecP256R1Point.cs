...
internal class SecP256R1Point : AbstractFpPoint
{
  ...
  {
    return (ECPoint) new SecP256R1Point((ECCurve) null, this.AffineXCoord, this.AffineYCoord);
  }
  ...
  public override ECPoint Add(ECPoint b)
  {
    ...
    {
      numArray4 = numArray2;
      SecP256R1Field.Square(rawZcoord1.x, numArray4, ext1);
      numArray3 = numArray1;
      SecP256R1Field.Multiply(numArray4, rawXcoord2.x, numArray3, ext1);
      SecP256R1Field.Multiply(numArray4, rawZcoord1.x, numArray4, ext1);
      SecP256R1Field.Multiply(numArray4, rawYcoord2.x, numArray4, ext1);
    }
    bool isOne2;
    uint[] numArray5;
    uint[] numArray6;
    ...
    {
      numArray6 = x1;
      SecP256R1Field.Square(rawZcoord2.x, numArray6, ext1);
      numArray5 = ext2;
      SecP256R1Field.Multiply(numArray6, rawXcoord1.x, numArray5, ext1);
      SecP256R1Field.Multiply(numArray6, rawZcoord2.x, numArray6, ext1);
      SecP256R1Field.Multiply(numArray6, rawYcoord1.x, numArray6, ext1);
    }
    ...
    SecP256R1Field.Square(numArray7, numArray9, ext1);
    uint[] numArray10 = Nat256.Create();
    SecP256R1Field.Multiply(numArray9, numArray7, numArray10, ext1);
    uint[] numArray11 = numArray2;
    SecP256R1Field.Multiply(numArray9, numArray5, numArray11, ext1);
    ...
    SecP256R1Field.Square(numArray8, x2.x, ext1);
    ...
    SecP256R1Field.MultiplyAddToExt(y.x, numArray8, ext2);
    SecP256R1Field.Reduce(ext2, y.x);
    SecP256R1FieldElement p256R1FieldElement = new SecP256R1FieldElement(numArray7);
    if (!isOne1)
      SecP256R1Field.Multiply(p256R1FieldElement.x, rawZcoord1.x, p256R1FieldElement.x, ext1);
    if (!isOne2)
      SecP256R1Field.Multiply(p256R1FieldElement.x, rawZcoord2.x, p256R1FieldElement.x, ext1);
    ECFieldElement[] zs = new ECFieldElement[1]
    {
      (ECFieldElement) p256R1FieldElement
    };
    return (ECPoint) new SecP256R1Point(curve, (ECFieldElement) x2, (ECFieldElement) y, zs);
  }
  ...
  public override ECPoint Twice()
  {
    ...
    SecP256R1Field.Square(rawYcoord.x, numArray3, ext);
    uint[] numArray4 = Nat256.Create();
    SecP256R1Field.Square(numArray3, numArray4, ext);
    int num = rawZcoord.IsOne ? 1 : 0;
    uint[] numArray5 = rawZcoord.x;
    if (num == 0)
    {
      numArray5 = numArray2;
      SecP256R1Field.Square(rawZcoord.x, numArray5, ext);
    }
    ...
    SecP256R1Field.Multiply(numArray6, numArray1, numArray6, ext);
    SecP256R1Field.Reduce32(Nat256.AddBothTo(numArray6, numArray6, numArray6), numArray6);
    uint[] numArray7 = numArray3;
    SecP256R1Field.Multiply(numArray3, rawXcoord.x, numArray7, ext);
    ...
    SecP256R1Field.Square(numArray6, x.x, ext);
    ...
    SecP256R1Field.Multiply(y.x, numArray6, y.x, ext);
    ...
    if (num == 0)
      SecP256R1Field.Multiply(p256R1FieldElement.x, rawZcoord.x, p256R1FieldElement.x, ext);
    return (ECPoint) new SecP256R1Point(curve, (ECFieldElement) x, (ECFieldElement) y, new ECFieldElement[1]
    {
      (ECFieldElement) p256R1FieldElement
    });
  }
  ...
  {
    return this.IsInfinity ? (ECPoint) this : (ECPoint) new SecP256R1Point(this.Curve, this.RawXCoord, this.RawYCoord.Negate(), this.RawZCoords);
  }
}
