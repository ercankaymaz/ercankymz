...
public class KeyAgreeRecipientInformation : RecipientInformation
{
    ...
    CmsSecureReadable secureReadable)
  {
    try
    {
      ...
      {
        RecipientEncryptedKey instance = RecipientEncryptedKey.GetInstance((object) recipientEncryptedKey.ToAsn1Object());
        ...
        infos.Add((RecipientInformation) new KeyAgreeRecipientInformation(info, rid, instance.EncryptedKey, secureReadable));
      }
    }
    catch (IOException ex)
    {
      throw new ArgumentException("invalid rid in KeyAgreeRecipientInformation", (Exception) ex);
    }
  }
    ...
    AsymmetricKeyParameter receiverPrivateKey)
  {
    ...
    ICipherParameters cipherParameters1 = (ICipherParameters) senderPublicKey;
    ICipherParameters cipherParameters2 = (ICipherParameters) receiverPrivateKey;
    if (algorithm.Id.Equals(CmsEnvelopedGenerator.ECMqvSha1Kdf))
    {
      MQVuserKeyingMaterial instance = MQVuserKeyingMaterial.GetInstance((object) Asn1Object.FromByteArray(this.info.UserKeyingMaterial.GetOctets()));
      ...
      cipherParameters1 = (ICipherParameters) new MqvPublicParameters((ECPublicKeyParameters) cipherParameters1, (ECPublicKeyParameters) originatorPublicKey);
      cipherParameters2 = (ICipherParameters) new MqvPrivateParameters((ECPrivateKeyParameters) cipherParameters2, (ECPrivateKeyParameters) cipherParameters2);
    }
  ...
  private KeyParameter UnwrapSessionKey(string wrapAlg, KeyParameter agreedKey)
  {
    ...
    wrapper.Init(false, (ICipherParameters) agreedKey);
  ...
  internal KeyParameter GetSessionKey(AsymmetricKeyParameter receiverPrivateKey)
  {
    try
    {
      string id = DerObjectIdentifier.GetInstance((object) Asn1Sequence.GetInstance((object) this.keyEncAlg.Parameters)[0]).Id;
  ...
  public override CmsTypedStream GetContentStream(ICipherParameters key)
  {
    if (!(key is AsymmetricKeyParameter receiverPrivateKey))
      throw new ArgumentException("KeyAgreement requires asymmetric key", nameof (key));
