...
public class SecretKeyPacket : ContainedPacket
{
  ...
  public const int UsageChecksum = 255 /*0xFF*/;
  ...
  {
    this.pubKeyPacket = !(this is SecretSubkeyPacket) ? new PublicKeyPacket(bcpgIn) : (PublicKeyPacket) new PublicSubkeyPacket(bcpgIn);
    this.s2kUsage = bcpgIn.ReadByte();
    if (this.s2kUsage != (int) byte.MaxValue && this.s2kUsage != 254)
    {
    ...
    {
      this.iv = this.encAlgorithm >= SymmetricKeyAlgorithmTag.Aes128 ? new byte[16 /*0x10*/] : new byte[8];
    ...
    byte[] secKeyData)
  {
    ...
    this.s2kUsage = encAlgorithm == SymmetricKeyAlgorithmTag.Null ? 0 : (int) byte.MaxValue;
    this.s2k = s2k;
  ...
  {
    bcpgOut.WritePacket(PacketTag.SecretKey, this.GetEncodedContents());
  }
}
