...
public class NtruLPRimeKemExtractor : IEncapsulatedSecretExtractor
{
  ...
  private void InitCipher(NtruLPRimeParameters param) => this._primeEngine = param.PrimeEngine;

  public byte[] ExtractSecret(byte[] encapsulation)
  {
    byte[] ss = new byte[this._primeEngine.SessionKeySize];
    this._primeEngine.kem_dec(ss, encapsulation, ((NtruLPRimePrivateKeyParameters) this._primeKey).privKey);
    return ss;
  }
