...
public class CommitmentTypeQualifier : Asn1Encodable
{
  ...
  public CommitmentTypeQualifier(Asn1Sequence seq)
  {
    ...
    this.commitmentTypeIdentifier = seq.Count >= 1 && seq.Count <= 2 ? (DerObjectIdentifier) seq[0].ToAsn1Object() : throw new ArgumentException("Bad sequence size: " + seq.Count.ToString(), nameof (seq));
    if (seq.Count <= 1)
      return;
  ...
  {
    Asn1EncodableVector elementVector = new Asn1EncodableVector((Asn1Encodable) this.commitmentTypeIdentifier);
    elementVector.AddOptional((Asn1Encodable) this.qualifier);
