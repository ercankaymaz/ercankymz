...
public sealed class KdfFeedbackBytesGenerator : IMacDerivationFunction, IDerivationFunction
{
  ...
  {
    if (!(parameters is KdfFeedbackParameters feedbackParameters))
      throw new ArgumentException("Wrong type of arguments given");
    this.prf.Init((ICipherParameters) new KeyParameter(feedbackParameters.Ki));
    this.fixedInputData = feedbackParameters.FixedInputData;
    int r = feedbackParameters.R;
    this.ios = new byte[r / 8];
    if (feedbackParameters.UseCounter)
    {
    ...
    this.iv = feedbackParameters.Iv;
    this.useCounter = feedbackParameters.UseCounter;
