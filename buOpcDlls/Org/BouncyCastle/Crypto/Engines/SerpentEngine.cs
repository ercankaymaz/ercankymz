...
public sealed class SerpentEngine : SerpentEngineBase
{
  internal override int[] MakeWorkingKey(byte[] key)
  {
    ...
    for (off = 0; off + 4 < key.Length; off += 4)
      sourceArray[num1++] = (int) Pack.LE_To_UInt32(key, off);
    if (off % 4 != 0)
  ...
  internal override void EncryptBlock(byte[] input, int inOff, byte[] output, int outOff)
  {
    ...
    Pack.UInt32_To_LE((uint) (this.wKey[128 /*0x80*/] ^ this.X0), output, outOff);
  ...
  {
    this.X0 = this.wKey[128 /*0x80*/] ^ (int) Pack.LE_To_UInt32(input, inOff);
