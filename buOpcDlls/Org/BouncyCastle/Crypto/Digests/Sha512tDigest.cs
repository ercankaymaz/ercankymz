...
{
  private const ulong A5 = 11936128518282651045 /*0xA5A5A5A5A5A5A5A5*/;
    ...
    : base((LongDigest) t)
  {
    ...
    this.Reset((IMemoable) t);
  }

  public override string AlgorithmName => "SHA-512/" + (this.digestLength * 8).ToString();
  ...
  public override int DoFinal(byte[] output, int outOff)
  {
    ...
    Sha512tDigest.UInt64_To_BE(this.H3, output, outOff + 16 /*0x10*/, this.digestLength - 16 /*0x10*/);
    Sha512tDigest.UInt64_To_BE(this.H4, output, outOff + 24, this.digestLength - 24);
    Sha512tDigest.UInt64_To_BE(this.H5, output, outOff + 32 /*0x20*/, this.digestLength - 32 /*0x20*/);
    Sha512tDigest.UInt64_To_BE(this.H6, output, outOff + 40, this.digestLength - 40);
    Sha512tDigest.UInt64_To_BE(this.H7, output, outOff + 48 /*0x30*/, this.digestLength - 48 /*0x30*/);
  ...
  private void tIvGenerate(int bitLength)
  {
    ...
    if (bitLength > 100)
    {
      this.Update((byte) (bitLength / 100 + 48 /*0x30*/));
      bitLength %= 100;
      this.Update((byte) (bitLength / 10 + 48 /*0x30*/));
      bitLength %= 10;
      this.Update((byte) (bitLength + 48 /*0x30*/));
    }
  ...
  private static void UInt64_To_BE(ulong n, byte[] bs, int off, int max)
  {
    if (max <= 0)
      return;
    Sha512tDigest.UInt32_To_BE((uint) (n >> 32 /*0x20*/), bs, off, max);
    if (max <= 4)
      return;
  ...
  public override IMemoable Copy() => (IMemoable) new Sha512tDigest(this);
