...
internal class JPath
{
  ...
  private PathFilter ParseArrayIndexer(char indexerCloseChar)
  {
    ...
    while (this._currentIndex < this._expression.Length)
    {
      ...
      else if ((int) c != (int) indexerCloseChar)
      {
        switch (c)
        {
          case '*':
      ...
      {
        nullable5 = nullable1;
        int length3 = (nullable5 ?? this._currentIndex) - currentIndex;
        if (indexes != null)
        {
          if (length3 == 0)
            throw new JsonException("Array index expected.");
          int int32 = Convert.ToInt32(this._expression.Substring(currentIndex, length3), (IFormatProvider) CultureInfo.InvariantCulture);
          indexes.Add(int32);
        ...
        if (length3 == 0)
          throw new JsonException("Array index expected.");
        int int32_1 = Convert.ToInt32(this._expression.Substring(currentIndex, length3), (IFormatProvider) CultureInfo.InvariantCulture);
        return (PathFilter) new ArrayIndexFilter()
        {
