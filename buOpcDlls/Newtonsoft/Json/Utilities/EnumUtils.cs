...
internal static class EnumUtils
{
  ...
  private static EnumInfo InitializeValuesAndNames([Nullable(new byte[] {0, 1, 2})] StructMultiKey<Type, NamingStrategy> key)
  {
    ...
    for (int count = 0; count < names.Length; ++count)
    {
      ...
      string str = field.GetCustomAttributes(typeof (EnumMemberAttribute), true).Cast<EnumMemberAttribute>().Select<EnumMemberAttribute, string>((Func<EnumMemberAttribute, string>) ([NullableContext(0)] (a) => a.Value)).SingleOrDefault<string>();
  ...
  public static bool TryToString(
    Type enumType,
    object value,
    [Nullable(2)] NamingStrategy namingStrategy,
    [Nullable(2), NotNullWhen(true)] out string name)
  {
