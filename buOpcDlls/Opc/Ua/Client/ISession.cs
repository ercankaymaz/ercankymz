...
public interface ISession : ISessionClient, ISessionClientMethods, IClientBase, IDisposable
{
  ...
  Opc.Ua.Node ReadNode(NodeId nodeId);

  Opc.Ua.Node ReadNode(NodeId nodeId, NodeClass nodeClass, bool optionalAttributes = true);
  ...
  Task<(IList<Opc.Ua.Node>, IList<ServiceResult>)> ReadNodesAsync(
    IList<NodeId> nodeIds,
    NodeClass nodeClass,
    bool optionalAttributes = false,
    CancellationToken ct = default (CancellationToken));
  ...
  Task<Opc.Ua.Node> ReadNodeAsync(NodeId nodeId, CancellationToken ct = default (CancellationToken));

  Task<Opc.Ua.Node> ReadNodeAsync(
    NodeId nodeId,
    NodeClass nodeClass,
