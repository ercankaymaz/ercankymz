...
public class QueryNextMessage : IServiceMessage
{
  ...
  public IServiceRequest GetRequest() => (IServiceRequest) this.QueryNextRequest;

  public object CreateResponse(IServiceResponse response)
  {
    if (!(response is QueryNextResponse QueryNextResponse))
    {
      QueryNextResponse = new QueryNextResponse();
      QueryNextResponse.ResponseHeader = ((ServiceFault) response).ResponseHeader;
    }
    return (object) new QueryNextResponseMessage(QueryNextResponse);
  }
}
