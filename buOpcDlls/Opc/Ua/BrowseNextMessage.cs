...
public class BrowseNextMessage : IServiceMessage
{
  ...
  public IServiceRequest GetRequest() => (IServiceRequest) this.BrowseNextRequest;

  public object CreateResponse(IServiceResponse response)
  {
    if (!(response is BrowseNextResponse BrowseNextResponse))
    {
      BrowseNextResponse = new BrowseNextResponse();
      BrowseNextResponse.ResponseHeader = ((ServiceFault) response).ResponseHeader;
    }
    return (object) new BrowseNextResponseMessage(BrowseNextResponse);
  }
}
