...
internal class IOThreadTimer
{
	private class TimerManager
	{
		...
		private void ScheduleElapsedTimers(TimerGroup timerGroup, long now)
		{
			...
			while (timerQueue.Count > 0)
			{
				...
				if (num <= minTimer.maxSkew)
				{
					...
					if (minTimer.isAsyncCallback)
					{
						ActionItem.Schedule(minTimer.asyncCallback, minTimer.callbackState);
					}
					else
					{
		...
		{
			if ((!stableTimerGroup.WaitableTimer.dead || !volatileTimerGroup.WaitableTimer.dead) && (stableTimerGroup.TimerQueue.Count > 0 || volatileTimerGroup.TimerQueue.Count > 0))
			{
				ScheduleWait();
			}
		}
	...
	public class WaitableTimer : EventWaitHandle
	{
		...
		public static int WaitAny(WaitableTimer[] waitableTimers)
		{
			while (true)
			{
				long num = waitableTimers[0].dueTime;
				for (int i = 1; i < waitableTimers.Length; i++)
				{
					...
					if (waitableTimers[i].dueTime < num)
					{
						num = waitableTimers[i].dueTime;
					}
					waitableTimers[i].Reset();
				}
				long num2 = (num - DateTime.UtcNow.Ticks) / 10000;
				if (num2 < 0)
				{
					break;
				}
				WaitHandle.WaitAny(waitableTimers, (int)num2);
			}
			return 0;
		}
	}
	...
	public IOThreadTimer(Func<object, Task> asyncCallback, object callbackState, bool isTypicallyCanceledShortlyAfterBeingSet)
		: this(null, asyncCallback, callbackState, isTypicallyCanceledShortlyAfterBeingSet, 100)
	{
	...
	public IOThreadTimer(Action<object> callback, object callbackState, bool isTypicallyCanceledShortlyAfterBeingSet)
		: this(callback, null, callbackState, isTypicallyCanceledShortlyAfterBeingSet, 100)
	{
	}
