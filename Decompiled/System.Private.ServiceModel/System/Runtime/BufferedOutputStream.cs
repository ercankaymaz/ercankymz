...
internal class BufferedOutputStream : Stream
{
	...
	{
		int num = ((_currentChunk.Length <= 1073741823) ? (_currentChunk.Length * 2) : int.MaxValue);
		if (minimumChunkSize > num)
		{
			num = minimumChunkSize;
		}
		byte[] array = _bufferManager.TakeBuffer(num);
		if (_chunkCount == _chunks.Length)
		{
			byte[][] array2 = new byte[_chunks.Length * 2][];
			Array.Copy(_chunks, array2, _chunks.Length);
			_chunks = array2;
		}
		_chunks[_chunkCount++] = array;
		_currentChunk = array;
		_currentChunkSize = 0;
	}
	...
	public byte[] ToArray(out int bufferSize)
	{
		...
		{
			array = _bufferManager.TakeBuffer(_totalSize);
			int num = 0;
			int num2 = _chunkCount - 1;
			...
			Buffer.BlockCopy(_currentChunk, 0, array, num, _currentChunkSize);
	...
	private void WriteCore(byte[] buffer, int offset, int size)
	{
		...
		int num = _totalSize + size;
		if (num > _maxSize)
		{
			throw Fx.Exception.AsError(CreateQuotaExceededException(_maxSizeQuota));
		}
		int num2 = _currentChunk.Length - _currentChunkSize;
		if (size > num2)
		{
			if (num2 > 0)
			{
				if (buffer != null)
				{
					Buffer.BlockCopy(buffer, offset, _currentChunk, _currentChunkSize, num2);
				}
