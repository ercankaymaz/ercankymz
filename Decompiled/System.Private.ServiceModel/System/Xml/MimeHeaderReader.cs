...
internal class MimeHeaderReader
{
	...
	private unsafe bool ProcessBuffer(int maxBuffer, ref int remaining)
	{
		fixed (byte* ptr = buffer)
		{
			...
			string mimeHeaderInvalidCharacter;
			object p;
			int num;
			switch (readState)
			{
			...
			case ReadState.ReadValue:
				...
				while (ptr4 < ptr3)
				{
					if (*ptr4 != 13)
					{
						if (*ptr4 == 10)
						{
							throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new FormatException(System.SR.MimeReaderMalformedHeader));
						}
						ptr4++;
						continue;
					}
			...
			case ReadState.EOF:
				...
				mimeHeaderInvalidCharacter = System.SR.MimeHeaderInvalidCharacter;
				p = (char)(*ptr4);
				num = *ptr4;
	...
	public void Reset(Stream stream)
	{
		...
		{
			throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(System.SR.MimeReaderResetCalledBeforeEOF));
		}
		this.stream = stream;
		readState = ReadState.ReadName;
		maxOffset = 0;
