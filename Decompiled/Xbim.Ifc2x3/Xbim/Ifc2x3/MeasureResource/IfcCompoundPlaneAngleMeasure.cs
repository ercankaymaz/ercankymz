...
public struct IfcCompoundPlaneAngleMeasure : IfcDerivedMeasureValue, IfcValue, IExpressSelectType, IPersist, IExpressValueType, IExpressComplexType, IEquatable<List<long>>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcCompoundPlaneAngleMeasureClause clause)
	{
		...
		try
		{
			...
			case IfcCompoundPlaneAngleMeasureClause.WR1:
				result = -360 <= ItemAt(0) && ItemAt(0) < 360;
				break;
			case IfcCompoundPlaneAngleMeasureClause.WR2:
				result = -60 <= ItemAt(1) && ItemAt(1) < 60;
				break;
			case IfcCompoundPlaneAngleMeasureClause.WR3:
				result = -60 <= ItemAt(2) && ItemAt(2) < 60;
				break;
			case IfcCompoundPlaneAngleMeasureClause.WR4:
				result = (ItemAt(0) >= 0 && ItemAt(1) >= 0 && ItemAt(2) >= 0) || (ItemAt(0) <= 0 && ItemAt(1) <= 0 && ItemAt(2) <= 0);
				break;
			}
		}
		catch (Exception ex)
	...
	public IEnumerable<ValidationResult> Validate()
	{
		if (!ValidateClause(IfcCompoundPlaneAngleMeasureClause.WR1))
		{
			...
			{
				Item = this,
				IssueSource = "IfcCompoundPlaneAngleMeasure.WR1",
		...
		if (!ValidateClause(IfcCompoundPlaneAngleMeasureClause.WR2))
		{
			...
			{
				Item = this,
				IssueSource = "IfcCompoundPlaneAngleMeasure.WR2",
				IssueType = ValidationFlags.EntityWhereClauses
			};
		}
		if (!ValidateClause(IfcCompoundPlaneAngleMeasureClause.WR3))
		{
			...
			{
				Item = this,
				IssueSource = "IfcCompoundPlaneAngleMeasure.WR3",
				IssueType = ValidationFlags.EntityWhereClauses
			};
		}
		if (!ValidateClause(IfcCompoundPlaneAngleMeasureClause.WR4))
		{
			...
			{
				Item = this,
				IssueSource = "IfcCompoundPlaneAngleMeasure.WR4",
				IssueType = ValidationFlags.EntityWhereClauses
			};
		}
	}
