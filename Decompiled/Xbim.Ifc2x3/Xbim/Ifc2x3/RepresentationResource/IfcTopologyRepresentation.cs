...
public class IfcTopologyRepresentation : IfcShapeModel, IIfcTopologyRepresentation, IIfcShapeModel, IIfcRepresentation, IPersistEntity, IPersist, IfcLayeredItem, IIfcLayeredItem, IExpressSelectType, IInstantiableEntity, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcTopologyRepresentation>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcTopologyRepresentationClause clause)
	{
		...
		try
		{
			...
			case IfcTopologyRepresentationClause.WR21:
				result = Functions.SIZEOF(Enumerable.Where(base.Items, (IfcRepresentationItem temp) => !Functions.TYPEOF(temp).Contains("IFC2X3.IFCTOPOLOGICALREPRESENTATIONITEM"))) == 0;
				break;
