...
internal class ColorSorter : IComparer
{
	public int Compare(object firstItem, object secondItem)
	{
		...
		double num = Math.Round(color.GetHue(), 3);
		double num2 = Math.Round(color2.GetHue(), 3);
		if (num > num2)
		{
			return 1;
		}
		...
		double num3 = Math.Round(color.GetSaturation(), 3);
		double num4 = Math.Round(color2.GetSaturation(), 3);
		if (num3 > num4)
		{
			return 1;
		}
		...
		double num5 = Math.Round(color.GetBrightness(), 3);
		double num6 = Math.Round(color2.GetBrightness(), 3);
		if (num5 > num6)
		{
			return 1;
		}
