...
internal class LinkLabelBehaviorInherit : PaletteContentInherit
{
	...
	public LinkLabelBehaviorInherit(IPaletteContent inherit, KryptonLinkBehavior linkBehavior)
	{
	...
	{
		Font font = _inherit.GetContentShortTextFont(state);
		if (font != null)
		{
			font = GetContentFont(state, font);
		}
		return font;
	}
	...
	{
		Font font = _inherit.GetContentShortTextNewFont(state);
		if (font != null)
		{
			font = GetContentFont(state, font);
		}
	...
	private Font GetContentFont(PaletteState state, Font font)
	{
		if (!CommonHelper.IsOverrideState(state))
		{
			...
			case KryptonLinkBehavior.HoverUnderline:
				flag = state == PaletteState.Tracking || state == PaletteState.Pressed;
				break;
			}
			if (flag)
			{
