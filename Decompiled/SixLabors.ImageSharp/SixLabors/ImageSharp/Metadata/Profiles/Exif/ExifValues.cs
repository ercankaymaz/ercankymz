...
internal static class ExifValues
{
	...
	{
		return Create(tag, dataType, numberOfComponents != 1);
	}
	...
	public static ExifValue? Create(ExifTagValue tag, ExifDataType dataType, bool isArray)
	{
		...
		case ExifDataType.Byte:
			if (!isArray)
			{
				return new ExifByte(tag, dataType);
			}
			return new ExifByteArray(tag, dataType);
		case ExifDataType.DoubleFloat:
			if (!isArray)
			{
				return new ExifDouble(tag);
			}
			return new ExifDoubleArray(tag);
		case ExifDataType.SingleFloat:
			if (!isArray)
			{
				return new ExifFloat(tag);
			}
			return new ExifFloatArray(tag);
		...
		case ExifDataType.Long8:
			if (!isArray)
			{
				return new ExifLong8(tag);
			}
			return new ExifLong8Array(tag);
		...
		case ExifDataType.SignedByte:
			if (!isArray)
			{
				return new ExifSignedByte(tag);
			}
			return new ExifSignedByteArray(tag);
		case ExifDataType.SignedLong:
			if (!isArray)
			{
				return new ExifSignedLong(tag);
			}
			return new ExifSignedLongArray(tag);
		case ExifDataType.SignedLong8:
			if (!isArray)
			{
				return new ExifSignedLong8(tag);
			}
			return new ExifSignedLong8Array(tag);
		...
		case ExifDataType.SignedShort:
			if (!isArray)
			{
				return new ExifSignedShort(tag);
			}
			return new ExifSignedShortArray(tag);
	...
	private static object? CreateValue(ExifTagValue tag)
	{
		return tag switch
		{
			...
			ExifTagValue.InteroperabilityIndex => new ExifString(ExifTag.GPSLatitudeRef), 
