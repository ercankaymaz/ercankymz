...
internal static class ExifTags
{
	public static ExifParts GetPart(ExifTag tag)
	{
		...
		if (exifTagValue <= ExifTagValue.SubIFDOffset)
		{
			if (exifTagValue > ExifTagValue.BatteryLevel)
			{
				if (exifTagValue <= ExifTagValue.MDFileUnits)
				{
					if (exifTagValue <= ExifTagValue.ExposureTime)
					{
						if (exifTagValue == ExifTagValue.Copyright)
						{
							goto IL_0536;
						}
						if (exifTagValue == ExifTagValue.ExposureTime)
					...
					{
						if (exifTagValue == ExifTagValue.FNumber)
						{
							goto IL_0538;
						}
						if ((uint)(exifTagValue - 33445) <= 1u || (uint)(exifTagValue - 33448) <= 4u)
						{
							goto IL_0536;
						}
					}
				}
				else if (exifTagValue <= ExifTagValue.ModelTiePoint)
				{
					if (exifTagValue == ExifTagValue.PixelScale || (uint)(exifTagValue - 33918) <= 2u || exifTagValue == ExifTagValue.ModelTiePoint)
					{
						goto IL_0536;
					}
				}
				...
				{
					if (exifTagValue == ExifTagValue.SEMInfo || exifTagValue == ExifTagValue.ModelTransform)
					{
						goto IL_0536;
					}
					if (exifTagValue == ExifTagValue.SubIFDOffset)
			...
			case ExifTagValue.InteroperabilityIndex:
