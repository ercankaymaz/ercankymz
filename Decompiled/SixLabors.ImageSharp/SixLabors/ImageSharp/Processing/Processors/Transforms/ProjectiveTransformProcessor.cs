...
{
	private readonly struct NNProjectiveOperation(Buffer2D<TPixel> source, Rectangle bounds, Buffer2D<TPixel> destination, Matrix4x4 matrix) : IRowOperation
	{
		...
		public void Invoke(int y)
		{
			...
			{
				Vector2 val = TransformUtils.ProjectiveTransform2D(i, y, matrix);
	...
	private readonly struct ProjectiveOperation<TResampler>(Configuration configuration, Buffer2D<TPixel> source, Rectangle bounds, Buffer2D<TPixel> destination, in TResampler sampler, Matrix4x4 matrix) : IRowIntervalOperation<Vector4> where TResampler : struct, IResampler
	{
		...
		private readonly float yRadius = LinearTransformUtility.GetSamplingRadius(in sampler, bounds.Height, destination.Height);

		private readonly float xRadius = LinearTransformUtility.GetSamplingRadius(in sampler, bounds.Width, destination.Width);
		...
		public void Invoke(in RowInterval rows, Span<Vector4> span)
		{
			...
			for (int i = rows.Min; i < rows.Max; i++)
			{
				...
				{
					Vector2 val3 = TransformUtils.ProjectiveTransform2D(j, i, val);
					float y2 = val3.Y;
					float x2 = val3.X;
	...
	public void ApplyTransform<TResampler>(in TResampler sampler) where TResampler : struct, IResampler
	{
		...
		Matrix4x4 val = transformMatrix;
		if (((Matrix4x4)(ref val)).Equals(Matrix4x4.Identity))
		{
		...
		{
			Matrix4x4.Invert(val, ref val);
			if (sampler is NearestNeighborResampler)
			{
				NNProjectiveOperation operation = new NNProjectiveOperation(imageFrame.PixelBuffer, Rectangle.Intersect(base.SourceRectangle, imageFrame.Bounds()), imageFrame2.PixelBuffer, val);
				ParallelRowIterator.IterateRows(configuration, imageFrame2.Bounds(), in operation);
			}
			else
			{
				ProjectiveOperation<TResampler> operation2 = new ProjectiveOperation<TResampler>(configuration, imageFrame.PixelBuffer, Rectangle.Intersect(base.SourceRectangle, imageFrame.Bounds()), imageFrame2.PixelBuffer, in sampler, val);
