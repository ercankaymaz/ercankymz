...
internal class BlackIsZero4TiffColor<TPixel> : TiffBaseColorDecoder<TPixel> where TPixel : unmanaged, IPixel<TPixel>
{
	public override void Decode(ReadOnlySpan<byte> data, Buffer2D<TPixel> pixels, int left, int top, int width, int height)
	{
		...
		for (int i = top; i < top + height; i++)
		{
			...
			{
				byte num3 = data[num++];
				byte packedValue = (byte)(((num3 & 0xF0) >> 4) * 17);
				source.PackedValue = packedValue;
				val.FromL8(source);
				span[num2++] = val;
				byte packedValue2 = (byte)((num3 & 0xF) * 17);
			...
			if (flag)
			{
				byte packedValue3 = (byte)(((data[num++] & 0xF0) >> 4) * 17);
