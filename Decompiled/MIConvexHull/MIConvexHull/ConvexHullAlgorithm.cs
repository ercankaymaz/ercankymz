...
internal class ConvexHullAlgorithm
{
	...
	internal static ConvexHull<TVertex, TFace> GetConvexHull<TVertex, TFace>(IList<TVertex> data, double PlaneDistanceTolerance) where TVertex : IVertex where TFace : ConvexFace<TVertex, TFace>, new()
	{
		...
		{
			return convexHullAlgorithm.Return2DResultInOrder<TVertex, TFace>();
		}
		...
		if (list.Count <= NumOfDimensions && IsLifted)
		{
			...
			while (num3 < NumOfDimensions && list4.Any())
			{
				...
				for (int num14 = list4.Count - 1; num14 >= 0; num14--)
				{
					...
					{
						mathHelper.RandomOffsetToLift(num15, maxima.Last() - minima.Last());
	...
	private ConvexHull<TVertex, TFace> Return2DResultInOrder<TVertex, TFace>() where TVertex : IVertex where TFace : ConvexFace<TVertex, TFace>, new()
	{
		...
		List<TVertex> list = new List<TVertex> { val2 };
		List<TFace> list2 = new List<TFace> { convexFaces[0] };
		int num2 = 0;
		int num3 = 0;
	...
	{
		ConvexHullAlgorithm convexHullAlgorithm = new ConvexHullAlgorithm(data.Cast<IVertex>().ToArray(), lift: true, PlaneDistanceTolerance);
		convexHullAlgorithm.GetConvexHull();
