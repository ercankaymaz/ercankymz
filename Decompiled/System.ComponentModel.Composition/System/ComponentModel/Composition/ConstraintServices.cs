...
internal static class ConstraintServices
{
	...
	public static Expression<Func<ExportDefinition, bool>> CreateConstraint(string contractName, string? requiredTypeIdentity, IEnumerable<KeyValuePair<string, Type>> requiredMetadata, CreationPolicy requiredCreationPolicy)
	{
		...
		{
			Expression right = CreateTypeIdentityConstraint(requiredTypeIdentity, parameterExpression);
		...
		{
			Expression right2 = CreateCreationPolicyConstraint(requiredCreationPolicy, parameterExpression);
	...
	{
		ArgumentNullException.ThrowIfNull(requiredMetadata, "requiredMetadata");
		ArgumentNullException.ThrowIfNull(parameter, "parameter");
	...
	{
		ArgumentNullException.ThrowIfNull(requiredTypeIdentity, "requiredTypeIdentity");
		ArgumentNullException.ThrowIfNull(parameter, "parameter");
	...
	private static MethodCallExpression CreateMetadataContainsKeyExpression(ParameterExpression parameter, string constantKey)
	{
		...
		if (constantKey == null)
		{
			throw new ArgumentException("constantKey");
		}
	...
	private static MethodCallExpression CreateMetadataOfTypeExpression(ParameterExpression parameter, string constantKey, Type constantType)
	{
		...
		{
			throw new ArgumentException("constantType");
		}
	...
	private static MethodCallExpression CreateMetadataValueEqualsExpression(ParameterExpression parameter, object constantValue, string metadataName)
	{
		...
		{
			throw new ArgumentException("constantValue");
		}
