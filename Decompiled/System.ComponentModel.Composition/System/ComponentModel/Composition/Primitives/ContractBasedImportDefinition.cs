...
public class ContractBasedImportDefinition : ImportDefinition
{
	...
	public override Expression<Func<ExportDefinition, bool>> Constraint => _constraint ?? (_constraint = ConstraintServices.CreateConstraint(ContractName, RequiredTypeIdentity, RequiredMetadata, RequiredCreationPolicy));
	...
	private void ValidateRequiredMetadata()
	{
		...
		foreach (KeyValuePair<string, Type> requiredMetadatum in _requiredMetadata)
		{
			...
			{
				throw new InvalidOperationException(System.SR.Format(System.SR.Argument_NullElement, "requiredMetadata"));
			}
		}
	...
	private bool MatchRequiredMetadata(ExportDefinition definition)
	{
		...
		foreach (KeyValuePair<string, Type> requiredMetadatum in RequiredMetadata)
		{
			...
			if (!definition.Metadata.TryGetValue(key, out object value3))
			{
				return false;
			}
			if (value3 != null)
	...
	public override string ToString()
	{
		...
		if (_requiredMetadata.Any())
		{
			...
			foreach (KeyValuePair<string, Type> requiredMetadatum in _requiredMetadata)
			{
				stringBuilder.Append("\n\t\t").Append(requiredMetadatum.Key).Append("\t(")
					.Append(requiredMetadatum.Value)
					.Append(')');
			}
		}
