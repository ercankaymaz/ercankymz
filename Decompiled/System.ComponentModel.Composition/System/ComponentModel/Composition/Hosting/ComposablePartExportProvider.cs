...
public class ComposablePartExportProvider : ExportProvider, IDisposable
{
	...
	public void Compose(CompositionBatch batch)
	{
		...
		using (AtomicComposition atomicComposition = new AtomicComposition())
		{
			...
			{
				throw new InvalidOperationException(System.SR.ReentrantCompose);
			}
	...
	{
		ArgumentNullException.ThrowIfNull(batch, "batch");
		List<ComposablePart> list = null;
		using (_lock.LockStateForRead())
		{
			list = _parts.ToList();
	...
	private void Recompose(CompositionBatch batch, AtomicComposition atomicComposition)
	{
		...
		IEnumerable<ExportDefinition> addedExports = ((batch.PartsToAdd.Count != 0) ? batch.PartsToAdd.SelectMany((ComposablePart part) => part.ExportDefinitions).ToArray() : Array.Empty<ExportDefinition>());
		IEnumerable<ExportDefinition> removedExports = ((batch.PartsToRemove.Count != 0) ? batch.PartsToRemove.SelectMany((ComposablePart part) => part.ExportDefinitions).ToArray() : Array.Empty<ExportDefinition>());
