...
public class FilteredCatalog : ComposablePartCatalog, INotifyComposablePartCatalogChanged
{
	internal sealed class DependenciesTraversal : IComposablePartCatalogTraversal
	{
		...
		public DependenciesTraversal(FilteredCatalog catalog, Func<ImportDefinition, bool> importFilter)
		{
			...
			ArgumentNullException.ThrowIfNull(importFilter, "importFilter");
		...
		{
			if (!_exportersIndex.TryGetValue(contractName, out var value))
			{
	...
	internal sealed class DependentsTraversal : IComposablePartCatalogTraversal
	{
		...
		{
			if (!_importersIndex.TryGetValue(contractName, out var value))
			{
				value = new List<ComposablePartDefinition>();
	...
	private ComposablePartCatalogChangeEventArgs ProcessEventArgs(ComposablePartCatalogChangeEventArgs e)
	{
		...
		if (e2.AddedDefinitions.Any() || e2.RemovedDefinitions.Any())
		{
			return e2;
		}
		return null;
	...
	{
		ArgumentNullException.ThrowIfNull(traversal, "traversal");
		FreezeInnerCatalog();
		try
		{
	...
	private static void GetTraversalClosure(IEnumerable<ComposablePartDefinition> parts, HashSet<ComposablePartDefinition> traversedParts, IComposablePartCatalogTraversal traversal)
	{
		...
		{
			if (traversedParts.Add(part) && traversal.TryTraverse(part, out IEnumerable<ComposablePartDefinition> reachableParts))
			{
