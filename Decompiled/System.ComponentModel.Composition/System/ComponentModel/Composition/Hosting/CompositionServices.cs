...
internal static class CompositionServices
{
	...
	{
		ArgumentNullException.ThrowIfNull(specifiedContractType, "specifiedContractType");
	...
	internal static string GetContractNameFromImport(this IAttributedImport import, ImportType importType)
	{
		...
		return AttributedModelServices.GetContractName(contractTypeFromImport);
	}
	...
	{
		Dictionary<string, object> dictionary = new Dictionary<string, object>(StringComparers.MetadataKeyNames);
		...
		{
			if (!reservedMetadataNames.Contains<string>(partMetadataAttribute.Name, StringComparers.MetadataKeyNames) && !dictionary.ContainsKey(partMetadataAttribute.Name))
			{
	...
	internal static void TryExportMetadataForMember(this MemberInfo member, out IDictionary<string, object?> dictionary)
	{
		...
		foreach (Attribute attribute in attributes)
		{
			...
			{
				if (reservedMetadataNames.Contains<string>(exportMetadataAttribute.Name, StringComparers.MetadataKeyNames))
				{
					throw ExceptionBuilder.CreateDiscoveryException(System.SR.Discovery_ReservedMetadataNameUsed, member.GetDisplayName(), exportMetadataAttribute.Name);
				}
				...
				{
					throw ExceptionBuilder.CreateDiscoveryException(System.SR.Discovery_DuplicateMetadataNameValues, member.GetDisplayName(), exportMetadataAttribute.Name);
				}
				continue;
			}
			...
			foreach (PropertyInfo propertyInfo in properties)
			{
				...
				{
					if (reservedMetadataNames.Contains<string>(propertyInfo.Name, StringComparers.MetadataKeyNames))
					{
						throw ExceptionBuilder.CreateDiscoveryException(System.SR.Discovery_ReservedMetadataNameUsed, propertyInfo.GetDisplayName(), propertyInfo.Name);
					}
					object value = propertyInfo.GetValue(attribute, null);
					if (value != null && !IsValidAttributeType(value.GetType()))
					{
						throw ExceptionBuilder.CreateDiscoveryException(System.SR.Discovery_MetadataContainsValueWithInvalidType, propertyInfo.GetDisplayName(), value.GetType().GetDisplayName());
					}
					if (!dictionary.TryContributeMetadataValue(propertyInfo.Name, value, propertyInfo.PropertyType, allowsMultiple))
					{
						throw ExceptionBuilder.CreateDiscoveryException(System.SR.Discovery_DuplicateMetadataNameValues, member.GetDisplayName(), propertyInfo.Name);
					}
				}
			}
		}
	...
	internal static IEnumerable<KeyValuePair<string, Type>> GetRequiredMetadata(Type? metadataViewType)
	{
		...
		List<PropertyInfo> source = (from property in metadataViewType.GetAllProperties()
			where property.GetFirstAttribute<DefaultValueAttribute>() == null
			select property).ToList();
		return source.Select((PropertyInfo property) => new KeyValuePair<string, Type>(property.Name, property.PropertyType));
	}
