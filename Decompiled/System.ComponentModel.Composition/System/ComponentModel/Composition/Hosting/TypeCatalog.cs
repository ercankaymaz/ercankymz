...
public class TypeCatalog : ComposablePartCatalog, ICompositionElement
{
			...
			if (_parts == null)
			{
				lock (_thisLock)
				{
					if (_parts == null)
					{
						...
						{
							ComposablePartDefinition composablePartDefinition = AttributedModelDiscovery.CreatePartDefinitionIfDiscoverable(type, _definitionOrigin);
	...
	public TypeCatalog(IEnumerable<Type> types)
	{
		...
		_contractPartIndex = new Lazy<Dictionary<string, List<ComposablePartDefinition>>>(CreateIndex, isThreadSafe: true);
	}
	...
	private void InitializeTypeCatalog(IEnumerable<Type> types, ReflectionContext reflectionContext)
	{
		...
		foreach (Type type in types)
		{
			...
			{
				throw new ArgumentException(System.SR.Format(System.SR.Argument_ElementReflectionOnlyType, "types"), "types");
			}
	...
	private List<ComposablePartDefinition> GetCandidateParts(string contractName)
	{
		...
		_contractPartIndex.Value.TryGetValue(contractName, out var value);
		return value;
	}
	...
	{
		return System.SR.Format(System.SR.TypeCatalog_DisplayNameFormat, GetType().Name, GetTypesDisplay());
	}
	...
	private string GetTypesDisplay()
	{
		...
		if (num == 0)
		{
			return System.SR.TypeCatalog_Empty;
		}
