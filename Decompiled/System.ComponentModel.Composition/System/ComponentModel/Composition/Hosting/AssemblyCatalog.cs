...
public class AssemblyCatalog : ComposablePartCatalog, ICompositionElement
{
	...
	[MemberNotNull("_assembly")]
	private void InitializeAssemblyCatalog(Assembly assembly)
	{
		if (assembly.ReflectionOnly)
		{
			throw new ArgumentException(System.SR.Format(System.SR.Argument_AssemblyReflectionOnly, "assembly"), "assembly");
		}
		_assembly = assembly;
	}
	...
	protected override void Dispose(bool disposing)
	{
		try
		{
			if (Interlocked.CompareExchange(ref _isDisposed, 1, 0) == 0 && disposing)
			{
	...
	{
		return GetType().Name + " (Assembly=\"" + Assembly.FullName + "\")";
	}

	[UnconditionalSuppressMessage("SingleFile", "IL3000: Avoid accessing Assembly file path when publishing as a single file", Justification = "Setting a CodeBase is single file compatible")]
	private static Assembly LoadAssembly(string codeBase)
	{
		...
		{
			return System.Reflection.Assembly.LoadFrom(codeBase);
		}
	}
}
