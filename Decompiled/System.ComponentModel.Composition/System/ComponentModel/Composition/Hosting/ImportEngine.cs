...
public class ImportEngine : ICompositionService, IDisposable
{
	...
	private sealed class PartManager
	{
		...
		public IEnumerable<string> GetImportedContractNames()
		{
			...
			return _importedContractNames ?? (_importedContractNames = Part.ImportDefinitions.Select((ImportDefinition import) => import.ContractName ?? ImportDefinition.EmptyContractName).Distinct().ToArray());
		}
		...
		private void RemoveIndexEntries(PartManager partManager)
		{
			foreach (string importedContractName in partManager.GetImportedContractNames())
			{
				if (_partManagerIndex.TryGetValue(importedContractName, out var value))
				{
					...
					List<PartManager> list = value.AliveItemsToList();
					if (list.Count == 0)
					{
	...
	private void StopSatisfyingImports(PartManager partManager, AtomicComposition atomicComposition)
	{
		if (atomicComposition == null)
		{
			ConditionalWeakTable<ComposablePart, PartManager> conditionalWeakTable = null;
			RecompositionManager recompositionManager = null;
			using (_lock.LockStateForRead())
			{
				conditionalWeakTable = _partManagers;
				recompositionManager = _recompositionManager;
			}
			if (conditionalWeakTable != null)
			{
				conditionalWeakTable.Remove(partManager.Part);
	...
	{
		ArgumentNullException.ThrowIfNull(atomicComposition, "atomicComposition");
	...
	private static CompositionResult<IEnumerable<Export>> TryGetExports(ExportProvider provider, ComposablePart part, ImportDefinition definition, AtomicComposition atomicComposition)
	{
		try
		{
			IEnumerable<Export> value = null;
			if (provider != null)
			{
				value = provider.GetExports(definition, atomicComposition).AsArray();
			}
			return new CompositionResult<IEnumerable<Export>>(value);
		}
