...
internal sealed class GenericSpecializationPartCreationInfo : IReflectionPartCreationInfo, ICompositionElement
{
	...
	public bool IsIdentityComparison => false;
	...
	public GenericSpecializationPartCreationInfo(IReflectionPartCreationInfo originalPartCreationInfo, ReflectionComposablePartDefinition originalPart, Type[] specialization)
	{
		...
		ArgumentNullException.ThrowIfNull(originalPartCreationInfo, "originalPartCreationInfo");
		ArgumentNullException.ThrowIfNull(originalPart, "originalPart");
		ArgumentNullException.ThrowIfNull(specialization, "specialization");
	...
	private MemberInfo[] GetAccessors(LazyMemberInfo originalLazyMember)
	{
		...
		{
			throw new ArgumentNullException("_membersTable");
		}
	...
	private ParameterInfo GetParameter(Lazy<ParameterInfo> originalParameter)
	{
		...
		{
			throw new ArgumentNullException("_parametersTable");
		}
	...
	private Dictionary<LazyMemberInfo, MemberInfo[]> BuildMembersTable(List<LazyMemberInfo> members)
	{
		...
		Type[] interfaces = partType.GetInterfaces();
		foreach (Type type in interfaces)
		{
			dictionary2[type.MetadataToken] = type;
		}
		Type[] nestedTypes = partType.GetNestedTypes();
		foreach (Type type2 in nestedTypes)
		{
			dictionary2[type2.MetadataToken] = type2;
		}
		Type baseType = partType.BaseType;
		while (baseType != null && baseType != typeof(object))
		{
			dictionary2[baseType.MetadataToken] = baseType;
		...
		foreach (LazyMemberInfo member in members)
		{
			...
			for (int k = 0; k < accessors.Length; k++)
			{
				if (accessors[k] != null)
				{
					dictionary2.TryGetValue(accessors[k].MetadataToken, out array[k]);
					if (array[k] == null)
					{
						throw new Exception(System.SR.Diagnostic_InternalExceptionMessage);
					}
	...
	private ImportDefinition TranslateImport(ReflectionImportDefinition reflectionImport, List<LazyMemberInfo> members, List<Lazy<ParameterInfo>> parameters)
	{
		...
		if (reflectionImport is ReflectionMemberImportDefinition reflectionMemberImportDefinition)
		{
		...
		{
			if (!(reflectionImport is ReflectionParameterImportDefinition reflectionParameterImportDefinition))
			{
				throw new Exception(System.SR.Diagnostic_InternalExceptionMessage);
			}
	...
	public ExportDefinition TranslateExpot(ReflectionMemberExportDefinition reflectionExport, List<LazyMemberInfo> members)
	{
		...
		exportDefinition = new ReflectionMemberExportDefinition(member, new LazyExportDefinition(contractName, metadata), ((ICompositionElement)reflectionExport).Origin);
	...
	{
		if (obj is GenericSpecializationPartCreationInfo genericSpecializationPartCreationInfo && _originalPartCreationInfo.Equals(genericSpecializationPartCreationInfo._originalPartCreationInfo))
		{
