...
internal sealed class ReflectionPartCreationInfo : IReflectionPartCreationInfo, ICompositionElement
{
	...
	{
		ArgumentNullException.ThrowIfNull(partType, "partType");
		_partType = partType;
	...
	public IEnumerable<ExportDefinition> GetExports()
	{
		...
		foreach (ExportDefinition item in value)
		{
			...
			{
				throw new InvalidOperationException(System.SR.Format(System.SR.ReflectionModel_InvalidExportDefinition, item.GetType()));
			}
	...
	public IEnumerable<ImportDefinition> GetImports()
	{
		...
		foreach (ImportDefinition item in value)
		{
			...
			{
				throw new InvalidOperationException(System.SR.Format(System.SR.ReflectionModel_InvalidMemberImportDefinition, item.GetType()));
			}
