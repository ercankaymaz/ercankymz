...
internal class CompactFontFormatIsoAdobeCharset : ICompactFontFormatCharset
{
	...
	{
		Dictionary<int, KeyValuePair<int, string>> dictionary = new Dictionary<int, KeyValuePair<int, string>>();
		int num = 0;
		foreach (KeyValuePair<int, string> item in StringIdToName)
		{
			dictionary[num++] = item;
		}
	...
	{
		return glyphIdToStringIdAndName.Single<KeyValuePair<int, KeyValuePair<int, string>>>((KeyValuePair<int, KeyValuePair<int, string>> x) => x.Value.Key == stringId).Value.Value;
	}
	...
	{
		foreach (KeyValuePair<int, KeyValuePair<int, string>> item in glyphIdToStringIdAndName)
		{
			if (string.Equals(item.Value.Value, characterName, StringComparison.Ordinal))
			{
				return item.Key;
			}
		}
