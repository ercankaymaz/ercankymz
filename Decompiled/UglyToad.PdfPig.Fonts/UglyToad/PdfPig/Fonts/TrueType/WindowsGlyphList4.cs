...
{
	public static readonly int NumberOfMacGlyphs;

	public static readonly IReadOnlyList<string> MacGlyphNames;

	public static readonly IReadOnlyDictionary<string, int> MacGlyphNamesIndices;

	static WindowsGlyphList4()
	{
		NumberOfMacGlyphs = 258;
		MacGlyphNames = new string[258]
		{
		...
		Dictionary<string, int> dictionary = new Dictionary<string, int>(NumberOfMacGlyphs);
		for (int i = 0; i < NumberOfMacGlyphs; i++)
		{
			dictionary[MacGlyphNames[i]] = i;
		}
		MacGlyphNamesIndices = dictionary;
	}
}
