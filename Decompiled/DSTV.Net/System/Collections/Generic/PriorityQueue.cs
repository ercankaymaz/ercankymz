...
internal class PriorityQueue<TElement, TPriority>
{
	...
	public sealed class UnorderedItemsCollection : IReadOnlyCollection<(TElement Element, TPriority Priority)>, IEnumerable<(TElement Element, TPriority Priority)>, IEnumerable, ICollection
	{
		...
		IEnumerator<(TElement Element, TPriority Priority)> IEnumerable<(TElement, TPriority)>.GetEnumerator()
		{
			...
			return _003CT_System_Collections_Generic_PriorityQueue_2_g_003EF68671A7C2ECA139CE8C749EB3E572A5EA92BDFFBFD6E4420DC99177051680054__EnumerableHelpers.GetEmptyEnumerator<(TElement, TPriority)>();
		}
	...
	public PriorityQueue(int initialCapacity, IComparer<TPriority>? comparer)
	{
		...
		{
			throw new ArgumentOutOfRangeException("initialCapacity", string.Format("{0} ('{1}') must be a non-negative value.", "initialCapacity", initialCapacity));
		}
	...
	public PriorityQueue(IEnumerable<(TElement Element, TPriority Priority)> items, IComparer<TPriority>? comparer)
	{
		...
		_nodes = _003CT_System_Collections_Generic_PriorityQueue_2_g_003EF68671A7C2ECA139CE8C749EB3E572A5EA92BDFFBFD6E4420DC99177051680054__EnumerableHelpers.ToArray(items, out _size);
		_comparer = InitializeComparer(comparer);
		if (_size > 1)
		{
			Heapify();
	...
	public int EnsureCapacity(int capacity)
	{
		if (capacity < 0)
		{
			throw new ArgumentOutOfRangeException("capacity", string.Format("{0} ('{1}') must be a non-negative value.", "capacity", capacity));
		}
