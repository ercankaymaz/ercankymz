...
{
	private static readonly Lazy<PseudoClassSelectorFactory> Lazy = new Lazy<PseudoClassSelectorFactory>(delegate
	{
		PseudoClassSelectorFactory result = new PseudoClassSelectorFactory();
	...
	private static readonly Dictionary<string, ISelector> Selectors = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
	{
	...
	}.ToDictionary((string x) => x, PseudoClassSelector.Create);

	internal static PseudoClassSelectorFactory Instance => Lazy.Value;

	public ISelector Create(string name)
	{
		if (!Selectors.TryGetValue(name, out var value))
		{
			return null;
		}
		return value;
