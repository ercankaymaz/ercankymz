...
internal static class Pool
{
	...
	public static SelectorConstructor NewSelectorConstructor(AttributeSelectorFactory attributeSelector, PseudoClassSelectorFactory pseudoClassSelector, PseudoElementSelectorFactory pseudoElementSelector)
	{
		lock (Lock)
		{
			return (Selector.Count == 0) ? new SelectorConstructor(attributeSelector, pseudoClassSelector, pseudoElementSelector) : Selector.Pop().Reset(attributeSelector, pseudoClassSelector, pseudoElementSelector);
		}
	}
	...
	public static ValueBuilder NewValueBuilder()
	{
		lock (Lock)
		{
			return (Value.Count == 0) ? new ValueBuilder() : Value.Pop().Reset();
		}
	}
	...
	{
		TokenValue result = vb.GetResult();
		lock (Lock)
		{
			Value.Push(vb);
			return result;
