...
public class IfcDistributionChamberElementType : IfcDistributionFlowElementType, IInstantiableEntity, IPersistEntity, IPersist, IIfcDistributionChamberElementType, IIfcDistributionFlowElementType, IIfcDistributionElementType, IIfcElementType, IIfcTypeProduct, IIfcTypeObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcProductSelect, IIfcProductSelect, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcDistributionChamberElementType>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcDistributionChamberElementTypeClause clause)
	{
		...
		try
		{
			if (clause == IfcDistributionChamberElementTypeClause.CorrectPredefinedType)
			{
				result = PredefinedType != IfcDistributionChamberElementTypeEnum.USERDEFINED || (PredefinedType == IfcDistributionChamberElementTypeEnum.USERDEFINED && Functions.EXISTS(base.ElementType));
			}
		}
		catch (Exception ex)
		{
			ValidationLogging.CreateLogger<IfcDistributionChamberElementType>()?.LogError($"Exception thrown evaluating where-clause 'IfcDistributionChamberElementType.{clause}' for #{base.EntityLabel}.", ex);
		}
		return result;
	}
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcDistributionChamberElementTypeClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcDistributionChamberElementType.CorrectPredefinedType",
