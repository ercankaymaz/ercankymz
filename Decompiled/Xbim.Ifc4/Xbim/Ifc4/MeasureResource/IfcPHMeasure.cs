...
public struct IfcPHMeasure : IfcDerivedMeasureValue, IfcValue, IfcAppliedValueSelect, IIfcAppliedValueSelect, IExpressSelectType, IPersist, IfcMetricValueSelect, IIfcMetricValueSelect, IIfcValue, IExpressValueType, IIfcDerivedMeasureValue, IExpressRealType, IEquatable<double>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcPHMeasureClause clause)
	{
		...
		try
		{
			if (clause == IfcPHMeasureClause.WR21)
			{
				result = 0.0 <= (double)this && (double)this <= 14.0;
			}
		}
		catch (Exception ex)
		{
			ValidationLogging.CreateLogger<IfcPHMeasure>()?.LogError($"Exception thrown evaluating where-clause 'IfcPHMeasure.{clause}'.", ex);
		}
		return result;
	}
