...
public class IfcElementAssemblyType : IfcElementType, IInstantiableEntity, IPersistEntity, IPersist, IIfcElementAssemblyType, IIfcElementType, IIfcTypeProduct, IIfcTypeObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcProductSelect, IIfcProductSelect, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcElementAssemblyType>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcElementAssemblyTypeClause clause)
	{
		...
		try
		{
			if (clause == IfcElementAssemblyTypeClause.CorrectPredefinedType)
			{
				result = PredefinedType != IfcElementAssemblyTypeEnum.USERDEFINED || (PredefinedType == IfcElementAssemblyTypeEnum.USERDEFINED && Functions.EXISTS(base.ElementType));
			}
		}
		catch (Exception ex)
		{
			ValidationLogging.CreateLogger<IfcElementAssemblyType>()?.LogError($"Exception thrown evaluating where-clause 'IfcElementAssemblyType.{clause}' for #{base.EntityLabel}.", ex);
		}
		return result;
	}
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcElementAssemblyTypeClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcElementAssemblyType.CorrectPredefinedType",
