...
public class IfcPileType : IfcBuildingElementType, IInstantiableEntity, IPersistEntity, IPersist, IIfcPileType, IIfcBuildingElementType, IIfcElementType, IIfcTypeProduct, IIfcTypeObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcProductSelect, IIfcProductSelect, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcPileType>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcPileTypeClause clause)
	{
		...
		try
		{
			if (clause == IfcPileTypeClause.CorrectPredefinedType)
			{
				result = PredefinedType != IfcPileTypeEnum.USERDEFINED || (PredefinedType == IfcPileTypeEnum.USERDEFINED && Functions.EXISTS(base.ElementType));
			}
		}
		catch (Exception ex)
		{
			ValidationLogging.CreateLogger<IfcPileType>()?.LogError($"Exception thrown evaluating where-clause 'IfcPileType.{clause}' for #{base.EntityLabel}.", ex);
		}
		return result;
	}
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcPileTypeClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcPileType.CorrectPredefinedType",
