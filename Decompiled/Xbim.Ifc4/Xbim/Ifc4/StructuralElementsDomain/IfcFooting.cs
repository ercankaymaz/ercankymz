...
public class IfcFooting : IfcBuildingElement, IInstantiableEntity, IPersistEntity, IPersist, IIfcFooting, IIfcBuildingElement, IIfcElement, IIfcProduct, IIfcObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcProductSelect, IIfcProductSelect, IfcStructuralActivityAssignmentSelect, IIfcStructuralActivityAssignmentSelect, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcFooting>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcFootingClause clause)
	{
		...
		try
		{
			...
			case IfcFootingClause.CorrectPredefinedType:
				result = !Functions.EXISTS(PredefinedType) || PredefinedType != IfcFootingTypeEnum.USERDEFINED || (PredefinedType == IfcFootingTypeEnum.USERDEFINED && Functions.EXISTS(base.ObjectType));
				break;
			case IfcFootingClause.CorrectTypeAssigned:
				result = Functions.SIZEOF(base.IsTypedBy) == 0 || Functions.TYPEOF(base.IsTypedBy.ItemAt(0L).RelatingType).Contains("IFC4.IFCFOOTINGTYPE");
				break;
			}
		}
		catch (Exception ex)
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcFootingClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcFooting.CorrectPredefinedType",
		...
		if (!ValidateClause(IfcFootingClause.CorrectTypeAssigned))
		{
			...
			{
				Item = this,
				IssueSource = "IfcFooting.CorrectTypeAssigned",
				IssueType = ValidationFlags.EntityWhereClauses
			};
		}
	}
