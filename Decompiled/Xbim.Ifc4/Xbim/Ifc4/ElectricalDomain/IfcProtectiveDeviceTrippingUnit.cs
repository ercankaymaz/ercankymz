...
public class IfcProtectiveDeviceTrippingUnit : IfcDistributionControlElement, IInstantiableEntity, IPersistEntity, IPersist, IIfcProtectiveDeviceTrippingUnit, IIfcDistributionControlElement, IIfcDistributionElement, IIfcElement, IIfcProduct, IIfcObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcProductSelect, IIfcProductSelect, IfcStructuralActivityAssignmentSelect, IIfcStructuralActivityAssignmentSelect, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcProtectiveDeviceTrippingUnit>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcProtectiveDeviceTrippingUnitClause clause)
	{
		...
		try
		{
			...
			case IfcProtectiveDeviceTrippingUnitClause.CorrectPredefinedType:
				result = !Functions.EXISTS(PredefinedType) || PredefinedType != IfcProtectiveDeviceTrippingUnitTypeEnum.USERDEFINED || (PredefinedType == IfcProtectiveDeviceTrippingUnitTypeEnum.USERDEFINED && Functions.EXISTS(base.ObjectType));
				break;
			case IfcProtectiveDeviceTrippingUnitClause.CorrectTypeAssigned:
				result = Functions.SIZEOF(base.IsTypedBy) == 0 || Functions.TYPEOF(base.IsTypedBy.ItemAt(0L).RelatingType).Contains("IFC4.IFCPROTECTIVEDEVICETRIPPINGUNITTYPE");
				break;
			}
		}
		catch (Exception ex)
		{
			ValidationLogging.CreateLogger<IfcProtectiveDeviceTrippingUnit>()?.LogError($"Exception thrown evaluating where-clause 'IfcProtectiveDeviceTrippingUnit.{clause}' for #{base.EntityLabel}.", ex);
		}
		return result;
	}
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcProtectiveDeviceTrippingUnitClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcProtectiveDeviceTrippingUnit.CorrectPredefinedType",
		...
		if (!ValidateClause(IfcProtectiveDeviceTrippingUnitClause.CorrectTypeAssigned))
		{
			...
			{
				Item = this,
				IssueSource = "IfcProtectiveDeviceTrippingUnit.CorrectTypeAssigned",
				IssueType = ValidationFlags.EntityWhereClauses
			};
		}
	}
