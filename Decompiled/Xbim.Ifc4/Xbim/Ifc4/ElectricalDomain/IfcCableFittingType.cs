...
public class IfcCableFittingType : IfcFlowFittingType, IInstantiableEntity, IPersistEntity, IPersist, IIfcCableFittingType, IIfcFlowFittingType, IIfcDistributionFlowElementType, IIfcDistributionElementType, IIfcElementType, IIfcTypeProduct, IIfcTypeObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcProductSelect, IIfcProductSelect, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcCableFittingType>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcCableFittingTypeClause clause)
	{
		...
		try
		{
			if (clause == IfcCableFittingTypeClause.CorrectPredefinedType)
			{
				result = PredefinedType != IfcCableFittingTypeEnum.USERDEFINED || (PredefinedType == IfcCableFittingTypeEnum.USERDEFINED && Functions.EXISTS(base.ElementType));
			}
		}
		catch (Exception ex)
		{
			ValidationLogging.CreateLogger<IfcCableFittingType>()?.LogError($"Exception thrown evaluating where-clause 'IfcCableFittingType.{clause}' for #{base.EntityLabel}.", ex);
		}
		return result;
	}
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcCableFittingTypeClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcCableFittingType.CorrectPredefinedType",
