...
public class IfcSwitchingDevice : IfcFlowController, IInstantiableEntity, IPersistEntity, IPersist, IIfcSwitchingDevice, IIfcFlowController, IIfcDistributionFlowElement, IIfcDistributionElement, IIfcElement, IIfcProduct, IIfcObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcProductSelect, IIfcProductSelect, IfcStructuralActivityAssignmentSelect, IIfcStructuralActivityAssignmentSelect, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcSwitchingDevice>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcSwitchingDeviceClause clause)
	{
		...
		try
		{
			...
			case IfcSwitchingDeviceClause.CorrectPredefinedType:
				result = !Functions.EXISTS(PredefinedType) || PredefinedType != IfcSwitchingDeviceTypeEnum.USERDEFINED || (PredefinedType == IfcSwitchingDeviceTypeEnum.USERDEFINED && Functions.EXISTS(base.ObjectType));
				break;
			case IfcSwitchingDeviceClause.CorrectTypeAssigned:
				result = Functions.SIZEOF(base.IsTypedBy) == 0 || Functions.TYPEOF(base.IsTypedBy.ItemAt(0L).RelatingType).Contains("IFC4.IFCSWITCHINGDEVICETYPE");
				break;
			}
		}
		catch (Exception ex)
		{
			ValidationLogging.CreateLogger<IfcSwitchingDevice>()?.LogError($"Exception thrown evaluating where-clause 'IfcSwitchingDevice.{clause}' for #{base.EntityLabel}.", ex);
		}
		return result;
	}
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcSwitchingDeviceClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcSwitchingDevice.CorrectPredefinedType",
		...
		if (!ValidateClause(IfcSwitchingDeviceClause.CorrectTypeAssigned))
		{
			...
			{
				Item = this,
				IssueSource = "IfcSwitchingDevice.CorrectTypeAssigned",
				IssueType = ValidationFlags.EntityWhereClauses
			};
		}
	}
