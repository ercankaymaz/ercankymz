...
public class IfcArbitraryProfileDefWithVoids : IfcArbitraryClosedProfileDef, IInstantiableEntity, IPersistEntity, IPersist, IIfcArbitraryProfileDefWithVoids, IIfcArbitraryClosedProfileDef, IIfcProfileDef, IfcResourceObjectSelect, IIfcResourceObjectSelect, IExpressSelectType, IContainsEntityReferences, IEquatable<IfcArbitraryProfileDefWithVoids>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcArbitraryProfileDefWithVoidsClause clause)
	{
		...
		try
		{
			...
			case IfcArbitraryProfileDefWithVoidsClause.WR3:
				result = Functions.SIZEOF(Enumerable.Where(InnerCurves, (IfcCurve temp) => Functions.TYPEOF(temp).Contains("IFC4.IFCLINE"))) == 0;
				break;
			}
		}
		catch (Exception ex)
