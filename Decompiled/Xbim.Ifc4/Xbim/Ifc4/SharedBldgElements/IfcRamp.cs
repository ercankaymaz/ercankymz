...
public class IfcRamp : IfcBuildingElement, IInstantiableEntity, IPersistEntity, IPersist, IIfcRamp, IIfcBuildingElement, IIfcElement, IIfcProduct, IIfcObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcProductSelect, IIfcProductSelect, IfcStructuralActivityAssignmentSelect, IIfcStructuralActivityAssignmentSelect, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcRamp>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcRampClause clause)
	{
		...
		try
		{
			...
			case IfcRampClause.CorrectPredefinedType:
				result = !Functions.EXISTS(PredefinedType) || PredefinedType != IfcRampTypeEnum.USERDEFINED || (PredefinedType == IfcRampTypeEnum.USERDEFINED && Functions.EXISTS(base.ObjectType));
				break;
			case IfcRampClause.CorrectTypeAssigned:
				result = Functions.SIZEOF(base.IsTypedBy) == 0 || Functions.TYPEOF(base.IsTypedBy.ItemAt(0L).RelatingType).Contains("IFC4.IFCRAMPTYPE");
				break;
			}
		}
		catch (Exception ex)
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcRampClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcRamp.CorrectPredefinedType",
		...
		if (!ValidateClause(IfcRampClause.CorrectTypeAssigned))
		{
			...
			{
				Item = this,
				IssueSource = "IfcRamp.CorrectTypeAssigned",
				IssueType = ValidationFlags.EntityWhereClauses
			};
		}
	}
