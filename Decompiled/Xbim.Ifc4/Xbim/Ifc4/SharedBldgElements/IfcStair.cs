...
public class IfcStair : IfcBuildingElement, IInstantiableEntity, IPersistEntity, IPersist, IIfcStair, IIfcBuildingElement, IIfcElement, IIfcProduct, IIfcObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcProductSelect, IIfcProductSelect, IfcStructuralActivityAssignmentSelect, IIfcStructuralActivityAssignmentSelect, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcStair>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcStairClause clause)
	{
		...
		try
		{
			...
			case IfcStairClause.CorrectPredefinedType:
				result = !Functions.EXISTS(PredefinedType) || PredefinedType != IfcStairTypeEnum.USERDEFINED || (PredefinedType == IfcStairTypeEnum.USERDEFINED && Functions.EXISTS(base.ObjectType));
				break;
			case IfcStairClause.CorrectTypeAssigned:
				result = Functions.SIZEOF(base.IsTypedBy) == 0 || Functions.TYPEOF(base.IsTypedBy.ItemAt(0L).RelatingType).Contains("IFC4.IFCSTAIRTYPE");
				break;
			}
		}
		catch (Exception ex)
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcStairClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcStair.CorrectPredefinedType",
		...
		if (!ValidateClause(IfcStairClause.CorrectTypeAssigned))
		{
			...
			{
				Item = this,
				IssueSource = "IfcStair.CorrectTypeAssigned",
				IssueType = ValidationFlags.EntityWhereClauses
			};
		}
	}
