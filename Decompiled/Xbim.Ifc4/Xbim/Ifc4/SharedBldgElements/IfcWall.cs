...
public class IfcWall : IfcBuildingElement, IInstantiableEntity, IPersistEntity, IPersist, IIfcWall, IIfcBuildingElement, IIfcElement, IIfcProduct, IIfcObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcProductSelect, IIfcProductSelect, IfcStructuralActivityAssignmentSelect, IIfcStructuralActivityAssignmentSelect, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcWall>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcWallClause clause)
	{
		...
		try
		{
			...
			case IfcWallClause.CorrectPredefinedType:
				result = !Functions.EXISTS(PredefinedType) || PredefinedType != IfcWallTypeEnum.USERDEFINED || (PredefinedType == IfcWallTypeEnum.USERDEFINED && Functions.EXISTS(base.ObjectType));
				break;
			case IfcWallClause.CorrectTypeAssigned:
				result = Functions.SIZEOF(base.IsTypedBy) == 0 || Functions.TYPEOF(base.IsTypedBy.ItemAt(0L).RelatingType).Contains("IFC4.IFCWALLTYPE");
				break;
			}
		}
		catch (Exception ex)
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcWallClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcWall.CorrectPredefinedType",
		...
		if (!ValidateClause(IfcWallClause.CorrectTypeAssigned))
		{
			...
			{
				Item = this,
				IssueSource = "IfcWall.CorrectTypeAssigned",
				IssueType = ValidationFlags.EntityWhereClauses
			};
		}
	}
