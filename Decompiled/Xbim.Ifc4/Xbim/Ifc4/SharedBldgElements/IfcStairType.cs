...
public class IfcStairType : IfcBuildingElementType, IInstantiableEntity, IPersistEntity, IPersist, IIfcStairType, IIfcBuildingElementType, IIfcElementType, IIfcTypeProduct, IIfcTypeObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcProductSelect, IIfcProductSelect, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcStairType>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcStairTypeClause clause)
	{
		...
		try
		{
			if (clause == IfcStairTypeClause.CorrectPredefinedType)
			{
				result = PredefinedType != IfcStairTypeEnum.USERDEFINED || (PredefinedType == IfcStairTypeEnum.USERDEFINED && Functions.EXISTS(base.ElementType));
			}
		}
		catch (Exception ex)
		{
			ValidationLogging.CreateLogger<IfcStairType>()?.LogError($"Exception thrown evaluating where-clause 'IfcStairType.{clause}' for #{base.EntityLabel}.", ex);
		}
		return result;
	}
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcStairTypeClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcStairType.CorrectPredefinedType",
