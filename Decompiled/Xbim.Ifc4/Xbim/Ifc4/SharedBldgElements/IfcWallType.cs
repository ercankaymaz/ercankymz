...
public class IfcWallType : IfcBuildingElementType, IInstantiableEntity, IPersistEntity, IPersist, IIfcWallType, IIfcBuildingElementType, IIfcElementType, IIfcTypeProduct, IIfcTypeObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcProductSelect, IIfcProductSelect, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcWallType>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcWallTypeClause clause)
	{
		...
		try
		{
			if (clause == IfcWallTypeClause.CorrectPredefinedType)
			{
				result = PredefinedType != IfcWallTypeEnum.USERDEFINED || (PredefinedType == IfcWallTypeEnum.USERDEFINED && Functions.EXISTS(base.ElementType));
			}
		}
		catch (Exception ex)
		{
			ValidationLogging.CreateLogger<IfcWallType>()?.LogError($"Exception thrown evaluating where-clause 'IfcWallType.{clause}' for #{base.EntityLabel}.", ex);
		}
		return result;
	}
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcWallTypeClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcWallType.CorrectPredefinedType",
