...
public class IfcShadingDevice : IfcBuildingElement, IInstantiableEntity, IPersistEntity, IPersist, IIfcShadingDevice, IIfcBuildingElement, IIfcElement, IIfcProduct, IIfcObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcProductSelect, IIfcProductSelect, IfcStructuralActivityAssignmentSelect, IIfcStructuralActivityAssignmentSelect, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcShadingDevice>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcShadingDeviceClause clause)
	{
		...
		try
		{
			...
			case IfcShadingDeviceClause.CorrectPredefinedType:
				result = !Functions.EXISTS(PredefinedType) || PredefinedType != IfcShadingDeviceTypeEnum.USERDEFINED || (PredefinedType == IfcShadingDeviceTypeEnum.USERDEFINED && Functions.EXISTS(base.ObjectType));
				break;
			case IfcShadingDeviceClause.CorrectTypeAssigned:
				result = Functions.SIZEOF(base.IsTypedBy) == 0 || Functions.TYPEOF(base.IsTypedBy.ItemAt(0L).RelatingType).Contains("IFC4.IFCSHADINGDEVICETYPE");
				break;
			}
		}
		catch (Exception ex)
		{
			ValidationLogging.CreateLogger<IfcShadingDevice>()?.LogError($"Exception thrown evaluating where-clause 'IfcShadingDevice.{clause}' for #{base.EntityLabel}.", ex);
		}
		return result;
	}
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcShadingDeviceClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcShadingDevice.CorrectPredefinedType",
		...
		if (!ValidateClause(IfcShadingDeviceClause.CorrectTypeAssigned))
		{
			...
			{
				Item = this,
				IssueSource = "IfcShadingDevice.CorrectTypeAssigned",
				IssueType = ValidationFlags.EntityWhereClauses
			};
		}
	}
