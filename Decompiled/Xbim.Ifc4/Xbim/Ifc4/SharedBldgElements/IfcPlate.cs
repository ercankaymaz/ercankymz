...
public class IfcPlate : IfcBuildingElement, IInstantiableEntity, IPersistEntity, IPersist, IIfcPlate, IIfcBuildingElement, IIfcElement, IIfcProduct, IIfcObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcProductSelect, IIfcProductSelect, IfcStructuralActivityAssignmentSelect, IIfcStructuralActivityAssignmentSelect, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcPlate>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcPlateClause clause)
	{
		...
		try
		{
			...
			case IfcPlateClause.CorrectPredefinedType:
				result = !Functions.EXISTS(PredefinedType) || PredefinedType != IfcPlateTypeEnum.USERDEFINED || (PredefinedType == IfcPlateTypeEnum.USERDEFINED && Functions.EXISTS(base.ObjectType));
				break;
			case IfcPlateClause.CorrectTypeAssigned:
				result = Functions.SIZEOF(base.IsTypedBy) == 0 || Functions.TYPEOF(base.IsTypedBy.ItemAt(0L).RelatingType).Contains("IFC4.IFCPLATETYPE");
				break;
			}
		}
		catch (Exception ex)
		{
			ValidationLogging.CreateLogger<IfcPlate>()?.LogError($"Exception thrown evaluating where-clause 'IfcPlate.{clause}' for #{base.EntityLabel}.", ex);
		}
		return result;
	}
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcPlateClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcPlate.CorrectPredefinedType",
		...
		if (!ValidateClause(IfcPlateClause.CorrectTypeAssigned))
		{
			...
			{
				Item = this,
				IssueSource = "IfcPlate.CorrectTypeAssigned",
				IssueType = ValidationFlags.EntityWhereClauses
			};
		}
	}
