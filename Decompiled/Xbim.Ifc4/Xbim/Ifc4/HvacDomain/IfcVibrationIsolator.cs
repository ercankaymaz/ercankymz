...
public class IfcVibrationIsolator : IfcElementComponent, IInstantiableEntity, IPersistEntity, IPersist, IIfcVibrationIsolator, IIfcElementComponent, IIfcElement, IIfcProduct, IIfcObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcProductSelect, IIfcProductSelect, IfcStructuralActivityAssignmentSelect, IIfcStructuralActivityAssignmentSelect, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcVibrationIsolator>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcVibrationIsolatorClause clause)
	{
		...
		try
		{
			...
			case IfcVibrationIsolatorClause.CorrectPredefinedType:
				result = !Functions.EXISTS(PredefinedType) || PredefinedType != IfcVibrationIsolatorTypeEnum.USERDEFINED || (PredefinedType == IfcVibrationIsolatorTypeEnum.USERDEFINED && Functions.EXISTS(base.ObjectType));
				break;
			case IfcVibrationIsolatorClause.CorrectTypeAssigned:
				result = Functions.SIZEOF(base.IsTypedBy) == 0 || Functions.TYPEOF(base.IsTypedBy.ItemAt(0L).RelatingType).Contains("IFC4.IFCVIBRATIONISOLATORTYPE");
				break;
			}
		}
		catch (Exception ex)
		{
			ValidationLogging.CreateLogger<IfcVibrationIsolator>()?.LogError($"Exception thrown evaluating where-clause 'IfcVibrationIsolator.{clause}' for #{base.EntityLabel}.", ex);
		}
		return result;
	}
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcVibrationIsolatorClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcVibrationIsolator.CorrectPredefinedType",
		...
		if (!ValidateClause(IfcVibrationIsolatorClause.CorrectTypeAssigned))
		{
			...
			{
				Item = this,
				IssueSource = "IfcVibrationIsolator.CorrectTypeAssigned",
				IssueType = ValidationFlags.EntityWhereClauses
			};
		}
	}
