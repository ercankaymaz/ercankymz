...
public class IfcPipeFitting : IfcFlowFitting, IInstantiableEntity, IPersistEntity, IPersist, IIfcPipeFitting, IIfcFlowFitting, IIfcDistributionFlowElement, IIfcDistributionElement, IIfcElement, IIfcProduct, IIfcObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcProductSelect, IIfcProductSelect, IfcStructuralActivityAssignmentSelect, IIfcStructuralActivityAssignmentSelect, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcPipeFitting>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcPipeFittingClause clause)
	{
		...
		try
		{
			...
			case IfcPipeFittingClause.CorrectPredefinedType:
				result = !Functions.EXISTS(PredefinedType) || PredefinedType != IfcPipeFittingTypeEnum.USERDEFINED || (PredefinedType == IfcPipeFittingTypeEnum.USERDEFINED && Functions.EXISTS(base.ObjectType));
				break;
			case IfcPipeFittingClause.CorrectTypeAssigned:
				result = Functions.SIZEOF(base.IsTypedBy) == 0 || Functions.TYPEOF(base.IsTypedBy.ItemAt(0L).RelatingType).Contains("IFC4.IFCPIPEFITTINGTYPE");
				break;
			}
		}
		catch (Exception ex)
		{
			ValidationLogging.CreateLogger<IfcPipeFitting>()?.LogError($"Exception thrown evaluating where-clause 'IfcPipeFitting.{clause}' for #{base.EntityLabel}.", ex);
		}
		return result;
	}
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcPipeFittingClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcPipeFitting.CorrectPredefinedType",
		...
		if (!ValidateClause(IfcPipeFittingClause.CorrectTypeAssigned))
		{
			...
			{
				Item = this,
				IssueSource = "IfcPipeFitting.CorrectTypeAssigned",
				IssueType = ValidationFlags.EntityWhereClauses
			};
		}
	}
