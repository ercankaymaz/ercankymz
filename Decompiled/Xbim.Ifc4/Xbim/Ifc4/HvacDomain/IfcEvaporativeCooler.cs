...
public class IfcEvaporativeCooler : IfcEnergyConversionDevice, IInstantiableEntity, IPersistEntity, IPersist, IIfcEvaporativeCooler, IIfcEnergyConversionDevice, IIfcDistributionFlowElement, IIfcDistributionElement, IIfcElement, IIfcProduct, IIfcObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcProductSelect, IIfcProductSelect, IfcStructuralActivityAssignmentSelect, IIfcStructuralActivityAssignmentSelect, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcEvaporativeCooler>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcEvaporativeCoolerClause clause)
	{
		...
		try
		{
			...
			case IfcEvaporativeCoolerClause.CorrectPredefinedType:
				result = !Functions.EXISTS(PredefinedType) || PredefinedType != IfcEvaporativeCoolerTypeEnum.USERDEFINED || (PredefinedType == IfcEvaporativeCoolerTypeEnum.USERDEFINED && Functions.EXISTS(base.ObjectType));
				break;
			case IfcEvaporativeCoolerClause.CorrectTypeAssigned:
				result = Functions.SIZEOF(base.IsTypedBy) == 0 || Functions.TYPEOF(base.IsTypedBy.ItemAt(0L).RelatingType).Contains("IFC4.IFCEVAPORATIVECOOLERTYPE");
				break;
			}
		}
		catch (Exception ex)
		{
			ValidationLogging.CreateLogger<IfcEvaporativeCooler>()?.LogError($"Exception thrown evaluating where-clause 'IfcEvaporativeCooler.{clause}' for #{base.EntityLabel}.", ex);
		}
		return result;
	}
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcEvaporativeCoolerClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcEvaporativeCooler.CorrectPredefinedType",
		...
		if (!ValidateClause(IfcEvaporativeCoolerClause.CorrectTypeAssigned))
		{
			...
			{
				Item = this,
				IssueSource = "IfcEvaporativeCooler.CorrectTypeAssigned",
				IssueType = ValidationFlags.EntityWhereClauses
			};
		}
	}
