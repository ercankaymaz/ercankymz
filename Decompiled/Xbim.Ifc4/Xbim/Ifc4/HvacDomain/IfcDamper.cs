...
public class IfcDamper : IfcFlowController, IInstantiableEntity, IPersistEntity, IPersist, IIfcDamper, IIfcFlowController, IIfcDistributionFlowElement, IIfcDistributionElement, IIfcElement, IIfcProduct, IIfcObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcProductSelect, IIfcProductSelect, IfcStructuralActivityAssignmentSelect, IIfcStructuralActivityAssignmentSelect, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcDamper>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcDamperClause clause)
	{
		...
		try
		{
			...
			case IfcDamperClause.CorrectPredefinedType:
				result = !Functions.EXISTS(PredefinedType) || PredefinedType != IfcDamperTypeEnum.USERDEFINED || (PredefinedType == IfcDamperTypeEnum.USERDEFINED && Functions.EXISTS(base.ObjectType));
				break;
			case IfcDamperClause.CorrectTypeAssigned:
				result = Functions.SIZEOF(base.IsTypedBy) == 0 || Functions.TYPEOF(base.IsTypedBy.ItemAt(0L).RelatingType).Contains("IFC4.IFCDAMPERTYPE");
				break;
			}
		}
		catch (Exception ex)
		{
			ValidationLogging.CreateLogger<IfcDamper>()?.LogError($"Exception thrown evaluating where-clause 'IfcDamper.{clause}' for #{base.EntityLabel}.", ex);
		}
		return result;
	}
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcDamperClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcDamper.CorrectPredefinedType",
		...
		if (!ValidateClause(IfcDamperClause.CorrectTypeAssigned))
		{
			...
			{
				Item = this,
				IssueSource = "IfcDamper.CorrectTypeAssigned",
				IssueType = ValidationFlags.EntityWhereClauses
			};
		}
	}
