...
public class IfcFireSuppressionTerminalType : IfcFlowTerminalType, IInstantiableEntity, IPersistEntity, IPersist, IIfcFireSuppressionTerminalType, IIfcFlowTerminalType, IIfcDistributionFlowElementType, IIfcDistributionElementType, IIfcElementType, IIfcTypeProduct, IIfcTypeObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcProductSelect, IIfcProductSelect, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcFireSuppressionTerminalType>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcFireSuppressionTerminalTypeClause clause)
	{
		...
		try
		{
			if (clause == IfcFireSuppressionTerminalTypeClause.CorrectPredefinedType)
			{
				result = PredefinedType != IfcFireSuppressionTerminalTypeEnum.USERDEFINED || (PredefinedType == IfcFireSuppressionTerminalTypeEnum.USERDEFINED && Functions.EXISTS(base.ElementType));
			}
		}
		catch (Exception ex)
		{
			ValidationLogging.CreateLogger<IfcFireSuppressionTerminalType>()?.LogError($"Exception thrown evaluating where-clause 'IfcFireSuppressionTerminalType.{clause}' for #{base.EntityLabel}.", ex);
		}
		return result;
	}
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcFireSuppressionTerminalTypeClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcFireSuppressionTerminalType.CorrectPredefinedType",
