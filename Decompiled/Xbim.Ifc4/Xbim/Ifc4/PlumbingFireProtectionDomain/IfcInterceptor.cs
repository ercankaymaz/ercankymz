...
public class IfcInterceptor : IfcFlowTreatmentDevice, IInstantiableEntity, IPersistEntity, IPersist, IIfcInterceptor, IIfcFlowTreatmentDevice, IIfcDistributionFlowElement, IIfcDistributionElement, IIfcElement, IIfcProduct, IIfcObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcProductSelect, IIfcProductSelect, IfcStructuralActivityAssignmentSelect, IIfcStructuralActivityAssignmentSelect, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcInterceptor>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcInterceptorClause clause)
	{
		...
		try
		{
			...
			case IfcInterceptorClause.CorrectPredefinedType:
				result = !Functions.EXISTS(PredefinedType) || PredefinedType != IfcInterceptorTypeEnum.USERDEFINED || (PredefinedType == IfcInterceptorTypeEnum.USERDEFINED && Functions.EXISTS(base.ObjectType));
				break;
			case IfcInterceptorClause.CorrectTypeAssigned:
				result = Functions.SIZEOF(base.IsTypedBy) == 0 || Functions.TYPEOF(base.IsTypedBy.ItemAt(0L).RelatingType).Contains("IFC4.IFCINTERCEPTORTYPE");
				break;
			}
		}
		catch (Exception ex)
		{
			ValidationLogging.CreateLogger<IfcInterceptor>()?.LogError($"Exception thrown evaluating where-clause 'IfcInterceptor.{clause}' for #{base.EntityLabel}.", ex);
		}
		return result;
	}
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcInterceptorClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcInterceptor.CorrectPredefinedType",
		...
		if (!ValidateClause(IfcInterceptorClause.CorrectTypeAssigned))
		{
			...
			{
				Item = this,
				IssueSource = "IfcInterceptor.CorrectTypeAssigned",
				IssueType = ValidationFlags.EntityWhereClauses
			};
		}
	}
