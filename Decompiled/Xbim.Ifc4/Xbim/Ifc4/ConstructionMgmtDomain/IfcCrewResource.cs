...
public class IfcCrewResource : IfcConstructionResource, IInstantiableEntity, IPersistEntity, IPersist, IIfcCrewResource, IIfcConstructionResource, IIfcResource, IIfcObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcResourceSelect, IIfcResourceSelect, IContainsEntityReferences, IEquatable<IfcCrewResource>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcCrewResourceClause clause)
	{
		...
		try
		{
			if (clause == IfcCrewResourceClause.CorrectPredefinedType)
			{
				result = !Functions.EXISTS(PredefinedType) || PredefinedType != IfcCrewResourceTypeEnum.USERDEFINED || (PredefinedType == IfcCrewResourceTypeEnum.USERDEFINED && Functions.EXISTS(base.ObjectType));
			}
		}
		catch (Exception ex)
		{
			ValidationLogging.CreateLogger<IfcCrewResource>()?.LogError($"Exception thrown evaluating where-clause 'IfcCrewResource.{clause}' for #{base.EntityLabel}.", ex);
		}
		return result;
	}
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcCrewResourceClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcCrewResource.CorrectPredefinedType",
