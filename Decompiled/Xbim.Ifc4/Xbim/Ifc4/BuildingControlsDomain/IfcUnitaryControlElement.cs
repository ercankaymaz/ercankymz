...
public class IfcUnitaryControlElement : IfcDistributionControlElement, IInstantiableEntity, IPersistEntity, IPersist, IIfcUnitaryControlElement, IIfcDistributionControlElement, IIfcDistributionElement, IIfcElement, IIfcProduct, IIfcObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcProductSelect, IIfcProductSelect, IfcStructuralActivityAssignmentSelect, IIfcStructuralActivityAssignmentSelect, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcUnitaryControlElement>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcUnitaryControlElementClause clause)
	{
		...
		try
		{
			...
			case IfcUnitaryControlElementClause.CorrectPredefinedType:
				result = !Functions.EXISTS(PredefinedType) || PredefinedType != IfcUnitaryControlElementTypeEnum.USERDEFINED || (PredefinedType == IfcUnitaryControlElementTypeEnum.USERDEFINED && Functions.EXISTS(base.ObjectType));
				break;
			case IfcUnitaryControlElementClause.CorrectTypeAssigned:
				result = Functions.SIZEOF(base.IsTypedBy) == 0 || Functions.TYPEOF(base.IsTypedBy.ItemAt(0L).RelatingType).Contains("IFC4.IFCUNITARYCONTROLELEMENTTYPE");
				break;
			}
		}
		catch (Exception ex)
		{
			ValidationLogging.CreateLogger<IfcUnitaryControlElement>()?.LogError($"Exception thrown evaluating where-clause 'IfcUnitaryControlElement.{clause}' for #{base.EntityLabel}.", ex);
		}
		return result;
	}
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcUnitaryControlElementClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcUnitaryControlElement.CorrectPredefinedType",
		...
		if (!ValidateClause(IfcUnitaryControlElementClause.CorrectTypeAssigned))
		{
			...
			{
				Item = this,
				IssueSource = "IfcUnitaryControlElement.CorrectTypeAssigned",
				IssueType = ValidationFlags.EntityWhereClauses
			};
		}
	}
