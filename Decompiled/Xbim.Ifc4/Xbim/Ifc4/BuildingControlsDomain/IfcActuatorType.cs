...
public class IfcActuatorType : IfcDistributionControlElementType, IInstantiableEntity, IPersistEntity, IPersist, IIfcActuatorType, IIfcDistributionControlElementType, IIfcDistributionElementType, IIfcElementType, IIfcTypeProduct, IIfcTypeObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcProductSelect, IIfcProductSelect, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcActuatorType>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcActuatorTypeClause clause)
	{
		...
		try
		{
			if (clause == IfcActuatorTypeClause.CorrectPredefinedType)
			{
				result = PredefinedType != IfcActuatorTypeEnum.USERDEFINED || (PredefinedType == IfcActuatorTypeEnum.USERDEFINED && Functions.EXISTS(base.ElementType));
			}
		}
		catch (Exception ex)
		{
			ValidationLogging.CreateLogger<IfcActuatorType>()?.LogError($"Exception thrown evaluating where-clause 'IfcActuatorType.{clause}' for #{base.EntityLabel}.", ex);
		}
		return result;
	}
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcActuatorTypeClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcActuatorType.CorrectPredefinedType",
