...
public class IfcSensorType : IfcDistributionControlElementType, IInstantiableEntity, IPersistEntity, IPersist, IIfcSensorType, IIfcDistributionControlElementType, IIfcDistributionElementType, IIfcElementType, IIfcTypeProduct, IIfcTypeObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcProductSelect, IIfcProductSelect, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcSensorType>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcSensorTypeClause clause)
	{
		...
		try
		{
			if (clause == IfcSensorTypeClause.CorrectPredefinedType)
			{
				result = PredefinedType != IfcSensorTypeEnum.USERDEFINED || (PredefinedType == IfcSensorTypeEnum.USERDEFINED && Functions.EXISTS(base.ElementType));
			}
		}
		catch (Exception ex)
		{
			ValidationLogging.CreateLogger<IfcSensorType>()?.LogError($"Exception thrown evaluating where-clause 'IfcSensorType.{clause}' for #{base.EntityLabel}.", ex);
		}
		return result;
	}
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcSensorTypeClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcSensorType.CorrectPredefinedType",
