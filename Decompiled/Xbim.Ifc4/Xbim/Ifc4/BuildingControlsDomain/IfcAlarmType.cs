...
public class IfcAlarmType : IfcDistributionControlElementType, IInstantiableEntity, IPersistEntity, IPersist, IIfcAlarmType, IIfcDistributionControlElementType, IIfcDistributionElementType, IIfcElementType, IIfcTypeProduct, IIfcTypeObject, IIfcObjectDefinition, IIfcRoot, IfcDefinitionSelect, IIfcDefinitionSelect, IExpressSelectType, IfcProductSelect, IIfcProductSelect, IContainsEntityReferences, IContainsIndexedReferences, IEquatable<IfcAlarmType>, IExpressValidatable
{
	...
	public bool ValidateClause(IfcAlarmTypeClause clause)
	{
		...
		try
		{
			if (clause == IfcAlarmTypeClause.CorrectPredefinedType)
			{
				result = PredefinedType != IfcAlarmTypeEnum.USERDEFINED || (PredefinedType == IfcAlarmTypeEnum.USERDEFINED && Functions.EXISTS(base.ElementType));
			}
		}
		catch (Exception ex)
		{
			ValidationLogging.CreateLogger<IfcAlarmType>()?.LogError($"Exception thrown evaluating where-clause 'IfcAlarmType.{clause}' for #{base.EntityLabel}.", ex);
		}
		return result;
	}
	...
	public override IEnumerable<ValidationResult> Validate()
	{
		...
		if (!ValidateClause(IfcAlarmTypeClause.CorrectPredefinedType))
		{
			...
			{
				Item = this,
				IssueSource = "IfcAlarmType.CorrectPredefinedType",
