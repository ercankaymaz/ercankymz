...
public class F_NickItems : Form
{
	...
	public void Init()
	{
		...
		{
			checkedListBox_0.Items.Add(i + 1 + "-" + strNick, Nicks[i].Enable);
		}
	...
	internal void method_4(object sender, EventArgs e)
	{
		for (int i = 0; i <= Nicks.Count - 1; i++)
		{
			Nicks[i].Width = (double)numericUpDown_3.Value;
			Nicks[i].Height = (double)numericUpDown_2.Value;
		}
		if (applyCommandEventHandler_0 != null)
		{
			applyCommandEventHandler_0();
	...
	{
		if ((checkedListBox_0.SelectedIndex >= 0) & (checkedListBox_0.SelectedIndex <= Nicks.Count - 1))
		{
			Nicks[checkedListBox_0.SelectedIndex].Width = (double)numericUpDown_3.Value;
			Nicks[checkedListBox_0.SelectedIndex].Height = (double)numericUpDown_2.Value;
			Nicks[checkedListBox_0.SelectedIndex].Offset = (double)numericUpDown_1.Value;
			Nicks[checkedListBox_0.SelectedIndex].ExtractXPosition = (double)numericUpDown_0.Value;
			bool itemChecked = checkedListBox_0.GetItemChecked(checkedListBox_0.SelectedIndex);
			Nicks[checkedListBox_0.SelectedIndex].Enable = itemChecked;
		}
		if (applyCommandEventHandler_0 != null)
		{
			applyCommandEventHandler_0();
	...
	{
		if (((checkedListBox_0.SelectedIndex >= 0) & (checkedListBox_0.SelectedIndex <= Nicks.Count - 1) & bool_0) && buString.MessageBoxQuestion(strRemove) == DialogResult.Yes)
		{
			Nicks.RemoveAt(checkedListBox_0.SelectedIndex);
			Init();
			if (applyCommandEventHandler_0 != null)
			{
				applyCommandEventHandler_0();
	...
	{
		if ((checkedListBox_0.SelectedIndex >= 0) & (checkedListBox_0.SelectedIndex <= Nicks.Count - 1) & bool_0)
		{
			numericUpDown_3.Value = (decimal)Nicks[checkedListBox_0.SelectedIndex].Width;
			numericUpDown_2.Value = (decimal)Nicks[checkedListBox_0.SelectedIndex].Height;
			numericUpDown_1.Value = (decimal)Nicks[checkedListBox_0.SelectedIndex].Offset;
			numericUpDown_0.Value = (decimal)Nicks[checkedListBox_0.SelectedIndex].ExtractXPosition;
		}
	}
