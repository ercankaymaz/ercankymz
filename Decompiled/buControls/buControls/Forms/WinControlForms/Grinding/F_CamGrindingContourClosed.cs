...
public class F_CamGrindingContourClosed : Form
{
	...
	public F_CamGrindingContourClosed()
	{
		Class76.smethod_276(this);
	}
	...
	public void Init()
	{
		...
		panel_7.Visible = IsInsideOperation;
		panel_11.Visible = false;
		btn_holesetshow.Enabled = Operation.InsideOperationHoleEnable;
		arrayList = new ArrayList();
		buGeneral.GetEnumTypeValues(camPars.Operations.Direction, ref arrayList);
		buControlCommands.ComboboxAddItem(arrayList, Convert.ToInt32(camPars.Operations.Direction), ref comboBox_2);
		numericUpDown_0.Value = (decimal)camPars.Operations.TargetZ;
		numericUpDown_16.Value = (decimal)camPars.Offsets.OverlapDistance;
		numericUpDown_5.Value = (decimal)camPars.Offsets.Offset;
		numericUpDown_17.Value = camPars.Offsets.OffsetCount;
		...
		if ((SelectedTool >= 0) & (SelectedTool <= Tools.Count - 1))
		{
			...
			if ((Operation.IndieOperationHoleTool >= 0) & (Operation.IndieOperationHoleTool <= Tools.Count - 1))
			{
				comboBox_3.SelectedIndex = Operation.IndieOperationHoleTool;
			}
		}
		numericUpDown_18.Value = (decimal)Operation.IndiseOperationHoleDistance;
		checkBox_3.Checked = Operation.InsideOperationHoleEnable;
		Refresh();
		Properties.Result = DialogResult.None;
		Properties.Inited = true;
		Class76.smethod_469(this);
	}
	...
	internal void method_1(object sender, EventArgs e)
	{
		...
		if (control.Name == btn_holesetshow.Name)
		{
			if (!Properties.Inited)
			{
				return;
		...
		if (control.Name == btn_holecancel.Name)
		{
			panel_11.Visible = false;
		}
		if (control.Name == btn_holeok.Name)
		{
		...
		if (control.Name == btn_holecamdata.Name)
		{
			F_CamGrindingHole f_CamGrindingHole = new F_CamGrindingHole();
			f_CamGrindingHole.camPars = new camParameters(camParsHole);
			for (int i = 0; i <= Tools.Count - 1; i++)
			{
				ToolBase item = new ToolBase(Tools[i]);
				f_CamGrindingHole.Tools.Add(item);
			}
			f_CamGrindingHole.SelectedTool = Operation.IndieOperationHoleTool;
			f_CamGrindingHole.Init();
			f_CamGrindingHole.ShowDialog();
			if (f_CamGrindingHole.Properties.Result == DialogResult.OK)
			{
				camParsHole = new camParameters(f_CamGrindingHole.camPars);
			}
		}
	}
	...
	internal void method_5(object sender, EventArgs e)
	{
		if (Properties.Inited)
		{
			btn_holesetshow.Enabled = checkBox_3.Checked;
