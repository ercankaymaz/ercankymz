...
public class F_SettingsTreeView : Form
{
	...
	public F_SettingsTreeView()
	{
		Class76.smethod_721(this);
	}
	...
	public void Init()
	{
		...
		for (int i = 0; i <= Classes.Count - 1; i++)
		{
			...
			buSerilization.GetClassVariables(Classes[i], ref Vars);
		...
		for (int j = 0; j <= tempClasses.Count - 1; j++)
		{
			...
			buSerilization.GetClassVariables(tempClasses[j], ref Vars2);
			for (int k = 0; k <= Vars2.Count - 1; k++)
			{
				...
				bool flag2 = false;
				if ((type2.BaseType.Namespace == "buClass") | (type2.BaseType.FullName.IndexOf("buSerilization") >= 0))
				{
					flag2 = true;
				}
				if (!(flag2 && !flag))
				...
				buSerilization.GetClassVariables(Vars2[k].Value, ref Vars3);
	...
	internal void method_6(object sender, EventArgs e)
	{
		if (treeView_0.SelectedNode != null)
		{
			...
			for (int i = 0; i <= treeView_0.Nodes.Count - 1; i++)
			{
				...
				if (treeView_0.Nodes[i].Nodes != null)
				{
					for (int j = 0; j <= treeView_0.Nodes[i].Nodes.Count - 1; j++)
					{
						...
						buSerilization.GetClassVariables(ObjPar, ref Vars);
			...
			for (int k = 0; k <= tempClasses.Count - 1; k++)
			{
				...
				buSerilization.CopyClass(tempClasses[k], ref obj);
				Classes[k] = obj;
			}
			...
			if (treeView_0.Nodes[i].Nodes != null)
			{
				...
				else
				{
					for (int k = 0; k <= treeView_0.Nodes[i].Nodes.Count - 1; k++)
					{
						...
						buSerilization.GetClassVariables(tag2, ref Vars2);
