...
public struct Asn1Tag : IEquatable<Asn1Tag>
{
	...
	{
		return new Asn1Tag((byte)(_controlFlags & -33), TagValue);
	}
	...
	public static Asn1Tag Decode(ReadOnlySpan<byte> source, out int bytesConsumed)
	{
		...
		throw new AsnContentException(System_002EFormats_002EAsn13538873_002ESR.ContentException_InvalidTag);
	}
	...
	{
		string text = ((TagClass != TagClass.Universal) ? $"{TagClass}-{TagValue}" : ((UniversalTagNumber)TagValue/*cast due to constrained. prefix*/).ToString());
		if (IsConstructed)
		{
