...
{
	public struct Enumerator([In][System_002EMemory_002EIsReadOnly] ref ReadOnlySequence<T> sequence)
	{
	...
	public ReadOnlySequence<T> Slice(long start, SequencePosition end)
	{
		...
		else
		{
			...
			if (num3 <= start)
			{
				...
				SequencePosition start2 = SeekMultiSegment(readOnlySequenceSegment.Next, obj, (int)index, start - num3, System.ExceptionArgument.start);
	...
	public ReadOnlySequence<T> Slice(SequencePosition start, long length)
	{
		...
		else
		{
			...
			if (num3 < length)
			{
				...
				SequencePosition end = SeekMultiSegment(readOnlySequenceSegment.Next, obj3, (int)index3, length - num3, System.ExceptionArgument.length);
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	internal bool TryGetBuffer([In][System_002EMemory_002EIsReadOnly] ref SequencePosition position, out ReadOnlyMemory<T> memory, out SequencePosition next)
	{
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private SequencePosition Seek([In][System_002EMemory_002EIsReadOnly] ref SequencePosition start, [In][System_002EMemory_002EIsReadOnly] ref SequencePosition end, long offset, System.ExceptionArgument argument)
	{
	...
	private void BoundsCheck([In][System_002EMemory_002EIsReadOnly] ref SequencePosition position)
	{
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private static int GetIndex([In][System_002EMemory_002EIsReadOnly] ref SequencePosition position)
	{
		return position.GetInteger() & 0x7FFFFFFF;
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private ReadOnlySequence<T> SliceImpl([In][System_002EMemory_002EIsReadOnly] ref SequencePosition start, [In][System_002EMemory_002EIsReadOnly] ref SequencePosition end)
	{
