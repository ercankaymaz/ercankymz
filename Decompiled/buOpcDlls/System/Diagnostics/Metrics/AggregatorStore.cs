...
internal struct AggregatorStore<TAggregator>(Func<TAggregator> createAggregator) where TAggregator : Aggregator
{
	...
	public ConcurrentDictionary<TObjectSequence, TAggregator> GetLabelValuesDictionary<TStringSequence, TObjectSequence>([In][System_002EDiagnostics_002EDiagnosticSource_002EIsReadOnly] ref TStringSequence names) where TStringSequence : IStringSequence, IEquatable<TStringSequence> where TObjectSequence : IObjectSequence, IEquatable<TObjectSequence>
	{
		MultiSizeLabelNameDictionary<TAggregator> multiSizeLabelNameDictionary2;
		while (true)
		{
