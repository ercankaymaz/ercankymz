...
public class CallResponse : IEncodeable, ICloneable, IJsonEncodeable, IServiceResponse
{
	...
	public virtual void Decode(IDecoder decoder)
	{
		...
		Results = (CallMethodResult[])decoder.ReadEncodeableArray("Results", typeof(CallMethodResult));
	...
	public virtual bool IsEqual(IEncodeable encodeable)
	{
		...
		if (!(encodeable is CallResponse callResponse))
		{
			return false;
		}
		if (!Utils.IsEqual(m_responseHeader, callResponse.m_responseHeader))
		{
			return false;
		}
		if (!Utils.IsEqual(m_results, callResponse.m_results))
		{
			return false;
		}
		if (!Utils.IsEqual(m_diagnosticInfos, callResponse.m_diagnosticInfos))
		{
			return false;
		}
		return true;
	...
	{
		return (CallResponse)MemberwiseClone();
	}

	public new object MemberwiseClone()
	{
		CallResponse obj = (CallResponse)base.MemberwiseClone();
		obj.m_responseHeader = (ResponseHeader)Utils.Clone(m_responseHeader);
		obj.m_results = (CallMethodResultCollection)Utils.Clone(m_results);
