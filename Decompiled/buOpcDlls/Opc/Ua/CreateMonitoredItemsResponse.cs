...
[ComVisible(true)]
public class CreateMonitoredItemsResponse : IEncodeable, ICloneable, IJsonEncodeable, IServiceResponse
{
	...
	public virtual void Decode(IDecoder decoder)
	{
		...
		Results = (MonitoredItemCreateResult[])decoder.ReadEncodeableArray("Results", typeof(MonitoredItemCreateResult));
	...
	public virtual bool IsEqual(IEncodeable encodeable)
	{
		...
		if (!(encodeable is CreateMonitoredItemsResponse createMonitoredItemsResponse))
		{
			return false;
		}
		if (!Utils.IsEqual(m_responseHeader, createMonitoredItemsResponse.m_responseHeader))
		{
			return false;
		}
		if (!Utils.IsEqual(m_results, createMonitoredItemsResponse.m_results))
		{
			return false;
		}
		if (!Utils.IsEqual(m_diagnosticInfos, createMonitoredItemsResponse.m_diagnosticInfos))
		{
			return false;
		}
		return true;
	...
	{
		return (CreateMonitoredItemsResponse)MemberwiseClone();
	}

	public new object MemberwiseClone()
	{
		CreateMonitoredItemsResponse obj = (CreateMonitoredItemsResponse)base.MemberwiseClone();
		obj.m_responseHeader = (ResponseHeader)Utils.Clone(m_responseHeader);
		obj.m_results = (MonitoredItemCreateResultCollection)Utils.Clone(m_results);
