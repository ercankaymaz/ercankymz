...
[ComVisible(true)]
public class TransferSubscriptionsResponse : IEncodeable, ICloneable, IJsonEncodeable, IServiceResponse
{
	...
	public virtual void Decode(IDecoder decoder)
	{
		...
		Results = (TransferResult[])decoder.ReadEncodeableArray("Results", typeof(TransferResult));
	...
	public virtual bool IsEqual(IEncodeable encodeable)
	{
		...
		if (!(encodeable is TransferSubscriptionsResponse transferSubscriptionsResponse))
		{
			return false;
		}
		if (!Utils.IsEqual(m_responseHeader, transferSubscriptionsResponse.m_responseHeader))
		{
			return false;
		}
		if (!Utils.IsEqual(m_results, transferSubscriptionsResponse.m_results))
		{
			return false;
		}
		if (!Utils.IsEqual(m_diagnosticInfos, transferSubscriptionsResponse.m_diagnosticInfos))
		{
			return false;
		}
		return true;
	...
	{
		return (TransferSubscriptionsResponse)MemberwiseClone();
	}

	public new object MemberwiseClone()
	{
		TransferSubscriptionsResponse obj = (TransferSubscriptionsResponse)base.MemberwiseClone();
		obj.m_responseHeader = (ResponseHeader)Utils.Clone(m_responseHeader);
		obj.m_results = (TransferResultCollection)Utils.Clone(m_results);
