using System;
using System.CodeDom.Compiler;
using System.Runtime.InteropServices;
using System.Runtime.Serialization;

namespace Opc.Ua;

[GeneratedCode("Opc.Ua.ModelCompiler", "1.0.0.0")]
[DataContract(Namespace = "http://opcfoundation.org/UA/2008/02/Types.xsd")]
[ComVisible(true)]
public class FilterOperand : IEncodeable, ICloneable, IJsonEncodeable
{
	private ContentFilterElement m_parent;

	public virtual ExpandedNodeId TypeId => DataTypeIds.FilterOperand;

	public virtual ExpandedNodeId BinaryEncodingId => ObjectIds.FilterOperand_Encoding_DefaultBinary;

	public virtual ExpandedNodeId XmlEncodingId => ObjectIds.FilterOperand_Encoding_DefaultXml;

	public virtual ExpandedNodeId JsonEncodingId => ObjectIds.FilterOperand_Encoding_DefaultJson;

	public ContentFilterElement Parent
	{
		get
		{
			return m_parent;
		}
		internal set
		{
			m_parent = value;
		}
	}

	public FilterOperand()
	{
		Initialize();
	}

	[OnDeserializing]
	private void Initialize(StreamingContext context)
	{
		Initialize();
	}

	private void Initialize()
	{
	}

	public virtual void Encode(IEncoder encoder)
	{
		encoder.PushNamespace("http://opcfoundation.org/UA/2008/02/Types.xsd");
		encoder.PopNamespace();
	}

	public virtual void Decode(IDecoder decoder)
	{
		decoder.PushNamespace("http://opcfoundation.org/UA/2008/02/Types.xsd");
		decoder.PopNamespace();
	}

	public virtual bool IsEqual(IEncodeable encodeable)
	{
		if (this == encodeable)
		{
			return true;
		}
		if (!(encodeable is FilterOperand))
		{
			return false;
		}
		return true;
	}

	public virtual object Clone()
	{
		return (FilterOperand)MemberwiseClone();
	}

	public new object MemberwiseClone()
	{
		return (FilterOperand)base.MemberwiseClone();
	}

	public virtual ServiceResult Validate(FilterContext context, int index)
	{
		return ServiceResult.Create(2152136704u, "A sub-class of FilterOperand must be specified.");
	}

	public virtual string ToString(INodeTable nodeTable)
	{
		return Utils.Format("{0}", this);
	}
}
