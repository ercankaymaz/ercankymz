...
public class BrowseNextResponse : IEncodeable, ICloneable, IJsonEncodeable, IServiceResponse
{
	...
	public virtual bool IsEqual(IEncodeable encodeable)
	{
		...
		if (!(encodeable is BrowseNextResponse browseNextResponse))
		{
			return false;
		}
		if (!Utils.IsEqual(m_responseHeader, browseNextResponse.m_responseHeader))
		{
			return false;
		}
		if (!Utils.IsEqual(m_results, browseNextResponse.m_results))
		{
			return false;
		}
		if (!Utils.IsEqual(m_diagnosticInfos, browseNextResponse.m_diagnosticInfos))
		{
			return false;
		}
		return true;
	...
	{
		return (BrowseNextResponse)MemberwiseClone();
	}

	public new object MemberwiseClone()
	{
		BrowseNextResponse obj = (BrowseNextResponse)base.MemberwiseClone();
