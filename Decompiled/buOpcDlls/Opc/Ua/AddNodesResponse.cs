...
public class AddNodesResponse : IEncodeable, ICloneable, IJsonEncodeable, IServiceResponse
{
	...
	public virtual void Decode(IDecoder decoder)
	{
		...
		Results = (AddNodesResult[])decoder.ReadEncodeableArray("Results", typeof(AddNodesResult));
	...
	public virtual bool IsEqual(IEncodeable encodeable)
	{
		...
		if (!(encodeable is AddNodesResponse addNodesResponse))
		{
			return false;
		}
		if (!Utils.IsEqual(m_responseHeader, addNodesResponse.m_responseHeader))
		{
			return false;
		}
		if (!Utils.IsEqual(m_results, addNodesResponse.m_results))
		{
			return false;
		}
		if (!Utils.IsEqual(m_diagnosticInfos, addNodesResponse.m_diagnosticInfos))
		{
			return false;
		}
		return true;
	...
	{
		return (AddNodesResponse)MemberwiseClone();
	}

	public new object MemberwiseClone()
	{
		AddNodesResponse obj = (AddNodesResponse)base.MemberwiseClone();
		obj.m_responseHeader = (ResponseHeader)Utils.Clone(m_responseHeader);
		obj.m_results = (AddNodesResultCollection)Utils.Clone(m_results);
