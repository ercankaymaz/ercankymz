...
public class JsonEncoder : IJsonEncoder, IEncoder, IDisposable
{
	...
	public JsonEncoder(IServiceMessageContext context, bool useReversibleEncoding)
		: this(context, useReversibleEncoding, false, null, false, 1024)
	{
	}

	public JsonEncoder(IServiceMessageContext context, bool useReversibleEncoding, bool topLevelIsArray = false, Stream stream = null, bool leaveOpen = false, int streamSize = 1024)
	{
		Initialize();
		m_context = context;
		m_stream = stream;
	...
	public JsonEncoder(IServiceMessageContext context, bool useReversibleEncoding, StreamWriter writer, bool topLevelIsArray = false)
	{
		Initialize();
		m_context = context;
		m_writer = writer;
	...
	public static void EncodeSessionLessMessage(IEncodeable message, Stream stream, IServiceMessageContext context, bool leaveOpen)
	{
		...
		JsonEncoder jsonEncoder = new JsonEncoder(context, useReversibleEncoding: true, topLevelIsArray: false, stream, leaveOpen);
		try
		{
	...
	public static ArraySegment<byte> EncodeMessage(IEncodeable message, byte[] buffer, IServiceMessageContext context)
	{
		...
		using JsonEncoder jsonEncoder = new JsonEncoder(context, useReversibleEncoding: true, topLevelIsArray: false, stream);
	...
	{
		if (fieldName != null && !IncludeDefaultNumberValues && value > -1E-45f && value < float.Epsilon)
		{
			WriteSimpleField(fieldName, null, quotes: false);
		}
	...
	{
		if (fieldName != null && !IncludeDefaultNumberValues && value > -5E-324 && value < double.Epsilon)
		{
			WriteSimpleField(fieldName, null, quotes: false);
		}
	...
	private void WriteNamespaceIndex(string fieldName, ushort namespaceIndex)
	{
		...
		if ((!UseReversibleEncoding || ForceNamespaceUri) && (int)namespaceIndex > ((!ForceNamespaceUriForIndex1) ? 1 : 0))
		{
			string value = m_context.NamespaceUris.GetString(namespaceIndex);
	...
	public void WriteNodeId(string fieldName, NodeId value)
	{
		...
		if (ForceNamespaceUri && (int)namespaceIndex > ((!ForceNamespaceUriForIndex1) ? 1 : 0))
		{
	...
	public void WriteExpandedNodeId(string fieldName, ExpandedNodeId value)
	{
		...
		if (ForceNamespaceUri && text == null && (int)namespaceIndex > ((!ForceNamespaceUriForIndex1) ? 1 : 0))
		{
			text = Context.NamespaceUris.GetString(namespaceIndex);
		}
		WriteNodeIdContents(value.InnerNodeId, text);
		uint num = value.ServerIndex;
		if (num >= 1)
		{
			string value2 = m_context.ServerUris.GetString(num);
			if (!string.IsNullOrEmpty(value2))
			{
				WriteSimpleField("ServerUri", value2, quotes: true);
				PopStructure();
				return;
			}
	...
	public void WriteExtensionObject(string fieldName, ExtensionObject value)
	{
		...
		{
			if (value.Body is IStructureTypeInfo { StructureType: StructureType.Union })
			{
	...
	public void WriteArray(string fieldName, object array, int valueRank, BuiltInType builtInType)
	{
		if (valueRank == 1)
		{
			...
			case BuiltInType.Enumeration:
				...
				WriteEnumeratedArray(fieldName, array2, array2.GetType().GetElementType());
				break;
			case BuiltInType.Variant:
		...
		else
		{
			...
			if (matrix == null)
			{
				if (!(array is Array array3) || array3.Rank != valueRank)
				{
				...
				matrix = new Matrix(array3, builtInType);
			}
			if (matrix != null)
			{
				int index = 0;
