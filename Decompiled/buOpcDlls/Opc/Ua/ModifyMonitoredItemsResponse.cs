...
[ComVisible(true)]
public class ModifyMonitoredItemsResponse : IEncodeable, ICloneable, IJsonEncodeable, IServiceResponse
{
	...
	public virtual void Decode(IDecoder decoder)
	{
		...
		Results = (MonitoredItemModifyResult[])decoder.ReadEncodeableArray("Results", typeof(MonitoredItemModifyResult));
	...
	public virtual bool IsEqual(IEncodeable encodeable)
	{
		...
		if (!(encodeable is ModifyMonitoredItemsResponse modifyMonitoredItemsResponse))
		{
			return false;
		}
		if (!Utils.IsEqual(m_responseHeader, modifyMonitoredItemsResponse.m_responseHeader))
		{
			return false;
		}
		if (!Utils.IsEqual(m_results, modifyMonitoredItemsResponse.m_results))
		{
			return false;
		}
		if (!Utils.IsEqual(m_diagnosticInfos, modifyMonitoredItemsResponse.m_diagnosticInfos))
		{
			return false;
		}
		return true;
	...
	{
		return (ModifyMonitoredItemsResponse)MemberwiseClone();
	}

	public new object MemberwiseClone()
	{
		ModifyMonitoredItemsResponse obj = (ModifyMonitoredItemsResponse)base.MemberwiseClone();
		obj.m_responseHeader = (ResponseHeader)Utils.Clone(m_responseHeader);
		obj.m_results = (MonitoredItemModifyResultCollection)Utils.Clone(m_results);
