...
public class HistoryReadResponse : IEncodeable, ICloneable, IJsonEncodeable, IServiceResponse
{
	...
	public virtual void Decode(IDecoder decoder)
	{
		...
		Results = (HistoryReadResult[])decoder.ReadEncodeableArray("Results", typeof(HistoryReadResult));
	...
	public virtual bool IsEqual(IEncodeable encodeable)
	{
		...
		if (!(encodeable is HistoryReadResponse historyReadResponse))
		{
			return false;
		}
		if (!Utils.IsEqual(m_responseHeader, historyReadResponse.m_responseHeader))
		{
			return false;
		}
		if (!Utils.IsEqual(m_results, historyReadResponse.m_results))
		{
			return false;
		}
		if (!Utils.IsEqual(m_diagnosticInfos, historyReadResponse.m_diagnosticInfos))
		{
			return false;
		}
		return true;
	...
	{
		return (HistoryReadResponse)MemberwiseClone();
	}

	public new object MemberwiseClone()
	{
		HistoryReadResponse obj = (HistoryReadResponse)base.MemberwiseClone();
		obj.m_responseHeader = (ResponseHeader)Utils.Clone(m_responseHeader);
		obj.m_results = (HistoryReadResultCollection)Utils.Clone(m_results);
