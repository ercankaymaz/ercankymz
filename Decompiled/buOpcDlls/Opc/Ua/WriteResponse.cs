...
public class WriteResponse : IEncodeable, ICloneable, IJsonEncodeable, IServiceResponse
{
	...
	public virtual bool IsEqual(IEncodeable encodeable)
	{
		...
		if (!(encodeable is WriteResponse writeResponse))
		{
			return false;
		}
		if (!Utils.IsEqual(m_responseHeader, writeResponse.m_responseHeader))
		{
			return false;
		}
		if (!Utils.IsEqual(m_results, writeResponse.m_results))
		{
			return false;
		}
		if (!Utils.IsEqual(m_diagnosticInfos, writeResponse.m_diagnosticInfos))
		{
			return false;
		}
		return true;
	...
	{
		return (WriteResponse)MemberwiseClone();
	}

	public new object MemberwiseClone()
	{
		WriteResponse obj = (WriteResponse)base.MemberwiseClone();
