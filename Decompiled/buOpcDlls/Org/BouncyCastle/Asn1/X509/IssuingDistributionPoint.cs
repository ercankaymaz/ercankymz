...
public class IssuingDistributionPoint : Asn1Encodable
{
	...
	public override string ToString()
	{
		...
		{
			AppendObject(stringBuilder, "onlyContainsUserCerts", _onlyContainsUserCerts.ToString());
		}
		if (_onlyContainsCACerts)
		{
			AppendObject(stringBuilder, "onlyContainsCACerts", _onlyContainsCACerts.ToString());
		}
		...
		{
			AppendObject(stringBuilder, "onlyContainsAttributeCerts", _onlyContainsAttributeCerts.ToString());
		}
		if (_indirectCRL)
		{
			AppendObject(stringBuilder, "indirectCRL", _indirectCRL.ToString());
		}
