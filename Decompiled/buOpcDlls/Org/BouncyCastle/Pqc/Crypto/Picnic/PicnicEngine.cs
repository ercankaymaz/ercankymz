...
internal sealed class PicnicEngine
{
	...
	private int picnic_verify(byte[] pk, byte[] message, byte[] signature, uint sigLen)
	{
		...
		byte[] array2 = new byte[PICNIC_MAX_LOWMC_BLOCK_SIZE];
		picnic_read_public_key(array2, array, pk);
		uint[] array3 = new uint[stateSizeWords];
		uint[] array4 = new uint[stateSizeWords];
		Pack.LE_To_UInt32(array, 0, array4);
		Pack.LE_To_UInt32(array2, 0, array3);
		if (is_picnic3(parameters))
		{
			Signature2 sig = new Signature2(this);
			DeserializeSignature2(sig, signature, sigLen, message.Length + 4);
			return verify_picnic3(sig, array3, array4, message);
		}
		...
		return Verify(sig2, array3, array4, message);
	}
	...
	private int Verify(Signature sig, uint[] pubKey, uint[] plaintext, byte[] message)
	{
		...
		for (int num = 0; num < numMPCRounds; num++)
		{
			...
			VerifyProof(proofs[num], array5[num], array6[num], GetChallenge(challengeBits, num), sig.salt, (uint)num, tmp, plaintext, tape);
			...
			xor_three(array7, array5[num].outputShare, array6[num].outputShare, pubKey, stateSizeBytes);
	...
	private void VerifyProof(Signature.Proof proof, View view1, View view2, int challenge, byte[] salt, uint roundNumber, byte[] tmp, uint[] plaintext, Tape tape)
	{
		...
		byte[] array = new byte[stateSizeBytes * 4];
		Pack.UInt32_To_LE(view1.inputShare, array, 0);
		Arrays.Fill(array, stateSizeBytes, array.Length, 0);
		PicnicUtilities.ZeroTrailingBits(array, stateSizeBits);
		Pack.LE_To_UInt32(array, 0, view1.inputShare);
		Pack.UInt32_To_LE(view2.inputShare, array, 0);
		Arrays.Fill(array, stateSizeBytes, array.Length, 0);
		PicnicUtilities.ZeroTrailingBits(array, stateSizeBits);
		Pack.LE_To_UInt32(array, 0, view2.inputShare);
	...
	private void mpc_AND_verify(uint[] in1, uint[] in2, uint[] output, Tape rand, View view1, View view2)
	{
		...
		output[0] = (in1[0] & in2[1]) ^ (in1[1] & in2[0]) ^ (in1[0] & in2[0]) ^ array[0] ^ array[1];
	...
	private void mpc_xor_constant_verify(uint[] state, uint[] input, int inOffset, int length, int challenge)
	{
		...
		{
			state[i + num] = state[i + num] ^ input[i + inOffset];
		}
	}
	...
	private int DeserializeSignature(Signature sig, byte[] sigBytes, uint sigBytesLen, int sigBytesOffset)
	{
		...
		int num = ComputeInputShareSize(sigBytes, stateSizeBytes);
		int num2 = PicnicUtilities.NumBytes(2 * numMPCRounds) + saltSizeBytes + numMPCRounds * (2 * seedSizeBytes + andSizeBytes + digestSizeBytes) + num;
		if (transform == TRANSFORM_UR)
		{
			num2 += UnruhGWithoutInputBytes * numMPCRounds;
		}
		if (sigBytesLen < num2)
		{
			return -1;
		...
		for (int i = 0; i < numMPCRounds; i++)
		{
			...
			if (transform == TRANSFORM_UR)
			{
				...
				Array.Copy(sigBytes, sigBytesOffset, proofs[i].view3UnruhG, 0, num3);
				sigBytesOffset += num3;
			}
	...
	private bool IsChallengeValid(byte[] challengeBits)
	{
		for (int i = 0; i < numMPCRounds; i++)
		{
			if (GetChallenge(challengeBits, i) > 2)
			{
				return false;
			}
		}
	...
	{
		byte[] array = new byte[PICNIC_MAX_LOWMC_BLOCK_SIZE];
		Array.Copy(sk, 1, array, 0, stateSizeBytes);
		byte[] array2 = new byte[PICNIC_MAX_LOWMC_BLOCK_SIZE];
		Array.Copy(sk, 1 + stateSizeBytes, array2, 0, stateSizeBytes);
		byte[] array3 = new byte[PICNIC_MAX_LOWMC_BLOCK_SIZE];
		Array.Copy(sk, 1 + 2 * stateSizeBytes, array3, 0, stateSizeBytes);
		uint[] array4 = new uint[stateSizeWords];
		uint[] array5 = new uint[stateSizeWords];
		uint[] array6 = new uint[stateSizeWords];
		Pack.LE_To_UInt32(array, 0, array4);
		Pack.LE_To_UInt32(array3, 0, array6);
		Pack.LE_To_UInt32(array2, 0, array5);
		if (!is_picnic3(parameters))
		{
			Signature sig = new Signature(this);
			if (sign_picnic1(array4, array5, array6, message, sig) != 0)
			{
				Console.Error.Write("Failed to create signature\n");
			}
			int num = SerializeSignature(sig, signature, message.Length + 4);
			if (num == -1)
			{
				Console.Error.Write("Failed to serialize signature\n");
			}
			signatureLength = num;
			Pack.UInt32_To_LE((uint)num, signature, 0);
		}
		else
		{
			Signature2 sig2 = new Signature2(this);
			sign_picnic3(array4, array5, array6, message, sig2);
			Pack.UInt32_To_LE((uint)(signatureLength = SerializeSignature2(sig2, signature, message.Length + 4)), signature, 0);
		}
	}
	...
	{
		View[][] array = new View[numMPCRounds][];
		for (int i = 0; i < numMPCRounds; i++)
		{
			array[i] = new View[3];
		}
		...
		{
			array2[j] = new byte[numMPCParties][];
			for (int k = 0; k < numMPCParties; k++)
			{
				array2[j][k] = new byte[digestSizeBytes];
			}
		}
		byte[][][] array3 = new byte[numMPCRounds][][];
		for (int l = 0; l < numMPCRounds; l++)
		{
			...
			for (int m = 0; m < 3; m++)
			{
				array3[l][m] = new byte[UnruhGWithInputBytes];
			}
		}
		byte[] array4 = ComputeSeeds(privateKey, pubKey, plaintext, message);
		int num = numMPCParties * seedSizeBytes;
		Array.Copy(array4, num * numMPCRounds, sig.salt, 0, saltSizeBytes);
		Tape tape = new Tape(this);
		byte[] array5 = new byte[System.Math.Max(9 * stateSizeBytes, stateSizeBytes + andSizeBytes)];
		byte[] array6 = new byte[stateSizeBytes * 4];
		for (int n = 0; n < numMPCRounds; n++)
		{
			array[n][0] = new View(this);
			array[n][1] = new View(this);
			array[n][2] = new View(this);
			for (int num2 = 0; num2 < 2; num2++)
			{
				if (!CreateRandomTape(array4, num * n + num2 * seedSizeBytes, sig.salt, (uint)n, (uint)num2, array5, stateSizeBytes + andSizeBytes))
				{
					Console.Error.Write("createRandomTape failed \n");
					return -1;
				}
				Array.Copy(array5, 0, array6, 0, stateSizeBytes);
				PicnicUtilities.ZeroTrailingBits(array6, stateSizeBits);
				Pack.LE_To_UInt32(array6, 0, array[n][num2].inputShare);
				Array.Copy(array5, stateSizeBytes, tape.tapes[num2], 0, andSizeBytes);
			}
			if (!CreateRandomTape(array4, num * n + 2 * seedSizeBytes, sig.salt, (uint)n, 2u, tape.tapes[2], andSizeBytes))
			{
				Console.Error.Write("createRandomTape failed \n");
				return -1;
			}
			xor_three(array[n][2].inputShare, privateKey, array[n][0].inputShare, array[n][1].inputShare, stateSizeBytes);
			tape.pos = 0;
			uint[] array7 = Pack.LE_To_UInt32(array5, 0, array5.Length / 4);
			mpc_LowMC(tape, array[n], plaintext, array7);
			Pack.UInt32_To_LE(array7, array5, 0);
			uint[] array8 = new uint[LOWMC_MAX_WORDS];
			xor_three(array8, array[n][0].outputShare, array[n][1].outputShare, array[n][2].outputShare, stateSizeBytes);
			if (!SubarrayEquals(array8, pubKey, stateSizeWords))
			{
				Console.Error.WriteLine("Simulation failed; output does not match public key (round = " + n + ")");
				return -1;
			}
			int num3 = num * n;
			_ = seedSizeBytes;
			Commit(array4, num3 + 0, array[n][0], array2[n][0]);
			Commit(array4, num * n + seedSizeBytes, array[n][1], array2[n][1]);
			Commit(array4, num * n + 2 * seedSizeBytes, array[n][2], array2[n][2]);
			if (transform == TRANSFORM_UR)
			{
				int num4 = num * n;
				_ = seedSizeBytes;
				G(0, array4, num4 + 0, array[n][0], array3[n][0]);
				G(1, array4, num * n + seedSizeBytes, array[n][1], array3[n][1]);
				G(2, array4, num * n + 2 * seedSizeBytes, array[n][2], array3[n][2]);
			}
		}
		uint[][][] array9 = new uint[numMPCRounds][][];
		for (int num5 = 0; num5 < numMPCRounds; num5++)
		{
			array9[num5] = new uint[3][];
			for (int num6 = 0; num6 < 3; num6++)
			{
				array9[num5][num6] = array[num5][num6].outputShare;
			}
		}
		H3(pubKey, plaintext, array9, array2, sig.challengeBits, sig.salt, message, array3);
		for (int num7 = 0; num7 < numMPCRounds; num7++)
		{
			Signature.Proof proof = sig.proofs[num7];
			Prove(proof, GetChallenge(sig.challengeBits, num7), array4, num * num7, array[num7], array2[num7], (transform != TRANSFORM_UR) ? null : array3[num7]);
		}
		return 0;
	}
	...
	private void H3(uint[] circuitOutput, uint[] plaintext, uint[][][] viewOutputs, byte[][][] AS, byte[] challengeBits, byte[] salt, byte[] message, byte[][][] gs)
	{
		...
		for (int k = 0; k < numMPCRounds; k++)
		{
			for (int l = 0; l < 3; l++)
			{
				digest.BlockUpdate(AS[k][l], 0, digestSizeBytes);
			}
		}
		if (transform == TRANSFORM_UR)
		{
			for (int m = 0; m < numMPCRounds; m++)
			{
				for (int n = 0; n < 3; n++)
				{
					int inLen = ((n == 2) ? UnruhGWithInputBytes : UnruhGWithoutInputBytes);
					digest.BlockUpdate(gs[m][n], 0, inLen);
				}
			}
		}
		...
		while (flag)
		{
			for (int num2 = 0; num2 < digestSizeBytes; num2++)
			{
				uint num3 = array[num2];
				for (int num4 = 0; num4 < 8; num4 += 2)
				{
					uint num5 = (num3 >> 6 - num4) & 3;
					if (num5 < 3)
					{
						SetChallenge(challengeBits, num, num5);
						num++;
						if (num == numMPCRounds)
						{
							flag = false;
	...
	private void mpc_AND(uint[] in1, uint[] in2, uint[] output, Tape rand, View[] views)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			output[i] = (in1[i] & in2[(i + 1) % 3]) ^ (in1[(i + 1) % 3] & in2[i]) ^ (in1[i] & in2[i]) ^ array[i] ^ array[(i + 1) % 3];
	...
	private void mpc_xor(uint[] state, uint[] input, int len, int players)
	{
		for (int i = 0; i < players; i++)
		{
			for (int j = 0; j < len; j++)
			{
				state[j + (players + i) * stateSizeWords] = state[j + (players + i) * stateSizeWords] ^ input[j + i * stateSizeWords];
			}
		}
	}
	...
	private void sign_picnic3(uint[] privateKey, uint[] pubKey, uint[] plaintext, byte[] message, Signature2 sig)
	{
		...
		for (int n = 0; n < numMPCRounds; n++)
		{
			...
			xor_array(array8, array8, privateKey, 0, stateSizeWords);
	...
	private int SimulateOnline(uint[] maskedKey, Tape tape, uint[] tmp_shares, Msg msg, uint[] plaintext, uint[] pubKey)
	{
		...
		xor_array(array2, array, plaintext, 0, stateSizeWords);
		for (int i = 1; i <= numRounds; i++)
		{
			...
			kMatricesWithPointer = _lowmcConstants.RConstant(this, i - 1);
			xor_array(array2, array2, kMatricesWithPointer.GetData(), kMatricesWithPointer.GetMatrixPointer(), stateSizeWords);
			kMatricesWithPointer = _lowmcConstants.KMatrix(this, i);
			matrix_mul(array, maskedKey, kMatricesWithPointer.GetData(), kMatricesWithPointer.GetMatrixPointer());
			xor_array(array2, array, array2, 0, stateSizeWords);
		}
	...
	private void CreateRandomTapes(Tape tape, byte[][] seeds, uint seedsOffset, byte[] salt, uint t)
	{
		...
		for (uint num = 0u; num < numMPCParties; num++)
		{
			...
			digest.BlockUpdate(Pack.UInt32_To_LE(num), 0, 2);
	...
	private void ComputeSaltAndRootSeed(byte[] saltAndRoot, uint[] privateKey, uint[] pubKey, uint[] plaintext, byte[] message)
	{
		...
		byte[] array2 = new byte[PICNIC_MAX_LOWMC_BLOCK_SIZE];
		byte[] array3 = new byte[PICNIC_MAX_LOWMC_BLOCK_SIZE];
		Pack.UInt32_To_LE(privateKey, array, 0);
		Pack.UInt32_To_LE(pubKey, array2, 0);
		Pack.UInt32_To_LE(plaintext, array3, 0);
		array = Arrays.CopyOfRange(array, 0, stateSizeBytes);
		array2 = Arrays.CopyOfRange(array2, 0, stateSizeBytes);
		array3 = Arrays.CopyOfRange(array3, 0, stateSizeBytes);
		digest.BlockUpdate(array, 0, stateSizeBytes);
		digest.BlockUpdate(message, 0, message.Length);
		digest.BlockUpdate(array2, 0, stateSizeBytes);
		digest.BlockUpdate(array3, 0, stateSizeBytes);
	...
	private void LowMCEnc(uint[] plaintext, uint[] output, uint[] key)
	{
		...
		xor_array(output, output, array, 0, stateSizeWords);
		for (int i = 1; i <= numRounds; i++)
		{
			kMatricesWithPointer = _lowmcConstants.KMatrix(this, i);
			matrix_mul(array, key, kMatricesWithPointer.GetData(), kMatricesWithPointer.GetMatrixPointer());
			...
			kMatricesWithPointer = _lowmcConstants.RConstant(this, i - 1);
			xor_array(output, output, kMatricesWithPointer.GetData(), kMatricesWithPointer.GetMatrixPointer(), stateSizeWords);
			xor_array(output, output, array, 0, stateSizeWords);
		}
	}
