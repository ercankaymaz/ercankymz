...
internal sealed class BikeRing
{
	...
	{
		int length = (m_bits & 0x3F) + 7 >> 3;
		Pack.LE_To_UInt64(bs, 0, z, 0, Size - 1);
		byte[] array = new byte[8];
		Array.Copy(bs, Size - 1 << 3, array, 0, length);
		z[Size - 1] = Pack.LE_To_UInt64(array);
	}
	...
	internal void EncodeBytes(ulong[] x, byte[] bs)
	{
		...
		byte[] array = new byte[8];
		Pack.UInt64_To_LE(x[Size - 1], array);
		Array.Copy(array, 0, bs, Size - 1 << 3, length);
	}
