...
public class FrodoEngine
{
	...
	public void kem_enc(byte[] ct, byte[] ss, byte[] pk, SecureRandom random)
	{
		...
		((IXof)digest).OutputFinal(array3, 0, len_seedSE_bytes + len_k_bytes);
		byte[] array4 = Arrays.CopyOfRange(array3, 0, len_seedSE_bytes);
		byte[] input2 = Arrays.CopyOfRange(array3, len_seedSE_bytes, len_seedSE_bytes + len_k_bytes);
		byte[] array5 = new byte[(2 * mbar * n + mbar * nbar) * len_chi_bytes];
		digest.Update(150);
		digest.BlockUpdate(array4, 0, array4.Length);
		((IXof)digest).OutputFinal(array5, 0, array5.Length);
		short[] array6 = new short[array5.Length / 2];
		for (int i = 0; i < array6.Length; i++)
		{
			array6[i] = (short)Pack.LE_To_UInt16(array5, i * 2);
		}
		short[] x = SampleMatrix(array6, 0, mbar, n);
		short[] y = SampleMatrix(array6, mbar * n, mbar, n);
		short[] y2 = gen.GenMatrix(seedA);
		short[] c = MatrixAdd(MatrixMul(x, mbar, n, y2, n), y, mbar, n);
		byte[] array7 = FrodoPack(c);
		short[] y3 = SampleMatrix(array6, 2 * mbar * n, mbar, nbar);
		...
		byte[] array8 = FrodoPack(c2);
		Array.Copy(Arrays.Concatenate(array7, array8), 0, ct, 0, len_ct_bytes);
		digest.BlockUpdate(array7, 0, array7.Length);
		digest.BlockUpdate(array8, 0, array8.Length);
		digest.BlockUpdate(input2, 0, len_k_bytes);
