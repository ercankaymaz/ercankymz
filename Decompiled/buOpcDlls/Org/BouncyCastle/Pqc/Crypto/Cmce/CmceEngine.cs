...
internal class CmceEngine<GFImpl> : ICmceEngine where GFImpl : struct, GF
{
	...
	{
		return (byte)((int)(0L - (long)(uint)((x ^ y) - 1 >>> 31)) & 0xFF);
	}
	...
	private int PKGen(byte[] pk, byte[] sk, uint[] perm, ushort[] pi, ulong[] pivots)
	{
		...
		for (int i = 0; i < SYS_T; i++)
		{
			for (int j = 0; j < SYS_N; j += 8)
			{
				...
				{
					byte b = (byte)((array4[j + 7] >> k) & 1);
					b <<= 1;
					b |= (byte)((array4[j + 6] >> k) & 1);
					b <<= 1;
					b |= (byte)((array4[j + 5] >> k) & 1);
					b <<= 1;
					b |= (byte)((array4[j + 4] >> k) & 1);
					b <<= 1;
					b |= (byte)((array4[j + 3] >> k) & 1);
					b <<= 1;
					b |= (byte)((array4[j + 2] >> k) & 1);
					b <<= 1;
					b |= (byte)((array4[j + 1] >> k) & 1);
					b <<= 1;
					b |= (byte)((array4[j] >> k) & 1);
					array5[i * GFBITS + k][j / 8] = b;
				}
			}
		...
		for (int l = 0; l < PK_NROWS; l++)
		{
			int i = l >> 3;
			int j = l & 7;
			if (usePivots && l == PK_NROWS - 32 && MovColumns(array5, pi, pivots) != 0)
			{
				return -1;
			}
			byte[] array6 = array5[l];
			for (int k = l + 1; k < PK_NROWS; k++)
			{
				byte[] array7 = array5[k];
				byte b2 = (byte)(array6[i] ^ array7[i]);
				b2 = (byte)(b2 >> j);
				b2 &= 1;
				int m = 0;
				byte b3 = (byte)(-b2);
				for (int num = SYS_N / 8 - 4; m <= num; m += 4)
				{
					...
					array6[m + 1] ^= (byte)(array7[m + 1] & b3);
					array6[m + 2] ^= (byte)(array7[m + 2] & b3);
					array6[m + 3] ^= (byte)(array7[m + 3] & b3);
				}
				byte b4 = (byte)(-b2);
				for (; m < SYS_N / 8; m++)
				{
					array6[m] ^= (byte)(array7[m] & b4);
				}
			}
			if (((array6[i] >> j) & 1) == 0)
			{
			...
			for (int k = 0; k < PK_NROWS; k++)
			{
				if (k != l)
				{
					byte[] array8 = array5[k];
					byte b2 = (byte)(array8[i] >> j);
					b2 &= 1;
					int m = 0;
					byte b5 = (byte)(-b2);
					for (int num2 = SYS_N / 8 - 4; m <= num2; m += 4)
					{
						array8[m] ^= (byte)(array6[m] & b5);
						array8[m + 1] ^= (byte)(array6[m + 1] & b5);
						array8[m + 2] ^= (byte)(array6[m + 2] & b5);
						array8[m + 3] ^= (byte)(array6[m + 3] & b5);
					}
					byte b6 = (byte)(-b2);
					for (; m < SYS_N / 8; m++)
					{
						array8[m] ^= (byte)(array6[m] & b6);
					}
				}
			}
		}
		if (pk != null)
		{
			if (usePadding)
			{
				...
				for (int i = 0; i < PK_NROWS; i++)
				{
					byte[] array9 = array5[i];
					int j;
					for (j = (PK_NROWS - 1) / 8; j < SYS_N / 8 - 1; j++)
					{
						pk[num3++] = (byte)(((array9[j] & 0xFF) >> num4) | (array9[j + 1] << 8 - num4));
					}
					pk[num3++] = (byte)((array9[j] & 0xFF) >> num4);
				}
			}
			else
			{
				int num5 = (SYS_N - PK_NROWS + 7) / 8;
				for (int i = 0; i < PK_NROWS; i++)
				{
					Array.Copy(array5[i], PK_NROWS / 8, pk, num5 * i, num5);
				}
			}
		}
		return 0;
