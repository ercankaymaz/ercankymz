...
public class CertificateID
{
	...
	private static CertID CreateCertID(AlgorithmIdentifier hashAlg, X509Certificate issuerCert, DerInteger serialNumber)
	{
		try
		{
			...
			byte[] contents = DigestUtilities.CalculateDigest(algorithm, subjectX509Principal.GetEncoded());
			SubjectPublicKeyInfo subjectPublicKeyInfo = SubjectPublicKeyInfoFactory.CreateSubjectPublicKeyInfo(issuerCert.GetPublicKey());
			byte[] contents2 = DigestUtilities.CalculateDigest(algorithm, subjectPublicKeyInfo.PublicKeyData.GetBytes());
			return new CertID(hashAlg, new DerOctetString(contents), new DerOctetString(contents2), serialNumber);
		}
		catch (Exception ex)
		{
