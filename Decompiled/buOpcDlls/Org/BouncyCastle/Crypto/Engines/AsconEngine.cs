...
public sealed class AsconEngine : IAeadCipher
{
	...
	public void Init(bool forEncryption, ICipherParameters parameters)
	{
		...
		{
			throw new ArgumentException(asconParameters.ToString() + " requires exactly " + CRYPTO_ABYTES + " bytes of IV");
		}
		byte[] key = keyParameter.GetKey();
		if (key.Length != CRYPTO_KEYBYTES)
		{
			throw new ArgumentException(asconParameters.ToString() + " key must be " + CRYPTO_KEYBYTES + " bytes long");
		}
