...
public class OaepEncoding : IAsymmetricBlockCipher
{
	...
	private byte[] DecodeBlock(byte[] inBytes, int inOff, int inLen)
	{
		...
		int num = array2.Length - (2 * defHash.Length + 1) >> 31;
		...
		{
			num |= defHash[k] ^ array2[defHash.Length + k];
		}
		int num2 = -1;
		for (int l = 2 * defHash.Length; l != array2.Length; l++)
		{
			int num3 = (-array2[l] & num2) >> 31;
			num2 += l & num3;
		}
		num |= num2 >> 31;
		num2++;
		if ((num | (array2[num2] ^ 1)) != 0)
		{
			Arrays.Fill(array2, 0);
			throw new InvalidCipherTextException("data wrong");
		}
		num2++;
		byte[] array4 = new byte[array2.Length - num2];
		Array.Copy(array2, num2, array4, 0, array4.Length);
	...
	private byte[] MaskGeneratorFunction(byte[] Z, int zOff, int zLen, int length)
	{
		if (mgf1Hash is IXof xof)
		{
			...
			xof.OutputFinal(array, 0, length);
			return array;
		}
