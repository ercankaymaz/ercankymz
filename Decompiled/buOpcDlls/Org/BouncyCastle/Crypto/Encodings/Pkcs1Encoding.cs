...
public class Pkcs1Encoding : IAsymmetricBlockCipher
{
	...
	private byte[] DecodeBlockOrRandom(byte[] input, int inOff, int inLen)
	{
		...
		if (nextBytes == null)
		{
			nextBytes = SecureRandom.GetNextBytes(SecureRandom.ArbitraryRandom, pLen);
		}
		byte[] array2 = ((useStrictLength & (array.Length != engine.GetOutputBlockSize())) ? blockBuffer : array);
		int num = CheckPkcs1Encoding(array2, pLen);
		byte[] array3 = new byte[pLen];
		for (int i = 0; i < pLen; i++)
		{
			array3[i] = (byte)((array2[i + (array2.Length - pLen)] & ~num) | (nextBytes[i] & num));
		}
