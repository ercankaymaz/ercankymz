...
public class ArmoredOutputStream : BaseOutputStream
{
	...
	private static string CreateVersion()
	{
		...
		string title = executingAssembly.GetCustomAttribute<AssemblyTitleAttribute>().Title;
		...
		return title + " v" + informationalVersion;
	}
		...
		: this(outStream)
	{
		...
		{
			List<string> value = new List<string>(1) { header.Value };
			m_headers[header.Key] = value;
		}
	}
	...
	{
		IList<string> valueOrNull = CollectionUtilities.GetValueOrNull(m_headers, HeaderVersion);
		m_headers.Clear();
		if (valueOrNull != null)
		{
			m_headers[HeaderVersion] = valueOrNull;
		}
	}
	...
	public override void WriteByte(byte value)
	{
		...
		if (start)
		{
			...
			foreach (KeyValuePair<string, IList<string>> header in m_headers)
			{
