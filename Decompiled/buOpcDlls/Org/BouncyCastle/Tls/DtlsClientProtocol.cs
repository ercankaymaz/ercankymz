...
public class DtlsClientProtocol : DtlsProtocol
{
	...
	internal virtual DtlsTransport ClientHandshake(ClientHandshakeState state, DtlsRecordLayer recordLayer)
	{
		...
		DtlsReliableHandshake dtlsReliableHandshake = new DtlsReliableHandshake(state.clientContext, recordLayer, state.client.GetHandshakeTimeoutMillis(), TlsUtilities.GetHandshakeResendTimeMillis(state.client), null);
		...
		if (message.Type == 2)
		{
			...
			if (message.Type == 14)
			{
				...
				{
					DigitallySigned certificateVerify = TlsUtilities.GenerateCertificateVerifyClient(state.clientContext, tlsCredentialedSigner, signatureAndHashAlgorithm, tlsStreamSigner, dtlsReliableHandshake.HandshakeHash);
	...
	protected virtual void ProcessCertificateRequest(ClientHandshakeState state, byte[] body)
	{
		...
		state.clientContext.SecurityParameters.m_clientCertificateType = TlsExtensionsUtilities.GetClientCertificateTypeExtensionServer(state.serverExtensions, 0);
	}
	...
	protected virtual void ProcessServerHello(ClientHandshakeState state, byte[] body)
	{
		...
		{
			bool flag = TlsExtensionsUtilities.HasExtendedMasterSecretExtension(state.serverExtensions);
			if (flag)
			{
		...
		IDictionary<int, byte[]> dictionary2 = state.serverExtensions;
		if (state.resumedSession)
		{
