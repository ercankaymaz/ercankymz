...
public class DtlsServerProtocol : DtlsProtocol
{
	...
	internal virtual DtlsTransport ServerHandshake(ServerHandshakeState state, DtlsRecordLayer recordLayer, DtlsRequest request)
	{
		...
		DtlsReliableHandshake dtlsReliableHandshake = new DtlsReliableHandshake(state.serverContext, recordLayer, state.server.GetHandshakeTimeoutMillis(), TlsUtilities.GetHandshakeResendTimeMillis(state.server), request);
	...
	internal virtual byte[] GenerateServerHello(ServerHandshakeState state, DtlsRecordLayer recordLayer)
	{
		...
		if (ProtocolVersion.DTLSv12.Equals(securityParameters.NegotiatedVersion))
		{
			...
			{
				byte[] connectionIDLocal = TlsExtensionsUtilities.GetConnectionIDExtension(state.clientExtensions) ?? throw new TlsFatalAlert(80);
	...
	protected virtual void ProcessClientCertificate(ServerHandshakeState state, byte[] body)
	{
		...
		{
			CertificateType = TlsExtensionsUtilities.GetClientCertificateTypeExtensionServer(state.clientExtensions, 0),
