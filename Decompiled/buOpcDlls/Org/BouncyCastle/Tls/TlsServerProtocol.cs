...
public class TlsServerProtocol : TlsProtocol
{
	...
	protected virtual ServerHello Generate13ServerHello(ClientHello clientHello, HandshakeMessageInput clientHelloMessage, bool afterHelloRetryRequest)
	{
		...
		{
			securityParameters.m_maxFragmentLength = ProcessMaxFragmentLengthExtension(securityParameters.IsResumedSession ? null : extensions, dictionary, 80);
		}
	...
	protected virtual ServerHello GenerateServerHello(ClientHello clientHello, HandshakeMessageInput clientHelloMessage)
	{
		...
		bool flag2 = (securityParameters.m_resumedSession = EstablishSession(m_tlsServer.GetSessionToResume(clientHello.SessionID)));
		if (!flag2)
		{
		...
		m_tlsServer.NotifySession(m_tlsSession);
		TlsUtilities.NegotiatedVersionTlsServer(m_tlsServerContext);
		int cipherSuite = (flag2 ? m_sessionParameters.CipherSuite : m_tlsServer.GetSelectedCipherSuite());
		...
		IDictionary<int, byte[]> extensions = (flag2 ? m_sessionParameters.ReadServerExtensions() : m_tlsServer.GetServerExtensions());
		...
		if (m_serverExtensions.Count > 0)
		{
			...
			securityParameters.m_maxFragmentLength = ProcessMaxFragmentLengthExtension(flag2 ? null : m_clientExtensions, m_serverExtensions, 80);
	...
	protected virtual void Receive13ClientCertificate(MemoryStream buf)
	{
		...
		{
			CertificateType = TlsExtensionsUtilities.GetClientCertificateTypeExtensionServer(m_serverExtensions, 0),
