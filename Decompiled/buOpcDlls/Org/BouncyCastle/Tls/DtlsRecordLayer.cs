...
internal class DtlsRecordLayer : DatagramTransport, DatagramReceiver, DatagramSender, TlsCloseable
{
	...
	private int ProcessRecord(int received, byte[] record, byte[] buf, int off, int len, DtlsRecordCallback recordCallback)
	{
		...
		else if (num == 22 && m_retransmitEpoch != null && num2 == m_retransmitEpoch.Epoch)
		{
	...
	private int ReceivePendingRecord(byte[] buf, int off, int len)
	{
		...
		{
			short num2 = m_recordQueue.ReadUint8(0);
			int num3 = m_recordQueue.ReadUint16(3);
			DtlsEpoch dtlsEpoch = null;
			...
			if (m_recordQueue.Available >= num)
			{
				int num4 = m_recordQueue.ReadUint16(num - 2);
				num += num4;
			}
		}
		int num5 = System.Math.Min(m_recordQueue.Available, num);
		m_recordQueue.RemoveData(buf, off, num5, 0);
		return num5;
	}
	...
	private int ReceiveRecord(byte[] buf, int off, int len, int waitMillis)
	{
		...
		{
			m_inConnection = true;
			short num2 = TlsUtilities.ReadUint8(buf, off);
			int num3 = TlsUtilities.ReadUint16(buf, off + 3);
			DtlsEpoch dtlsEpoch = null;
			if (num3 == m_readEpoch.Epoch)
			{
				dtlsEpoch = m_readEpoch;
			...
			{
				int num4 = TlsUtilities.ReadUint16(buf, off + recordHeaderLengthRead - 2);
				int num5 = recordHeaderLengthRead + num4;
				if (num > num5)
				{
					m_recordQueue.AddData(buf, off + num5, num - num5);
					num = num5;
				}
			}
		}
