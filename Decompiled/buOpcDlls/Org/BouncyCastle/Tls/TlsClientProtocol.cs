...
public class TlsClientProtocol : TlsProtocol
{
	...
	protected virtual void ProcessServerHello(ServerHello serverHello)
	{
		...
		bool flag = TlsExtensionsUtilities.HasExtendedMasterSecretExtension(m_serverExtensions);
		bool isResumedSession = securityParameters.IsResumedSession;
		if (flag)
		{
			if (protocolVersion.IsSsl || (!isResumedSession && !m_tlsClient.ShouldUseExtendedMasterSecret()))
			{
				throw new TlsFatalAlert(40);
			}
		}
		else if (m_tlsClient.RequiresExtendedMasterSecret() || (isResumedSession && !m_tlsClient.AllowLegacyResumption()))
		{
			throw new TlsFatalAlert(40);
		}
		...
		IDictionary<int, byte[]> dictionary2 = m_serverExtensions;
		if (securityParameters.IsResumedSession)
		{
	...
	protected virtual void Receive13CertificateRequest(MemoryStream buf, bool postHandshakeAuth)
	{
		...
		m_tlsClientContext.SecurityParameters.m_clientCertificateType = TlsExtensionsUtilities.GetClientCertificateTypeExtensionServer(m_serverExtensions, 0);
	...
	protected virtual void SendClientHello()
	{
		...
		bool flag3 = m_tlsClient.IsFallback();
		...
		bool num = TlsUtilities.GetExtensionData(m_clientExtensions, 65281) == null;
		bool flag4 = !Arrays.Contains(array2, 255);
		if (num && flag4)
		{
			array2 = Arrays.Append(array2, 255);
		}
		if (flag3 && !Arrays.Contains(array2, 22016))
		{
