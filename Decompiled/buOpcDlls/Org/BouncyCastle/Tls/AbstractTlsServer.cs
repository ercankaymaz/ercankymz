...
public abstract class AbstractTlsServer : AbstractTlsPeer, TlsServer, TlsPeer
{
	...
	public virtual IDictionary<int, byte[]> GetServerExtensions()
	{
		...
		if (serverCertificateTypeExtensionClient != null)
		{
			...
			if (credentials == null || !Arrays.Contains(serverCertificateTypeExtensionClient, credentials.Certificate.CertificateType))
			{
				throw new TlsFatalAlert(43);
			}
			TlsExtensionsUtilities.AddServerCertificateTypeExtensionServer(m_serverExtensions, credentials.Certificate.CertificateType);
		}
	...
	public virtual void GetServerExtensionsForConnection(IDictionary<int, byte[]> serverExtensions)
	{
		...
		if (ProtocolVersion.DTLSv12.Equals(m_context.ServerVersion) && m_clientExtensions.ContainsKey(54))
		{
