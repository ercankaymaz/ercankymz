...
public sealed class BigInteger : IComparable, IComparable<BigInteger>, IEquatable<BigInteger>
{
	...
	public int CompareTo(BigInteger other)
	{
		...
		if (sign >= other.sign)
		{
			if (sign <= other.sign)
			{
				if (sign != 0)
				{
					return sign * CompareNoLeadingZeroes(0, magnitude, 0, other.magnitude);
				}
				return 0;
			}
			return 1;
	...
	private BigInteger ModInversePow2(BigInteger m)
	{
		...
		long num2 = (long)Org.BouncyCastle.Math.Raw.Mod.Inverse64((ulong)LongValue);
		if (num < 64)
		{
			num2 &= (1L << num) - 1;
		}
	...
	private static BigInteger ModPowBarrett(BigInteger b, BigInteger e, BigInteger m)
	{
		...
		{
			num4 = num3 & 0xFF;
			int num7 = num5 + BitLen((byte)num4);
	...
	private static BigInteger ModPowMonty(BigInteger b, BigInteger e, BigInteger m, bool convert)
	{
		...
		SquareMonty(a, array4, m.magnitude, mQuote, flag);
		for (int j = 1; j < num3; j++)
		{
			array3[j] = Arrays.Clone(array3[j - 1]);
			MultiplyMonty(a, array3[j], array4, m.magnitude, mQuote, flag);
		}
		int[] windowList = GetWindowList(e.magnitude, i);
		...
		{
			num5 = num4 & 0xFF;
			int num8 = num6 + BitLen((byte)num5);
			for (int k = 0; k < num8; k++)
			{
				SquareMonty(a, array5, m.magnitude, mQuote, flag);
			}
			MultiplyMonty(a, array5, array3[num5 >> 1], m.magnitude, mQuote, flag);
			num6 = num4 >> 8;
		}
	...
	{
		return Org.BouncyCastle.Math.Raw.Mod.Inverse32(0 - magnitude[magnitude.Length - 1]);
	}
	...
	private static BigInteger CreateUValueOf(ulong value)
	{
		...
		if (num2 != 0)
		{
			BigInteger bigInteger = new BigInteger(1, new uint[1] { num2 }, checkMag: false);
			if ((num2 & (0L - (long)num2)) == num2)
			{
				bigInteger.nBits = 1;
			}
			return bigInteger;
