...
public abstract class AbstractFpCurve : ECCurve
{
		...
		: base(FiniteFields.GetPrimeField(q))
	{
		if (!isInternal)
		{
			...
			lock (KnownQs)
			{
				flag = !KnownQs.Contains(q);
			}
			if (flag)
			{
				int num = ImplGetInteger("Org.BouncyCastle.EC.Fp_MaxSize", 1042);
				...
				if (Primes.HasAnySmallFactors(q) || !Primes.IsMRProbablePrime(q, SecureRandom.ArbitraryRandom, ImplGetNumberOfIterations(bitLength, certainty)))
				{
