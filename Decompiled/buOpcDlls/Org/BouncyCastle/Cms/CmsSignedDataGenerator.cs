...
public class CmsSignedDataGenerator : CmsSignedGenerator
{
	private class SignerInf
	{
		...
		internal SignerInf(CmsSignedGenerator outer, AsymmetricKeyParameter key, SecureRandom random, SignerIdentifier signerIdentifier, string digestOID, string encOID, CmsAttributeTableGenerator sAttr, CmsAttributeTableGenerator unsAttr, Org.BouncyCastle.Asn1.Cms.AttributeTable baseSignedTable)
		{
		...
		internal SignerInfo ToSignerInfo(DerObjectIdentifier contentType, CmsProcessable content)
		{
			...
			if (!outer.m_digests.TryGetValue(digestOID, out var value))
			{
				...
				outer.m_digests.Add(digestOID, (byte[])value.Clone());
			}
