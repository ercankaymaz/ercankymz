...
public sealed class JsonDocument : IDisposable
{
	...
	private struct MetadataDb : IDisposable
	{
		...
		internal void SetHasComplexChildren(int index)
		{
			...
			int value = MemoryMarshal.Read<int>(span) | int.MinValue;
			MemoryMarshal.Write(span, ref value);
		}
	...
	internal bool TextEquals(int index, ReadOnlySpan<char> otherText, bool isPropertyName)
	{
		...
		int written;
		bool result = JsonWriterHelper.ToUtf8(otherText, destination, out written) != OperationStatus.InvalidData && TextEquals(index, destination.Slice(0, written), isPropertyName, shouldUnescape: true);
		if (array != null)
		{
	...
	{
		TryParseValue(ref reader, out var document, shouldThrow: true, useArrayPools: true);
		return document;
	}
