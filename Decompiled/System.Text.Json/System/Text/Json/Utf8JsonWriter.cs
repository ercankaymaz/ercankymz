...
public sealed class Utf8JsonWriter : IDisposable, IAsyncDisposable
{
	...
	private void WriteNumberMinimized(ReadOnlySpan<char> escapedPropertyName, decimal value)
	{
		...
		span[BytesPending++] = 34;
		span[BytesPending++] = 58;
		Utf8Formatter.TryFormat(value, span.Slice(BytesPending), out var bytesWritten);
		BytesPending += bytesWritten;
	}
	...
	private void WriteNumberMinimized(ReadOnlySpan<char> escapedPropertyName, double value)
	{
		...
		span[BytesPending++] = 34;
		span[BytesPending++] = 58;
		TryFormatDouble(value, span.Slice(BytesPending), out var bytesWritten);
		BytesPending += bytesWritten;
	}
	...
	internal void WritePropertyName(double value)
	{
		...
		TryFormatDouble(value, destination, out var bytesWritten);
		WritePropertyNameUnescaped(destination.Slice(0, bytesWritten));
	}
	...
	private void WriteNumberMinimized(ReadOnlySpan<char> escapedPropertyName, float value)
	{
		...
		span[BytesPending++] = 34;
		span[BytesPending++] = 58;
		TryFormatSingle(value, span.Slice(BytesPending), out var bytesWritten);
		BytesPending += bytesWritten;
	}
	...
	{
		Span<byte> destination = stackalloc byte[128];
		TryFormatSingle(value, destination, out var bytesWritten);
		WritePropertyNameUnescaped(destination.Slice(0, bytesWritten));
	}
	...
	{
		JsonWriterHelper.ToUtf8(escapedPropertyName, output.Slice(BytesPending), out var written);
	...
	private void WriteCommentMinimized(ReadOnlySpan<char> value)
	{
		...
		if (JsonWriterHelper.ToUtf8(value, span.Slice(BytesPending), out var written) == OperationStatus.InvalidData)
		{
	...
	private void Base64EncodeAndWrite(ReadOnlySpan<byte> bytes, Span<byte> output)
	{
		...
		Base64.EncodeToUtf8(bytes, utf, out var _, out var bytesWritten);
		BytesPending += bytesWritten;
	}
