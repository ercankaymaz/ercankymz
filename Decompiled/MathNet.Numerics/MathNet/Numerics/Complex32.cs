...
public readonly struct Complex32(float real, float imaginary) : IFormattable, IEquatable<Complex32>
{
	[DataMember(Order = 1)]
	private readonly float _real = real;

	[DataMember(Order = 2)]
	private readonly float _imag = imaginary;
	...
	{
		return string.Format(CultureInfo.CurrentCulture, "({0}, {1})", _real.ToString(format, CultureInfo.CurrentCulture), _imag.ToString(format, CultureInfo.CurrentCulture));
	}
	...
	{
		return string.Format(provider, "({0}, {1})", _real.ToString(format, provider), _imag.ToString(format, provider));
	}
