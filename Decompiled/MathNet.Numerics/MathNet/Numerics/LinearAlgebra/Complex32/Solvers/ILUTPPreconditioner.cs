...
public sealed class ILUTPPreconditioner : IPreconditioner<MathNet.Numerics.Complex32>
{
	...
	private void PivotRow(Vector<MathNet.Numerics.Complex32> row)
	{
		...
		for (int i = 0; i < row.Count; i++)
		{
			if (_pivots[i] != i && !PivotMapFound(dictionary, i))
			{
				...
				MathNet.Numerics.Complex32 complex = row[_pivots[i]];
				MathNet.Numerics.Complex32 complex2 = row[i];
				MathNet.Numerics.Complex32 complex3 = (row[index] = complex);
				complex3 = (row[index2] = complex2);
			}
		}
	}
	...
	private static void SwapColumns(Matrix<MathNet.Numerics.Complex32> matrix, int firstColumn, int secondColumn)
	{
		for (int i = 0; i < matrix.RowCount; i++)
		{
			int row = i;
			int row2 = i;
			MathNet.Numerics.Complex32 complex = matrix[i, secondColumn];
			MathNet.Numerics.Complex32 complex2 = matrix[i, firstColumn];
			MathNet.Numerics.Complex32 complex3 = (matrix[row, firstColumn] = complex);
			complex3 = (matrix[row2, secondColumn] = complex2);
		}
	}
