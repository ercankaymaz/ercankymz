...
public sealed class ILUTPPreconditioner : IPreconditioner<System.Numerics.Complex>
{
	...
	private void PivotRow(Vector<System.Numerics.Complex> row)
	{
		...
		for (int i = 0; i < row.Count; i++)
		{
			if (_pivots[i] != i && !PivotMapFound(dictionary, i))
			{
				...
				System.Numerics.Complex complex = row[_pivots[i]];
				System.Numerics.Complex complex2 = row[i];
				System.Numerics.Complex complex3 = (row[index] = complex);
	...
	private static void SwapColumns(Matrix<System.Numerics.Complex> matrix, int firstColumn, int secondColumn)
	{
		for (int i = 0; i < matrix.RowCount; i++)
		{
			int row = i;
			int row2 = i;
			System.Numerics.Complex complex = matrix[i, secondColumn];
			System.Numerics.Complex complex2 = matrix[i, firstColumn];
			System.Numerics.Complex complex3 = (matrix[row, firstColumn] = complex);
