	...
	public override void Solve(Vector<double> input, Vector<double> result)
	{
		...
		for (int i = 0; i < Pivots.Length; i++)
		{
			if (Pivots[i] != i)
			{
				...
				int index2 = i;
				double num2 = result[i];
				double num3 = result[num];
				double num4 = (result[index] = num2);
				num4 = (vector[index2] = num3);
			}
		}
		int rowCount = Factors.RowCount;
		for (int j = 0; j < rowCount; j++)
		...
		for (int num7 = rowCount - 1; num7 >= 0; num7--)
		{
			result[num7] /= Factors.At(num7, num7);
			for (int l = 0; l < num7; l++)
			{
				result[l] -= result[num7] * Factors.At(l, num7);
			}
		}
	}
