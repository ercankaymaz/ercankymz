...
public sealed class ILUTPPreconditioner : IPreconditioner<double>
{
	...
	private void PivotRow(Vector<double> row)
	{
		...
		for (int i = 0; i < row.Count; i++)
		{
			if (_pivots[i] != i && !PivotMapFound(dictionary, i))
			{
				...
				double num = row[_pivots[i]];
				double num2 = row[i];
				double num3 = (row[index] = num);
				num3 = (row[index2] = num2);
			}
		}
	}
	...
	private static void SwapColumns(Matrix<double> matrix, int firstColumn, int secondColumn)
	{
		for (int i = 0; i < matrix.RowCount; i++)
		{
			int row = i;
			int row2 = i;
			double num = matrix[i, secondColumn];
			double num2 = matrix[i, firstColumn];
			double num3 = (matrix[row, firstColumn] = num);
			num3 = (matrix[row2, secondColumn] = num2);
		}
	}
