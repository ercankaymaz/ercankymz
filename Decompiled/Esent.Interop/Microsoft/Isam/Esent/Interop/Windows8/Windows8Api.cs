...
public static class Windows8Api
{
	...
	public static void JetGetSessionParameter(JET_SESID sesid, JET_sesparam sesparamid, out int value)
	{
		Api.Check(Api.Impl.JetGetSessionParameter(sesid, sesparamid, out value));
	}

	public static void JetGetSessionParameter(JET_SESID sesid, JET_sesparam sesparamid, byte[] data, int length, out int actualDataSize)
	{
	...
	{
		return Api.Impl.JetPrereadIndexRanges(sesid, tableid, indexRanges, rangeIndex, rangeCount, out rangesPreread, columnsPreread, grbit) >= 0;
	}
