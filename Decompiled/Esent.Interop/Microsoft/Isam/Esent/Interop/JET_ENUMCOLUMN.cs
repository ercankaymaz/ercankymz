...
public class JET_ENUMCOLUMN
{
	...
	{
		return string.Format(CultureInfo.InvariantCulture, "JET_ENUMCOLUMN(0x{0:x})", columnid);
	}
	...
	internal void SetFromNativeEnumColumn(NATIVE_ENUMCOLUMN value)
	{
		...
		checked
		{
			if (JET_wrn.ColumnSingleValue == err)
			{
				cbData = (int)value.cbData;
				pvData = value.pvData;
			}
			else
			{
				cEnumColumnValue = (int)value.cEnumColumnValue;
				rgEnumColumnValue = null;
			}
		}
	}
}
