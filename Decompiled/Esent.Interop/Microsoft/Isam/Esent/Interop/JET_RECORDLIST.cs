...
public class JET_RECORDLIST
{
	...
	{
		return string.Format(CultureInfo.InvariantCulture, "JET_RECORDLIST(0x{0:x},{1} records)", tableid, cRecords);
	}
	...
	internal void SetFromNativeRecordlist(NATIVE_RECORDLIST value)
	{
		...
		cRecords = checked((int)value.cRecords);
		columnidBookmark = new JET_COLUMNID
		{
