...
internal static class EsentExceptionHelper
{
	public static EsentErrorException JetErrToException(JET_err err)
	{
		...
		default:
		{
			IntPtr paramValue = new IntPtr((int)err);
			string paramString;
			int num = Api.Impl.JetGetSystemParameter(JET_INSTANCE.Nil, JET_SESID.Nil, JET_param.ErrorToString, ref paramValue, out paramString, 1024);
			err = (JET_err)paramValue.ToInt32();
			if (num != 0)
			{
				paramString = "Unknown error";
			}
			return new EsentErrorException(paramString, err);
		}
		}
	}
}
