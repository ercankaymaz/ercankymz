...
public struct JET_BKLOGTIME : IEquatable<JET_BKLOGTIME>, IJET_LOGTIME, INullableJetStruct
{
	...
	public bool fOSSnapshot => (bFiller2 & 1) != 0;
	...
	internal JET_BKLOGTIME(DateTime time, bool isSnapshot)
	{
		checked
		{
			...
			bFiller2 = unchecked((byte)(isSnapshot ? 1 : 0));
	...
	{
		return string.Format(CultureInfo.InvariantCulture, "JET_BKLOGTIME({0}:{1}:{2}:{3}:{4}:{5}:0x{6:x}:0x{7:x})", bSeconds, bMinutes, bHours, bDays, bMonth, bYear, bFiller1, bFiller2);
	}
	...
	public override bool Equals(object obj)
	{
		...
		return Equals((JET_BKLOGTIME)obj);
	}
