...
internal static class Util
{
	...
	public static string DumpBytes(byte[] data, int offset, int count)
	{
		...
		if (offset < 0 || count < 0 || offset >= data.Length || checked(offset + count) > data.Length)
		{
			return "<invalid>";
		}
		...
		if (count > 8)
		{
			stringBuilder.AppendFormat(CultureInfo.InvariantCulture, "... ({0} bytes)", count);
		}
	...
	public static T[] DeepCloneArray<T>(T[] value) where T : class, IDeepCloneable<T>
	{
		...
		if (value != null)
		{
			...
			{
				array[i] = ((value[i] == null) ? null : value[i].DeepClone());
			}
		}
		return array;
	}
	...
	public static byte[] ConvertToNullTerminatedUnicodeByteArray(string value)
	{
		...
		checked
		{
			byte[] array = new byte[Encoding.Unicode.GetByteCount(value) + 2];
