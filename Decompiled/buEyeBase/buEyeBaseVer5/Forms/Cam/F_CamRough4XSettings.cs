...
public class F_CamRough4XSettings : Form
{
	...
	public void Init()
	{
		...
		spn_depthstep.Value = Settings.Steps.DepthStep;
		...
		if (!Settings.Operations.isCircularCam)
		{
			...
			spn_rightoffset.Value = Settings.Operations.BorderMinOffset.X;
			spn_leftoffset.Value = Settings.Operations.BorderMaxOffset.X;
		}
		else
		{
	...
	public void Apply()
	{
		Settings.Steps.DepthStep = spn_depthstep.Value;
		...
		if (!Settings.Operations.isCircularCam)
		{
			...
			Settings.Operations.BorderMinOffset.X = spn_rightoffset.Value;
			Settings.Operations.BorderMaxOffset.X = spn_leftoffset.Value;
		}
		else
		{
			if (Settings.Strategy.RotaryAxis == VectorType.YVector)
	...
	internal void method_1(object sender, EventArgs e)
	{
		try
		{
			...
			if (control.Name == btn_stocksettings.Name)
			{
				F_CamStockOffset f_CamStockOffset = new F_CamStockOffset();
				f_CamStockOffset.Settings = Settings;
				f_CamStockOffset.PropertiesForm.FormCloseMode = FormCloseModeType.Dispose;
				f_CamStockOffset.PropertiesForm.FormPosition = FormStartPosition.CenterParent;
				f_CamStockOffset.Init();
				f_CamStockOffset.ShowDialog();
				if (f_CamStockOffset.PropertiesForm.Result == DialogResult.OK)
				{
					Settings = f_CamStockOffset.Settings;
				}
			}
		}
		catch (Exception)
