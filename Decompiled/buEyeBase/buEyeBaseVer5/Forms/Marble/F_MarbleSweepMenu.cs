...
public class F_MarbleSweepMenu : Form
{
	...
	public MarbleSweepMenuType SweepType = MarbleSweepMenuType.Editor;
	...
	public void LoadLanguage()
	{
		try
		{
			lbl_contourtype.Text = buLangTranslate.preDef.Sweep + " " + buLangTranslate.preDef.Type;
			...
			buGround1.Text = buLangTranslate.preDef.Sweep + " " + buLangTranslate.preDef.Menu;
		}
		catch (Exception)
		{
		}
			...
			if (!(control.Name == btn_strategy.Name))
			{
				if (!(control.Name == btn_toolsettings.Name))
				{
					if (!(control.Name == btn_camsettings.Name))
					{
						...
						{
							SweepType = MarbleSweepMenuType.Editor;
						}
						if (control.Name == btn_contourmenufilelist.Name)
						{
							SweepType = MarbleSweepMenuType.FromList;
						}
						if (control.Name == btn_contourmenufromfile.Name)
						{
							SweepType = MarbleSweepMenuType.FromFile;
						}
						if (PropertiesForm.FormCloseMode == FormCloseModeType.Dispose)
						{
							Dispose();
