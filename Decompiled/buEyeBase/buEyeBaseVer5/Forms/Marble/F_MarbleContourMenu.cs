...
public class F_MarbleContourMenu : Form
{
	...
	public MarbleContourMenuType ContourType = MarbleContourMenuType.FromList;
	...
	public void LoadLanguage()
	{
		try
		{
			lbl_contourtype.Text = buLangTranslate.preDef.Contour + " " + buLangTranslate.preDef.Type;
			...
			btn_digitizer.Text = buLangTranslate.preDef.Image + " " + buLangTranslate.preDef.Contour;
			...
			buGround1.Text = buLangTranslate.preDef.Contour + " " + buLangTranslate.preDef.Menu;
					...
					if (!(control.Name == btn_toolsettings.Name))
					{
						if (!(control.Name == btn_camsettings.Name))
						{
							if (!(control.Name == btn_objectlocation.Name))
							{
								...
								{
									ContourType = MarbleContourMenuType.Editor;
								}
								if (control.Name == btn_contourmenufilelist.Name)
								{
									ContourType = MarbleContourMenuType.FromList;
								}
								if (control.Name == btn_contourmenufromfile.Name)
								{
									ContourType = MarbleContourMenuType.FromFile;
								}
								if (control.Name == btn_digitizer.Name)
								{
									ContourType = MarbleContourMenuType.Digitizer;
								}
								if (PropertiesForm.FormCloseMode == FormCloseModeType.Dispose)
								{
									Dispose();
							...
							else
							{
								...
								f_ObjectLocation.Properties.FormPosition = FormStartPosition.CenterParent;
