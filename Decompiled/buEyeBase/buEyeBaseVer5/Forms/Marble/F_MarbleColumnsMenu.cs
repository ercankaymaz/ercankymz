...
public class F_MarbleColumnsMenu : Form
{
	...
	public MarbleColumnsMenuType ColumnsType = MarbleColumnsMenuType.Editor;
	...
	public void LoadLanguage()
	{
		try
		{
			lbl_contourtype.Text = buLangTranslate.preDef.Columns + " " + buLangTranslate.preDef.Type;
			...
			buGround1.Text = buLangTranslate.preDef.Columns + " " + buLangTranslate.preDef.Menu;
		}
		catch (Exception)
		{
		}
			...
			if (!(control.Name == btn_strategy.Name))
			{
				if (!(control.Name == btn_toolsettings.Name))
				{
					if (!(control.Name == btn_camsettings.Name))
					{
						...
						{
							ColumnsType = MarbleColumnsMenuType.Editor;
						}
						if (control.Name == btn_contourmenufilelist.Name)
						{
							ColumnsType = MarbleColumnsMenuType.FromList;
						}
						if (control.Name == btn_contourmenufromfile.Name)
						{
							ColumnsType = MarbleColumnsMenuType.FromFile;
						}
						if (PropertiesForm.FormCloseMode == FormCloseModeType.Dispose)
						{
							Dispose();
