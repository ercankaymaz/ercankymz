...
public class F_MarbleDrillMenu : Form
{
	...
	public MarbleDrillMenuType DrillType = MarbleDrillMenuType.Editor;
	...
	public void Init()
	{
		...
		{
			MenuType.selectedTool = MarbleToolType.Milling;
		}
		LoadLanguage();
		ToolToImageIndex();
		...
		btn_coordinate.Display.BackColor = clrButtonDisplay;
		btn_coordinate.ButtonDownDisplay.BackColor = clrButtonDown;
		btn_coordinate.ButtonOverDisplay.BackColor = clrButtonOver;
		btn_editor.Display.BackColor = clrButtonDisplay;
		btn_editor.ButtonDownDisplay.BackColor = clrButtonDown;
		btn_editor.ButtonOverDisplay.BackColor = clrButtonOver;
		...
		btn_fromtable.Display.BackColor = clrButtonDisplay;
		btn_fromtable.ButtonDownDisplay.BackColor = clrButtonDown;
		btn_fromtable.ButtonOverDisplay.BackColor = clrButtonOver;
	...
	public void LoadLanguage()
	{
		try
		{
			lbl_contourtype.Text = buLangTranslate.preDef.Drill + " " + buLangTranslate.preDef.Type;
			...
			btn_fromtable.Text = buLangTranslate.preDef.FromTable;
			btn_coordinate.Text = buLangTranslate.preDef.Coordinate;
			btn_strategy.Text = buLangTranslate.preDef.Strategy;
			btn_tool.Text = buLangTranslate.preDef.Tool;
			buGround1.Text = buLangTranslate.preDef.Drill + " " + buLangTranslate.preDef.Menu;
		}
		catch (Exception)
		{
		}
			...
			if (!(control.Name == btn_strategy.Name))
			{
				if (!(control.Name == btn_toolsettings.Name))
				{
					if (!(control.Name == btn_camsettings.Name))
					{
						...
						{
							DrillType = MarbleDrillMenuType.Editor;
						}
						if (control.Name == btn_fromfile.Name)
						{
							DrillType = MarbleDrillMenuType.FromFile;
						}
						if (control.Name == btn_fromtable.Name)
						{
							DrillType = MarbleDrillMenuType.FromTable;
						}
						if (control.Name == btn_coordinate.Name)
						{
							DrillType = MarbleDrillMenuType.Coordinate;
						}
						if (PropertiesForm.FormCloseMode == FormCloseModeType.Dispose)
						{
							Dispose();
					...
					{
						okCommandWithFiveDataEventHandler_0("CamSetting", MenuType.selectedTool, ItemType, null, null);
					}
				}
				else if (okCommandWithFiveDataEventHandler_0 != null)
				{
