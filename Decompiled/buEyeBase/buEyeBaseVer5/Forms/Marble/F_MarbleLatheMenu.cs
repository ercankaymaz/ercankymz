...
public class F_MarbleLatheMenu : Form
{
	...
	public MarbleLatheMenuType LatheType = MarbleLatheMenuType.Editor;
	...
	public void LoadLanguage()
	{
		try
		{
			lbl_contourtype.Text = buLangTranslate.preDef.Lathe + " " + buLangTranslate.preDef.Type;
			...
			buGround1.Text = buLangTranslate.preDef.Lathe + " " + buLangTranslate.preDef.Menu;
		}
		catch (Exception)
		{
		}
	...
	internal void method_1(object sender, EventArgs e)
	{
			...
			if (!(control.Name == btn_strategy.Name))
			{
				if (!(control.Name == btn_toolsettings.Name))
				{
					if (!(control.Name == btn_camsettings.Name))
					{
						...
						{
							LatheType = MarbleLatheMenuType.Editor;
						}
						if (control.Name == btn_contourmenufilelist.Name)
						{
							LatheType = MarbleLatheMenuType.FromList;
						}
						if (control.Name == btn_contourmenufromfile.Name)
						{
							LatheType = MarbleLatheMenuType.FromFile;
						}
						if (PropertiesForm.FormCloseMode == FormCloseModeType.Dispose)
						{
							Dispose();
		...
		{
			F_MarbleToolMillingSawMenu f_MarbleToolMillingSawMenu = new F_MarbleToolMillingSawMenu();
			f_MarbleToolMillingSawMenu.ToolType = MenuType.selectedTool;
			f_MarbleToolMillingSawMenu.PropertiesForm.FormCloseMode = FormCloseModeType.Dispose;
			f_MarbleToolMillingSawMenu.PropertiesForm.FormPosition = FormStartPosition.CenterParent;
			f_MarbleToolMillingSawMenu.Init();
			f_MarbleToolMillingSawMenu.ShowDialog();
			if (f_MarbleToolMillingSawMenu.PropertiesForm.Result == DialogResult.OK)
			{
				MenuType.selectedTool = f_MarbleToolMillingSawMenu.ToolType;
				ToolToImageIndex();
				StrategyFromToolType();
			}
		}
