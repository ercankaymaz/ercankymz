...
{
	private string string_0 = "F_MarbleEventBerak";
	...
	public void Init()
	{
		...
		try
		{
			...
			LoadLanguage();
			spn_breakdistance.Value = buMarbleCalc.varMarbleRunSettings.BreakDistance;
			if (buMarbleCalc.varMarbleRunSettings.BreakType != MarbleBreakType.Distance)
			{
	...
	public void LoadLanguage()
	{
		...
		try
		{
			ground_base.Text = buLangTranslate.preDef.Break;
			spn_breakdistance.Caption.Caption = buLangTranslate.preDef.Distance;
			radioButton_1.Text = buLangTranslate.preDef.Break + " - " + buLangTranslate.preDef.Distance;
			radioButton_0.Text = buLangTranslate.preDef.Break + " - " + buLangTranslate.preDef.ByMouse;
	...
	public void Apply()
	{
		...
		try
		{
			buMarbleCalc.varMarbleRunSettings.BreakDistance = spn_breakdistance.Value;
			if (!radioButton_1.Checked)
			{
				buMarbleCalc.varMarbleRunSettings.BreakType = MarbleBreakType.MouseClick;
			}
			else
			{
				buMarbleCalc.varMarbleRunSettings.BreakType = MarbleBreakType.Distance;
			}
		}
		catch (Exception ex)
		{
