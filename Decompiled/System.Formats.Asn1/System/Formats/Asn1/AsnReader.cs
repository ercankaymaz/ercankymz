...
public class AsnReader
{
	...
	public bool TryReadPrimitiveBitString(out int unusedBitCount, out ReadOnlyMemory<byte> value, Asn1Tag? expectedTag = null)
	{
		...
		int bytesConsumed;
		bool num = AsnDecoder.TryReadPrimitiveBitString(_data.Span, RuleSet, out unusedBitCount, out value2, out bytesConsumed, expectedTag);
		if (num)
		{
	...
	{
		int bytesConsumed;
		bool num = AsnDecoder.TryReadBitString(_data.Span, destination, RuleSet, out unusedBitCount, out bytesConsumed, out bytesWritten, expectedTag);
		if (num)
		{
			_data = _data.Slice(bytesConsumed);
		}
	...
	{
		int bytesConsumed;
		bool num = AsnDecoder.TryReadOctetString(_data.Span, destination, RuleSet, out bytesConsumed, out bytesWritten, expectedTag);
		if (num)
		{
			_data = _data.Slice(bytesConsumed);
		}
	...
	public bool TryReadPrimitiveOctetString(out ReadOnlyMemory<byte> contents, Asn1Tag? expectedTag = null)
	{
		...
		int bytesConsumed;
		bool num = AsnDecoder.TryReadPrimitiveOctetString(_data.Span, RuleSet, out value, out bytesConsumed, expectedTag);
		if (num)
		{
	...
	public bool TryReadPrimitiveCharacterStringBytes(Asn1Tag expectedTag, out ReadOnlyMemory<byte> contents)
	{
		ReadOnlySpan<byte> value;
		int bytesConsumed;
		bool num = AsnDecoder.TryReadPrimitiveCharacterStringBytes(_data.Span, RuleSet, expectedTag, out value, out bytesConsumed);
		if (num)
		{
			contents = AsnDecoder.Slice(_data, value);
			_data = _data.Slice(bytesConsumed);
	...
	{
		int bytesConsumed;
		bool num = AsnDecoder.TryReadCharacterStringBytes(_data.Span, destination, RuleSet, expectedTag, out bytesConsumed, out bytesWritten);
		if (num)
		{
			_data = _data.Slice(bytesConsumed);
		}
