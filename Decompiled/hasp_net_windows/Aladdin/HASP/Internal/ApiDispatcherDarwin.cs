...
internal class ApiDispatcherDarwin : ApiDispatcher
{
	public HaspStatus disp_login(int feature_id, string vendor_code, ref int handle)
	{
		...
		HaspStatus result = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_login(feature_id, zero, ref handle);
		if (zero != IntPtr.Zero)
		{
	...
	public HaspStatus disp_login_scope(int feature_id, string scope, string vendor_code, ref int handle)
	{
		...
		HaspStatus result = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_login_scope(feature_id, zero2, zero, ref handle);
		if (zero != IntPtr.Zero)
		{
			Marshal.FreeHGlobal(zero);
		}
	...
	public HaspStatus disp_encrypt(int handle, byte[] data)
	{
		...
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_encrypt(handle, zero, data.Length);
	...
	public HaspStatus disp_encrypt(int handle, double[] data)
	{
		...
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_encrypt(handle, zero, data.Length * 8);
		if (haspStatus == HaspStatus.StatusOk)
		{
			Marshal.Copy(zero, data, 0, data.Length);
		}
	...
	public HaspStatus disp_encrypt(int handle, short[] data)
	{
		...
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_encrypt(handle, zero, data.Length * 2);
		if (haspStatus == HaspStatus.StatusOk)
		{
			Marshal.Copy(zero, data, 0, data.Length);
		}
	...
	public HaspStatus disp_encrypt(int handle, int[] data)
	{
		...
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_encrypt(handle, zero, data.Length * 4);
		if (haspStatus == HaspStatus.StatusOk)
		{
			Marshal.Copy(zero, data, 0, data.Length);
		}
	...
	public HaspStatus disp_decrypt(int handle, byte[] data)
	{
		...
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_decrypt(handle, zero, data.Length);
		if (haspStatus == HaspStatus.StatusOk)
		{
			Marshal.Copy(zero, data, 0, data.Length);
		}
	...
	public HaspStatus disp_decrypt(int handle, double[] data)
	{
		...
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_decrypt(handle, zero, data.Length * 8);
		if (haspStatus == HaspStatus.StatusOk)
		{
			Marshal.Copy(zero, data, 0, data.Length);
		}
	...
	public HaspStatus disp_decrypt(int handle, short[] data)
	{
		...
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_decrypt(handle, zero, data.Length * 2);
		if (haspStatus == HaspStatus.StatusOk)
		{
			Marshal.Copy(zero, data, 0, data.Length);
		}
	...
	public HaspStatus disp_decrypt(int handle, int[] data)
	{
		...
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_decrypt(handle, zero, data.Length * 4);
		if (haspStatus == HaspStatus.StatusOk)
		{
			Marshal.Copy(zero, data, 0, data.Length);
		}
	...
	public HaspStatus disp_read(int handle, int fileid, int offset, int length, byte[] buffer)
	{
		...
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_read(handle, fileid, offset, length, zero);
		if (haspStatus == HaspStatus.StatusOk)
		{
	...
	public HaspStatus disp_read(int handle, int fileid, int offset, ref string buffer)
	{
		...
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_read(handle, fileid, offset, 1, ref buffer2);
		if (haspStatus == HaspStatus.StatusOk)
		{
	...
	public HaspStatus disp_write(int handle, int fileid, int offset, byte[] buffer)
	{
		...
		HaspStatus result = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_write(handle, fileid, offset, buffer.Length, zero);
		if (zero != IntPtr.Zero)
		{
			Marshal.FreeHGlobal(zero);
		}
	...
	public HaspStatus disp_write(int handle, int fileid, int offset, string buffer)
	{
		...
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_write(handle, fileid, offset, 1, ref zero);
		if (haspStatus == HaspStatus.StatusOk)
		{
	...
	public HaspStatus disp_legacy_encrypt(int handle, byte[] data)
	{
		...
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_legacy_encrypt(handle, zero, data.Length);
		if (haspStatus == HaspStatus.StatusOk)
		{
			Marshal.Copy(zero, data, 0, data.Length);
		}
	...
	public HaspStatus disp_legacy_encrypt(int handle, double[] data)
	{
		...
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_legacy_encrypt(handle, zero, data.Length * 8);
		if (haspStatus == HaspStatus.StatusOk)
		{
			Marshal.Copy(zero, data, 0, data.Length);
		}
	...
	public HaspStatus disp_legacy_encrypt(int handle, short[] data)
	{
		...
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_legacy_encrypt(handle, zero, data.Length * 2);
		if (haspStatus == HaspStatus.StatusOk)
		{
			Marshal.Copy(zero, data, 0, data.Length);
		}
	...
	public HaspStatus disp_legacy_encrypt(int handle, int[] data)
	{
		...
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_legacy_encrypt(handle, zero, data.Length * 4);
		if (haspStatus == HaspStatus.StatusOk)
		{
			Marshal.Copy(zero, data, 0, data.Length);
		}
	...
	public HaspStatus disp_legacy_decrypt(int handle, byte[] data)
	{
		...
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_legacy_decrypt(handle, zero, data.Length);
		if (haspStatus == HaspStatus.StatusOk)
		{
			Marshal.Copy(zero, data, 0, data.Length);
		}
	...
	public HaspStatus disp_legacy_decrypt(int handle, double[] data)
	{
		...
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_legacy_decrypt(handle, zero, data.Length * 8);
		if (haspStatus == HaspStatus.StatusOk)
		{
			Marshal.Copy(zero, data, 0, data.Length);
		}
	...
	public HaspStatus disp_legacy_decrypt(int handle, short[] data)
	{
		...
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_legacy_decrypt(handle, zero, data.Length * 2);
		if (haspStatus == HaspStatus.StatusOk)
		{
			Marshal.Copy(zero, data, 0, data.Length);
		}
	...
	public HaspStatus disp_legacy_decrypt(int handle, int[] data)
	{
		...
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_legacy_decrypt(handle, zero, data.Length * 4);
		if (haspStatus == HaspStatus.StatusOk)
		{
			Marshal.Copy(zero, data, 0, data.Length);
		}
	...
	public HaspStatus disp_get_info(string scope, string format, string vendor_code, ref string info)
	{
		...
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_get_info(zero, zero2, zero3, ref info2);
		if (haspStatus == HaspStatus.StatusOk)
		{
	...
	public HaspStatus disp_get_sessioninfo(int handle, string format, ref string info)
	{
		...
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_get_sessioninfo(handle, zero, ref info2);
		if (haspStatus == HaspStatus.StatusOk)
		{
			info = Marshal.PtrToStringAnsi(info2);
		}
	...
	public HaspStatus disp_update(string update_data, ref string ack_data)
	{
		...
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_update(zero, ref ack_data2);
		if (haspStatus == HaspStatus.StatusOk)
		{
	...
	public HaspStatus disp_detach(string detach_action, string scope, string vendor_code, string recipient, ref string info)
	{
		...
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_detach(zero2, zero, zero3, zero4, ref info2);
		if (haspStatus == HaspStatus.StatusOk)
		{
			info = Marshal.PtrToStringAnsi(info2);
		}
	...
	public HaspStatus disp_detach(string detach_action, string scope, byte[] vendor_code, string recipient, ref string info)
	{
		...
		zero3 = Marshal.StringToHGlobalAnsi(Encoding.ASCII.GetString(vendor_code));
		zero4 = Marshal.StringToHGlobalAnsi(recipient);
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_detach(detach_action, scope, vendor_code, recipient, ref info2);
		if (haspStatus == HaspStatus.StatusOk)
		{
			info = info2;
		}
	...
	public HaspStatus disp_transfer(string action, string scope, string vendor_code, string recipient, ref string info)
	{
		...
		zero3 = Marshal.StringToHGlobalAnsi(vendor_code);
		zero4 = Marshal.StringToHGlobalAnsi(recipient);
		HaspStatus haspStatus = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_transfer(zero2, zero, zero3, zero4, ref info2);
		if (haspStatus == HaspStatus.StatusOk)
		{
			info = Marshal.PtrToStringAnsi(info2);
		}
	...
	public HaspStatus disp_get_version(ref int major_version, ref int minor_version, ref int build_server, ref int build_number, string vendor_code)
	{
		IntPtr zero = IntPtr.Zero;
		zero = Marshal.StringToHGlobalAnsi(vendor_code);
		HaspStatus result = Aladdin.HASP.Internal.NativeMethodsDarwin.NativeMethods.hasp_get_version(ref major_version, ref minor_version, ref build_server, ref build_number, zero);
		if (zero != IntPtr.Zero)
		{
			Marshal.FreeHGlobal(zero);
		}
