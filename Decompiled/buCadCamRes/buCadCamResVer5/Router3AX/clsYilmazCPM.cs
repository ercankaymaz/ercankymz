...
{
	private string string_0 = "clsYilmazCPM";
	...
	public void AnalyzeCode(Router3AXItem Job, ref List<camTp> Cams)
	{
		...
		try
		{
			...
			for (int i = 0; i <= Job.CamList.Count - 1; i++)
			{
				...
				camTp2.PreCodes.Add("( " + Job.CamList[i].CamName + " " + buLangTranslate.preDef.Tool);
				for (int j = 0; j <= camTp2.CamPoints.Count - 1; j++)
				{
					...
					for (int k = 0; k <= camTp2.CamPoints[j].Points.Count - 1; k++)
					{
						...
						if (tpPnt9D.PlungeAxisMovement)
						{
							...
							{
								tpPnt9D.AfterCodes.Add("M75");
							}
							tpPnt9D.AfterCodes.Add("G43 H" + camTp2.Tool.Data.No + " Z" + tpPnt9D.P9.Z.ToString("f2"));
						}
						if (tpPnt9D.LeaveAxisMovement)
						{
							if (clsRouter3AX.varRouter3AXSettings.M75Mode && j < camTp2.CamPoints.Count - 1)
							{
								tpPnt9D.AfterCodes.Add("M75");
							}
							tpPnt9D.AfterCodes.Add("G43 H" + camTp2.Tool.Data.No + " Z" + tpPnt9D.P9.Z.ToString("f2"));
						}
						if (((tpPnt9D.PlungeAxisMovement && !flag) & (ccVars.PostActive.EnterToPattern.MoveFirstPoint.AfterCode.Count > 0)) && j == 0)
						{
				...
				bool flag2 = true;
				if (i < Job.CamList.Count - 1 && Job.CamList[i].Tool.Data.No == Job.CamList[i + 1].Tool.Data.No)
				{
					flag2 = false;
				}
				if (flag2)
				{
					...
					{
						camTp2.CamPoints[camTp2.CamPoints.Count - 1].AfterCodes.AddRange(ccVars.PostActive.LeaveFromPattern.SafeDistance.AfterCode);
					}
				...
				if (clsRouter3AX.varRouter3AXSettings.TableType != MachineTableType.OnlyA)
				{
					if (clsRouter3AX.varRouter3AXSettings.TableType != MachineTableType.OnlyB)
					{
						if (clsRouter3AX.varRouter3AXSettings.TableType == MachineTableType.SingleTable)
						{
							...
							postProcessor.StartLines.Insert(0, "( Sheet [ " + Job.Index + " ] - Size = " + num.ToString("f1") + " x " + num2.ToString("f1"));
