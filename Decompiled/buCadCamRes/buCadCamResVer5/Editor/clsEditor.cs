...
public class clsEditor
{
  ...
  private void SaveBuCadEditorFile(string FileName)
  {
    ...
    try
    {
      ...
      WriteFileParams writeFileParams = new WriteFileParams(clsItem.frmEditor.viewport.Document);
  ...
  public void cmdOpenLib()
  {
    ...
    if (openFileDialog.ShowDialog() != DialogResult.OK)
      return;
    DirectoryInfo extractionDirectory = new DirectoryInfo(AppPath.Base + "\\L");
    if (extractionDirectory.Exists)
      extractionDirectory.Delete(true);
    Directory.CreateDirectory(extractionDirectory.FullName);
    buFile5.ExtractToFolder(extractionDirectory.FullName, openFileDialog.FileName);
  ...
  public void cmdSaveLib(Design Viewport)
  {
    ...
    for (int index = 0; index <= Viewport.Entities.Count - 1; ++index)
    {
      if (Viewport.Entities[index] is SketchEntity)
      {
        ...
        writeFile.DoWork();
        this.SaveLibraryToZip();
        (Viewport.Entities[index] as SketchEntity).Edit((IDesign) Viewport);
        Viewport.Entities.Regen();
        Viewport.Invalidate();
        break;
      }
  ...
  {
    for (int index = 0; index < 2 && clsItem.frmEditor.viewport.Entities.Count > 0; ++index)
    {
  ...
  public bool Offset(Entity selEntity, Point3D refPoint, ref Entity entityOffseted)
  {
    ...
    if (curve1 == null)
      return false;
    if (Math.Abs(offsetValue) <= 1E-09)
    {
      buString5.MessageBoxWarning($"{buLangTranslate.preDef.Offset} {buLangTranslate.preDef.Value} =  0");
      this.Reset();
      return false;
    }
    ICurve[] positiveOffsets = curve1.Offset(offsetValue, Vector3D.AxisZ, true);
    ICurve[] negativeOffsets = curve1.Offset(-offsetValue, Vector3D.AxisZ, true);
    ...
    double t;
    if (positive != null && positive.Project(refPoint, out t))
      positiveDistance = positive.PointAt(t).DistanceTo(refPoint);
    if (negative != null && negative.Project(refPoint, out t))
      negativeDistance = negative.PointAt(t).DistanceTo(refPoint);
    ICurve selectedOffset = positiveDistance <= negativeDistance ? positive : negative;
    if (selectedOffset == null)
      return false;
    entityOffseted = (Entity) selectedOffset;
    return true;
  }
  ...
  public void UndoGetBack()
  {
    ...
    {
      Entity copiedEntity = (Entity) null;
      buEntity.Copy(snapshot[index], ref copiedEntity);
      if (copiedEntity != null)
        clsItem.frmEditor.viewport.Entities.Add(copiedEntity);
    }
    clsItem.frmEditor.viewport.Entities.RegenAllCurved();
    ...
    try
    {
      ...
      if (fileInfo.Exists)
      {
        ...
        try
        {
          ...
          if (CalcList.Count > 0)
          {
            buSerilization5.Decode(CalcList, "", SerilizationMode5.MultiLine, (object) clsVar.varEditorSet);
          ...
          if (CalcList.Count > 0)
          {
            buSerilization5.Decode(CalcList, "", SerilizationMode5.MultiLine, (object) clsVar.varEditorRuntimeSet);
