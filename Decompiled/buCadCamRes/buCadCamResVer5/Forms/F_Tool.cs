...
public class F_Tool : Form
{
  ...
  public void Init()
  {
    ...
    this.SetAxisLimitValue(this.numericUpDown_19, this.Tool.Limits.AxesMinLimits.A);
    this.SetAxisLimitValue(this.numericUpDown_21, this.Tool.Limits.AxesMinLimits.B);
    this.SetAxisLimitValue(this.numericUpDown_18, this.Tool.Limits.AxesMinLimits.C);
    this.SetAxisLimitValue(this.numericUpDown_20, this.Tool.Limits.AxesMaxLimits.A);
    this.SetAxisLimitValue(this.numericUpDown_22, this.Tool.Limits.AxesMaxLimits.B);
    this.SetAxisLimitValue(this.numericUpDown_17, this.Tool.Limits.AxesMaxLimits.C);
    this.SetAxisLimitValue(this.numericUpDown_13, this.Tool.Limits.AxesMinLimits.X);
    this.SetAxisLimitValue(this.numericUpDown_15, this.Tool.Limits.AxesMinLimits.Y);
    this.SetAxisLimitValue(this.numericUpDown_12, this.Tool.Limits.AxesMinLimits.Z);
    this.SetAxisLimitValue(this.numericUpDown_14, this.Tool.Limits.AxesMaxLimits.X);
    this.SetAxisLimitValue(this.numericUpDown_16, this.Tool.Limits.AxesMaxLimits.Y);
    this.SetAxisLimitValue(this.numericUpDown_11, this.Tool.Limits.AxesMaxLimits.Z);
    ...
    this.InitializeAxisLimitValidation();
  ...
  private void SaveMachineLimitsClick(object sender, EventArgs e)
  {
    ...
    {
      buString5.MessageBoxWarning("Administrator authorization is required to change XYZ/ABC machine limits.");
      return;
    }
    ...
    if (MessageBox.Show(
          "This will replace the machine-wide XYZ/ABC safety envelope and invalidate cached G-code. Continue?",
          "Confirm Machine Limits",
    ...
    try
    {
      ...
      FiveAxisSafetyProfileStore.ValidateAgainstControllerSettings(
        AppPath.Settings,
        AppPath.MachineSettings,
        AppPath.Base,
        profile,
        out controllerSummary);
      FiveAxisSafetyProfileStore.Save(this.GetMachineProfileFilePath(), profile);
      FiveAxisPathSafety.Configure(profile);
      buLogVer5.addToList(
        nameof (F_Tool),
        nameof (SaveMachineLimitsClick),
        "FiveAxisSafety",
        "SavedAndAudited",
        controllerSummary + "; revision=" + FiveAxisPathSafety.ConfigurationRevision.ToString());
      this.machineLimitsStatusLabel.Text = "Machine profile: saved, audited and active";
      this.machineLimitsStatusLabel.ForeColor = Color.DarkGreen;
    }
    catch (Exception ex)
    {
      buLogVer5.addToList(nameof (F_Tool), nameof (SaveMachineLimitsClick), "FiveAxisSafety", "SaveFailed", ex.Message);
  ...
  private void LoadMachineLimitsClick(object sender, EventArgs e)
  {
    try
    {
      ...
      if (!FiveAxisSafetyProfileStore.TryLoadFromSettings(
            AppPath.Settings,
            AppPath.MachineSettings,
            AppPath.Base,
            out profile,
            out sourceSummary))
      {
        buString5.MessageBoxWarning("An active Settings tree containing verified machine limits was not found.");
        return;
      }
      ...
      if (!this.TryValidateAxisLimits(out validationMessage))
      {
        FiveAxisPathSafety.ClearConfiguration();
        buLogVer5.addToList(nameof (F_Tool), nameof (LoadMachineLimitsClick), "FiveAxisSafety", "ReviewRequired", validationMessage);
        this.machineLimitsStatusLabel.Text = "Machine profile: review required";
        this.machineLimitsStatusLabel.ForeColor = Color.DarkRed;
        buString5.MessageBoxWarning("Saved machine limits cannot be activated: " + validationMessage);
        return;
      }
      FiveAxisPathSafety.Configure(profile);
      buLogVer5.addToList(
        nameof (F_Tool),
        nameof (LoadMachineLimitsClick),
        "FiveAxisSafety",
        "SettingsTreeLoaded",
        sourceSummary + "; revision=" + FiveAxisPathSafety.ConfigurationRevision.ToString());
      this.machineLimitsStatusLabel.Text = "Machine profile: Settings tree verified";
      this.machineLimitsStatusLabel.ForeColor = Color.DarkGreen;
    }
    catch (Exception ex)
    {
      FiveAxisPathSafety.ClearConfiguration();
      buLogVer5.addToList(nameof (F_Tool), nameof (LoadMachineLimitsClick), "FiveAxisSafety", "LoadFailed", ex.Message);
  ...
  {
    string settingsDirectory = AppPath.MachineSettings;
    if (string.IsNullOrWhiteSpace(settingsDirectory) || !Directory.Exists(settingsDirectory))
      settingsDirectory = AppPath.Settings;
  ...
  internal void method_0(object sender, EventArgs e)
  {
    ...
    if (control2.Name == this.btn_ok.Name)
    {
      ...
      if (!this.TryValidateAxisLimits(out axisLimitError))
      {
        if (this.tabControl_0.TabPages.Contains(this.tabPage_2))
  ...
  internal void method_2(object sender, KeyEventArgs e)
  {
    ...
    if (!(e.KeyCode == Keys.Return | e.KeyCode == Keys.Tab))
      return;
    string axisLimitError;
    if (!this.IsAxisLimitControl(control2) || this.TryValidateAxisLimits(out axisLimitError))
    {
      Class5.smethod_218(this);
      this.DrawTool();
    }
