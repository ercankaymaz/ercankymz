...
public class clsFiles
{
  ...
  private static bool machineCadCamSettingsLoaded;
  ...
  {
    clsFiles.ActivateFiveAxisSafetySettings();
    ...
    clsFiles.ApplyMachineScopedCadCamSettings();
  ...
  {
    string machineConfigFile = AppPath.Settings + "\\MachineConfig.prm";
    if (AppBool.MachineMode && !string.IsNullOrWhiteSpace(AppPath.MachineSettingsCam) &&
        Directory.Exists(AppPath.MachineSettingsCam))
      machineConfigFile = AppPath.MachineSettingsCam + "\\MachineConfig.prm";
    clsFiles.SaveMachineConfig(machineConfigFile);
  }
  ...
  public static void OpenMachineConfig()
  {
    ...
    if (AppBool.MachineMode && !string.IsNullOrWhiteSpace(AppPath.MachineSettingsCam))
    {
      string machineScopedConfig = AppPath.MachineSettingsCam + "\\MachineConfig.prm";
      if (File.Exists(machineScopedConfig))
        machineConfigFile = machineScopedConfig;
    }
    clsFiles.OpenMachineConfig(machineConfigFile);
    clsFiles.ActivateFiveAxisSafetySettings();
  }

  private static void ActivateFiveAxisSafetySettings()
  {
    if (AppBool.MachineMode && !clsFiles.machineCadCamSettingsLoaded)
    {
      buCadCamResVer5.Marble.FiveAxisPathSafety.ClearConfiguration();
      buLogVer5.addToList(clsFiles.sClass, nameof (ActivateFiveAxisSafetySettings), "FiveAxisSafety", "KinematicOrPostNotLoaded");
      return;
    }
    try
    {
      buCadCamResVer5.Marble.FiveAxisSafetyProfile profile;
      string sourceSummary;
      if (!buCadCamResVer5.Marble.FiveAxisSafetyProfileStore.TryLoadFromSettings(
            AppPath.Settings,
            AppPath.MachineSettings,
            AppPath.Base,
            out profile,
            out sourceSummary))
      {
        buCadCamResVer5.Marble.FiveAxisPathSafety.ClearConfiguration();
        buLogVer5.addToList(clsFiles.sClass, nameof (ActivateFiveAxisSafetySettings), "FiveAxisSafety", "SettingsMissing");
        return;
      }
      buCadCamResVer5.Marble.FiveAxisPathSafety.Configure(profile);
      buLogVer5.addToList(
        clsFiles.sClass,
        nameof (ActivateFiveAxisSafetySettings),
        "FiveAxisSafety",
        "SettingsTreeActivated",
        sourceSummary);
    }
    catch (Exception ex)
    {
      ...
      buLogVer5.addToList(
        ...
        nameof (ActivateFiveAxisSafetySettings),
        "FiveAxisSafety",
        "SettingsTreeRejected",
        ex.Message);
    }
  }

  private static void ApplyMachineScopedCadCamSettings()
  {
    if (!AppBool.MachineMode)
      return;
    clsFiles.machineCadCamSettingsLoaded = false;
    string method = nameof (ApplyMachineScopedCadCamSettings);
    try
    {
      if (string.IsNullOrWhiteSpace(AppPath.MachineSettings) || !Directory.Exists(AppPath.MachineSettings))
        throw new DirectoryNotFoundException("The active machine Settings directory is missing.");

      string kinematicFile = Path.Combine(AppPath.MachineSettings, "Kinematic5Axis.bukinematic");
      string postFile = Path.Combine(AppPath.MachineSettings, "postMachine.bupost");
      if (!File.Exists(kinematicFile))
        throw new FileNotFoundException("The active machine Kinematic5Axis.bukinematic file is missing.", kinematicFile);
      if (!File.Exists(postFile))
        throw new FileNotFoundException("The active machine postMachine.bupost file is missing.", postFile);

      AppPath.Kinematic = AppPath.MachineSettings;
      AppPath.PostProcessor = AppPath.MachineSettings;
      clsVar.varInterface.pathKinematic = AppPath.MachineSettings;
      clsVar.varInterface.fileNameKinematic = Path.GetFileName(kinematicFile);
      clsVar.varInterface.pathPost = AppPath.MachineSettings;
      clsVar.varInterface.PostName = Path.GetFileNameWithoutExtension(postFile);

      clsFiles.OpenKinematic(kinematicFile, ref ccVars.KinematicOrjinal);
      if (ccVars.KinematicOrjinal == null)
        throw new InvalidDataException("The active five-axis kinematic file could not be decoded.");
      buCadCamResVer5.Marble.clsMarble.activeKinematic = new KinematicBase5(ccVars.KinematicOrjinal);
      buFile.OpenPostProcessorFile(postFile, ref ccVars.PostActive);
      if (ccVars.PostActive == null)
        throw new InvalidDataException("The active machine postprocessor file could not be decoded.");
      clsFiles.machineCadCamSettingsLoaded = true;

      buLogVer5.addToList(
        clsFiles.sClass,
        method,
        "FiveAxisSettings",
        "MachineScopedKinematicAndPostLoaded",
        Path.GetFileName(kinematicFile) + ";" + Path.GetFileName(postFile));
    }
    catch (Exception ex)
    {
      clsFiles.machineCadCamSettingsLoaded = false;
      buCadCamResVer5.Marble.FiveAxisPathSafety.ClearConfiguration();
      ccVars.KinematicOrjinal = (KinematicBase5) null;
      ccVars.PostActive = new PostProcessor();
      buLogVer5.addToList(clsFiles.sClass, method, "FiveAxisSettings", "MachineScopedLoadRejected", ex.Message);
    }
  }
