...
internal sealed class DefaultHtmlEncoder : HtmlEncoder
{
	private sealed class EscaperImplementation : ScalarEscaperBase
	{
		...
		internal override int EncodeUtf8(Rune value, Span<byte> destination)
		{
			...
			return -1;
			static int TryEncodeScalarAsHex(object @this, uint scalarValue, Span<byte> span)
			{
				int num = (int)((uint)BitOperations.Log2(scalarValue) / 4u + 4);
				if (SpanUtility.IsValidIndex(span, num))
				{
					span[num] = 59;
					SpanUtility.TryWriteBytes(span, 38, 35, 120, 48);
					span = span.Slice(3, num - 3);
					int num2 = span.Length - 1;
					while (SpanUtility.IsValidIndex(span, num2))
					{
		...
		internal override int EncodeUtf16(Rune value, Span<char> destination)
		{
			...
			return -1;
			static int TryEncodeScalarAsHex(object @this, uint scalarValue, Span<char> span)
			{
				int num = (int)((uint)BitOperations.Log2(scalarValue) / 4u + 4);
				if (SpanUtility.IsValidIndex(span, num))
				{
					span[num] = ';';
					SpanUtility.TryWriteChars(span, '&', '#', 'x', '0');
					span = span.Slice(3, num - 3);
					int num2 = span.Length - 1;
					while (SpanUtility.IsValidIndex(span, num2))
					{
