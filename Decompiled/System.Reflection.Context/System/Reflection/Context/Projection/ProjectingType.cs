...
internal class ProjectingType : DelegatingType, IProjectable
{
	...
	public override Assembly Assembly => _projector.ProjectAssembly(base.Assembly);

	public override Type BaseType => _projector.ProjectType(base.BaseType);

	public override MethodBase DeclaringMethod => _projector.ProjectMethodBase(base.DeclaringMethod);

	public override Type DeclaringType => _projector.ProjectType(base.DeclaringType);

	public override Module Module => _projector.ProjectModule(base.Module);

	public override Type ReflectedType => _projector.ProjectType(base.ReflectedType);
	...
	{
		StringComparison comparisonType = (((bindingAttr & BindingFlags.IgnoreCase) == BindingFlags.IgnoreCase) ? StringComparison.OrdinalIgnoreCase : StringComparison.Ordinal);
		...
		list.RemoveAll((MemberInfo member) => !string.Equals(member.Name, name, comparisonType));
		return list.ToArray();
	}
	...
	public override bool IsAssignableFrom([NotNullWhen(true)] Type c)
	{
		...
		if (projectingType == null || Projector != projectingType.Projector)
		{
			return false;
		}
		return base.UnderlyingType.IsAssignableFrom(projectingType.UnderlyingType);
	}
	...
	{
		ProjectingType projectingType = other as ProjectingType;
		if (projectingType == null || Projector != projectingType.Projector)
		{
			return false;
		}
		return base.UnderlyingType.IsEquivalentTo(projectingType.UnderlyingType);
	}

	public override bool IsInstanceOfType([NotNullWhen(true)] object o)
	{
		Type c = _projector.ProjectType(o?.GetType());
	...
	public override bool IsSubclassOf(Type c)
	{
		...
		return base.UnderlyingType.IsSubclassOf(projectingType.UnderlyingType);
	}
	...
	public override MemberInfo[] GetMembers(BindingFlags bindingAttr)
	{
		...
		MemberInfo[] array = new MemberInfo[methods.Length + constructors.Length + properties.Length + events.Length + fields.Length + nestedTypes.Length];
		int num = 0;
		Array.Copy(methods, 0, array, num, methods.Length);
		num += methods.Length;
		Array.Copy(constructors, 0, array, num, constructors.Length);
		num += constructors.Length;
		Array.Copy(properties, 0, array, num, properties.Length);
		num += properties.Length;
		Array.Copy(events, 0, array, num, events.Length);
		num += events.Length;
		Array.Copy(fields, 0, array, num, fields.Length);
		num += fields.Length;
		Array.Copy(nestedTypes, 0, array, num, nestedTypes.Length);
		num += nestedTypes.Length;
		return array;
	}
	...
	{
		if (o is ProjectingType projectingType && Projector == projectingType.Projector)
		{
			return base.UnderlyingType.Equals(projectingType.UnderlyingType);
		}
		return false;
	}
	...
	{
		return Projector.GetHashCode() ^ base.UnderlyingType.GetHashCode();
	}
}
